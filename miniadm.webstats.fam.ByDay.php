...
if(isset($_GET["www-categories"])){section_categories();exit;}
...
function js(){
	...
	$title=$tpl->javascript_parse_text("$title::".$FAMS[$_GET["fam"]]["TITLE"]);
	$suffix=suffix();
...
function tabs(){
	...
	$title=$tpl->_ENGINE_parse_body("$date {website} {$_GET["familysite"]}");
	$suffix=suffix();
	$array["{categories}"]="$page?www-categories=yes$suffix";
...
function suffix(){
	...
	$suffix="&t=$t&xtime={$_GET["xtime"]}&familysite=$familysite&fam={$_GET["fam"]}";
	return $suffix;
}
...
function section_categories(){
	...
	$t=time();
	$boot=new boostrap_form();
	$form=$boot->SearchFormGen("sitename,category","categories-search",$suffix);
	echo $form;	
...
function graph1(){
	...
	$sql="SELECT SUM(hits) as thits, `hour`,`sitename`,`catfam` FROM $tablename GROUP BY `hour`,`familysite`,`catfam`
	HAVING `familysite`='$familysite' AND `catfam`='{$_GET["fam"]}'
	ORDER BY `hour`";
...
function graph1_1(){
	...
	$sql="SELECT SUM(size) as size, `hour`,`familysite`,`catfam` FROM $tablename GROUP BY `hour`,`familysite`,`catfam`
	HAVING `familysite`='$familysite' AND `catfam`='{$_GET["fam"]}'
	ORDER BY `hour`";
...
function graph2(){
	...
	$sql="SELECT SUM(hits) as thits, familysite,sitename,catfam FROM `$tablename` 
	GROUP BY familysite,sitename ,catfam
	HAVING familysite='$familysite' AND `catfam`='{$_GET["fam"]}'
...
function graph3(){
	...
	$sql="SELECT SUM(size) as thits, familysite,sitename,catfam FROM `$tablename`
	GROUP BY familysite,sitename ,catfam
	HAVING familysite='$familysite' AND `catfam`='{$_GET["fam"]}'
	ORDER BY thits DESC LIMIT 0,10";
	$unknown=$tpl->_ENGINE_parse_body("{unknown}");
...
function graph4(){
	...
	$tablename=date("Ymd",$_GET["xtime"])."_hour";
	$familysite=mysql_escape_string2($_GET["familysite"]);
	$sql="SELECT SUM(hits) as thits, familysite,uid,catfam FROM `$tablename`
	GROUP BY familysite,uid,catfam
	HAVING familysite='$familysite' AND `catfam`='{$_GET["fam"]}'
	ORDER BY thits DESC LIMIT 0,10";
	$unknown=$tpl->_ENGINE_parse_body("{unknown}");
	$c=0;
...
function graph5(){
	...
	$sql="SELECT SUM(size) as thits, familysite,uid,catfam FROM `$tablename`
	GROUP BY familysite,uid,catfam
	HAVING familysite='$familysite' AND `catfam`='{$_GET["fam"]}'
	ORDER BY thits DESC LIMIT 0,10";
	$unknown=$tpl->_ENGINE_parse_body("{unknown}");
