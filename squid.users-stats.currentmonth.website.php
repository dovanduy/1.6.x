...
function js(){
	...
	$suffix=suffix();
	$t=time();
	$title=$tpl->_ENGINE_parse_body("{this_month}::{$_GET["familysite"]}:: {$_GET["field"]}::{$_GET["value"]}");
...
function flow_day(){
	...
	FROM {$date}_maccess GROUP BY {$_GET["field"]},tday ,familysite
	HAVING {$_GET["field"]}='{$_GET["value"]}' AND familysite='{$_GET["familysite"]}' ORDER BY tday";
	$time=time();
	
	$results = $q->QUERY_SQL($sql,"artica_events");
	
	if(!$q->ok){
		echo FATAL_ERROR_SHOW_128($q->mysql_error_html().$sql);
		return;
	...
	if(mysql_num_rows($results)<2){
		echo FATAL_ERROR_SHOW_128("{request_is_less_2}$sql");
		return;
...
function websites_table(){
	...
	$title=$tpl->javascript_parse_text("{this_month}:: {$_GET["familysite"]}:: {$_GET["field"]}:: &laquo;{$_GET["value"]}&raquo;");
	$suffixMD=md5($suffix);
	$buttons="
	buttons : [
	...
	dataType: 'json',
	colModel : [
	{display: '$zDate', name : 'zDate', width : 641, sortable : true, align: 'left'},
