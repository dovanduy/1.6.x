...
function start(){
	...
	$page=CurrentPageName();
	$html="<div id='snort-start-page' style='width:$width'></div>
	
	<script>
		LoadAjax('snort-start-page','$page?tabs=yes$newinterface');
	</script>
...
if(isset($_GET["newinterface"])){
	...
	while (list ($num, $ligne) = each ($array) ){
		...
		$html[]= "<li><a href=\"$page?$num=yes\"><span $fontsize>". $tpl->_ENGINE_parse_body($ligne)."</span></a></li>\n";
	}
	
	
	echo "
	<div id='main_config_snort' style='width: $tabswith;margin:-8px;height:$height;overflow:auto'>
		<ul>". implode("\n",$html)."</ul>
	</div>
...
<tbody class='tbody'>";		
	...
	$others="<table style='width:99%' class=form>
			<tr>
