...
$sock->getFrameWork("squid.php?videocache-query-retreiver=$search&prepend={$_GET["prepend"]}&rp={$_POST["rp"]}");
$array=explode("\n", @file_get_contents("/usr/share/artica-postfix/ressources/logs/web/videocache-retreiver"));
$total = count($array);
$data = array();
$data['page'] = $page;
$data['total'] = $total;
...
while (list ($key, $line) = each ($array) ){
	...
	$style="<span>";
	$styleoff="</span>";
	if(!preg_match("#(.+?)\s+([0-9]+)\s+([A-Z]+)\s+(.*?)\s+(.*?)\s+(.*?)\s+(.*?)\s+(.*)#",$line,$re)){continue;}
	$xdate="{$re[1]}";
	$pid=$re[2];
	$logtyp=$re[3];
	$ipaddr=$re[4];
