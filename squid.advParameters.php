...
function sizelimit_popup(){
	...
	<tr>
		<td style='font-size:14px;' class=legend>{enable}:</td>
		<td>". Field_checkbox("EnableChangeRequestSize",1,$EnableChangeRequestSize,"EnableChangeRequestSizeCheck()")."</td>
		<td colspan=2>&nbsp;</td>
	</tr>	
	<tr>
		<td style='font-size:14px;' class=legend>{request_header_max_size}:</td>
		<td>". Field_text("request_header_max_size",$request_header_max_size_v,"font-size:14px;padding:3px;width:60px")."</td>
		<td style='font-size:14px;' width=1%>&nbsp;KB</td>
		<td>". help_icon("{request_header_max_size_text}")."</td>
	</tr>
	<tr>
		<td style='font-size:14px;' class=legend>{reply_header_max_size}:</td>
		<td>". Field_text("reply_header_max_size",$reply_header_max_size,"font-size:14px;padding:3px;width:60px")."</td>
		<td style='font-size:14px;' width=1%>&nbsp;KB</td>
	...
	<tr>
		<td style='font-size:14px;' class=legend>{client_request_buffer_max_size}:</td>
		<td>". Field_text("client_request_buffer_max_size",$client_request_buffer_max_size,"font-size:14px;padding:3px;width:60px")."</td>
		<td style='font-size:14px;' width=1%>&nbsp;KB</td>
	...
	<tr>
		<td style='font-size:14px' class=legend>{request_body_max_size}:</td>
		<td>". Field_text("request_body_max_size",$request_body_max_size_v,"font-size:14px;padding:3px;width:60px")."</td>
		<td style='font-size:14px;' width=1%>&nbsp;KB</td>
		<td>". help_icon("{request_body_max_size_text}")."</td>
	</tr>	
	
	<tr>
		<td style='font-size:14px' class=legend>{reply_body_max_size}:</td>
		<td>". Field_text("reply_body_max_size",$reply_body_max_size_v,"font-size:14px;padding:3px;width:60px")."</td>
		<td style='font-size:14px;' width=1%>&nbsp;KB</td>
	...
	<tr>
		<td colspan=4 align='right'>
			<hr>
				". button("{apply}","SquidAVParamSave()",16)."
		</td>
	</tr>
	</table>
	<hr>
	<table style='width:99%' class=form>
	<tr>
		<td class=legend style='font-size:14px'>{allow_squid_localhost}</td>
		<td>". Field_checkbox("allow_squid_localhost",1,$squid->allow_squid_localhost,"EnableChangeRequestSizeCheck()")."</td>
		<td>&nbsp;</td>
	</tr>
	<tr>
		<td class=legend style='font-size:14px'>{ignore_expect_100}</td>
	...
	<tr>
		<td colspan=3 align='right'>
			<hr>
				". button("{apply}","SquidLocalHostSave()",16)."
		</td>
	</tr>	
	</table>
...
$html="
	...
	<tr>
		<td style='font-size:14px;' class=legend>{wccp2_enabled}:</td>
	...
	<tr>
		<td style='font-size:14px' class=legend nowrap>{wccp2_router}:</td>
		<td>". Field_text("wccp2_router",$squid->wccp2_router,"font-size:14px;padding:3px;width:220px")."</td>
		<td></td>
	</tr>	
	
	<tr>
		<td style='font-size:14px' class=legend nowrap>{wccp2_forwarding_method}:</td>
		<td>". Field_array_Hash($wccp2_forwarding_method_hash,"wccp2_forwarding_method",$squid->wccp2_forwarding_method)."</td>
		<td>&nbsp;</td>
	</tr>		
	
	<tr>
		<td style='font-size:14px' class=legend nowrap>{wccp2_return_method}:</td>
		<td>". Field_array_Hash($wccp2_return_method_hash,"wccp2_return_method",$squid->wccp2_return_method)."</td>
		<td>&nbsp;</td>
	</tr>	
	<tr>
		<td style='font-size:14px' class=legend nowrap>{wccp2_assignment_method}:</td>
	...
	<tr>
		<td colspan=4 align='right'>
			<hr>
				". button("{apply}","SquidWccp2ParamSave()",16)."
		</td>
	</tr>
	</table>
...
$(document).ready(function(){
$('#$t').flexigrid({
	url: '$page?http-safe-ports-list=yes',
	dataType: 'json',
	colModel : [
...
buttons : [
	{name: '$new_port', bclass: 'add', onpress : HTTPSafePortAdd},
	...
	function REFRESH_HTTP_SAFE_PORTS_LIST(){
		...
		function HttpSafePortDelete(enc,id){
			rowSquidPosrt=id;
			var XHR = new XHRConnection();
...
$(document).ready(function(){
$('#$t').flexigrid({
	url: '$page?http-safe-ports-ssl-list=yes',
	dataType: 'json',
	colModel : [
		{display: '$ports', name : 'ID', width : 90, sortable : true, align: 'left'},
		{display: '$note', name : 'TaskType', width : 394, sortable : false, align: 'left'},
...
function http_safe_ports_ssl_list(){
	...
	$data = array();$data['page'] = 1;$data['total'] = count($ports);
	$data['rows'] = array();	
	
	if($_POST["sortname"]=="ID"){
	...
	while (list ($port, $explain) = each ($ports) ){
			...
			$delete=imgtootltip("delete-24.png","{delete}","HttpSafePortSSLDelete('$port_enc','SavePort$id')");
			
			
	$data['rows'][] = array(
		'id' => "SavePort$id",
		'cell' => array("
		<strong style='font-size:16px'>$port</strong>",
		"<span style='font-size:16px'>$explain</span>",$delete )
		);
	}
...
function http_safe_ports_list(){
	...
	while (list ($port, $explain) = each ($ports) ){
			$id=md5("$port$explain");
			$port_enc=base64_encode($port);
			$delete=imgtootltip("delete-24.png","{delete}","HttpSafePortDelete('$port_enc','SavePort$id')");
