...
function status(){
	...
	if($time>4){
		...
		if(!system_is_overloaded()){
			...
			system_admin_events("-> OVERLOADED {$GLOBALS["SYSTEM_INTERNAL_LOAD"]} -> Die()",__FUNCTION__,__FILE__,__LINE__,"ipband");
		}
	}
...
function parsereports(){
	...
	foreach (glob("/var/log/ipband.*") as $filename) {
		...
		if(sendtomysql($array)){
			...
			system_admin_events("Fatal while injecting $filename",__FUNCTION__,__FILE__,__LINE__,"ipband");
		}
	}
...
function ParseResolvConf(){
	...
	while($ligne=@mysql_fetch_array($results,MYSQL_ASSOC)){
		...
		if(!$q->ok){system_admin_events("$q->mysql_error\n",__FUNCTION__,__FILE__,__LINE__,"ipband");}
	...
	if($c>0){
		...
		system_admin_events("$c Ip addresses resolved in $took",__FUNCTION__,__FILE__,__LINE__,"ipband");
	}
