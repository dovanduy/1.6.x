...
<tbody class='tbody'>";	
	...
	if($_GET["System"]==0){
			...
			WHERE (`key` = 'myhostname' AND value LIKE '$search') OR (`key` = 'myhostname' AND ip_address LIKE '$search') ORDER BY value LIMIT 0,25";
			$q=new mysql();
		$results=$q->QUERY_SQL($sql,"artica_backup");
			if(!$q->ok){echo "<H2>$q->mysql_error</H2>";}
		...
		style='font-size:14px;text-decoration:underline;color:$fontcolor'>";
		
		$html=$html."
		
		
		<tr  class=$classtr>
		<td style='font-size:14px;font-weight:bold;color:$fontcolor'><img src=img/32-network-server.png></td>
		<td style='font-size:14px;font-weight:bold;color:$fontcolor'>$href$hostname_text</a></td>
		<td style='font-size:14px;font-weight:bold;color:$fontcolor'>$href$textou</a></td>
		<td style='font-size:14px;font-weight:bold;color:$fontcolor'>$href{$ligne["ip_address"]}</a></td>
		<td style='font-size:14px;font-weight:bold;color:$fontcolor'><span id='status-instance-$c'><img src='img/ok32-grey.png'><span></td>
		<td width=1%>". Field_checkbox("DisabledInstance_$md",1,$enabled,"DisableInstance('{$ligne["value"]}','$md')")."</td>
	...
	function POSTFIX_MULTI_INSTANCE_INFOS_DEL(ou,ip){
		if(confirm('$ask_perform_operation_delete_item\\n'+ou+'('+ip+')')){
				...
				AnimateDiv('instances-list-table');
