...
ForceDirectories('/etc/artica-postfix/settings/Daemons');
writeln('Preparing  '+articaversion + ' for Wordpress Appliance...');
end;


fpsystem('echo "OK" >/etc/artica-postfix/WORDPRESS_APPLIANCE');
fpsystem('echo "OK" >/etc/artica-postfix/FROM_ISO');
fpsystem('echo "OK" >/etc/artica-postfix/artica-iso-setup-launched');
fpsystem('echo "0" >/etc/artica-postfix/settings/Daemons/ProxyUseArticaDB');
fpsystem('echo "0" >/etc/artica-postfix/settings/Daemons/StatsPerfsSquidAnswered');
fpsystem('echo "0" >/etc/artica-postfix/settings/Daemons/CacheManagement2');
fpsystem('echo "1" >/etc/artica-postfix/settings/Daemons/EnablePHPFPM');
fpsystem('echo "0" >/etc/artica-postfix/settings/Daemons/EnableArticaFrontEndToNGninx');
fpsystem('echo "0" >/etc/artica-postfix/settings/Daemons/EnableArticaInNGINX');
fpsystem('echo "1" >/etc/artica-postfix/settings/Daemons/EnableArticaFrontEndToApache');
fpsystem('echo "1" >/etc/artica-postfix/settings/Daemons/EnableNginx');
fpsystem('echo "0" >/etc/artica-postfix/settings/Daemons/EnablePHPFPMFrameWork');
fpsystem('echo "0" >/etc/artica-postfix/settings/Daemons/EnableArticaApachePHPFPM');

if not DirectoryExists('/usr/share/wordpress-src') then begin
   writeln('Installing Wordpress core');
   fpsystem('php5 /usr/share/artica-postfix/exec.wordpress.download.php');
...
fpsystem('/etc/init.d/slapd restart');
fpsystem('/etc/init.d/artica-postfix stop daemon');
fpsystem('/etc/init.d/artica-postfix start daemon');


if Not FileExists('/usr/sbin/nginx') then begin
   writeln('Downloading Nginx');
   if not WGET_DOWNLOAD_FILE('http://www.articatech.net/download/nginx/nginx-debian7-64-1.7.0.1.tar.gz','/tmp/nginx-debian7-64-1.7.0.1.tar.gz') then begin
      writeln('Failed to get NGinx package...');
      exit;
   end;
   writeln('Extracting Nginx');
   fpsystem('tar -xvf /tmp/nginx-debian7-64-1.7.0.1.tar.gz -C /');
   writeln('Build Nginx startup script');
   fpsystem('php5 /usr/share/artica-postfix/exec.initslapd.php --nginx');
   writeln('Restarting Nginx service');
   fpsystem('/etc/init.d/nginx restart');
end;

if not FileExists('usr/local/sbin/vsftpd') then begin
   if not WGET_DOWNLOAD_FILE('http://www.articatech.net/download/Debian7-squid/vsftpd-x64-debian7-3.0.2.tar.gz','/tmp/vsftpd-x64-debian7-3.0.2.tar.gz') then begin
       writeln('Failed to get NGinx package...');
      exit;
   end;
   writeln('Extracting vsftpd');
   fpsystem('tar -xvf /tmp/vsftpd-x64-debian7-3.0.2.tar.gz -C /');
   writeln('Build vsftpd startup script');
   fpsystem('php5 /usr/share/artica-postfix/exec.initslapd.php --vsftpd');
end;

fpsystem('/etc/init.d/artica-webconsole restart');
fpsystem('/etc/init.d/artica-framework restart');
fpsystem('/usr/share/artica-postfix/bin/process1 --force --verbose --123456');
...
begin
     base:=trim(base);
      if length(base)>0 then begin
             writeln('Welcome on the Artica installation process 1.0 For Wordpress appliance');
             writeln('');
...
writeln('Install '+IntToStr(length(plistBase)-1)+' package(s): [Yes]');
         ...
         writeln('Type Enter to check installation:');
         readln(u);
         InstallArtica();
