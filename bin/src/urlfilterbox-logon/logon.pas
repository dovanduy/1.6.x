...
begin
    ...
    DNS:=ParseResolvConf();
    if(length(DNS)=0) then DNS:= Gayteway;
...
//#############################################################################
function tlogon.WriteToFile(zText:string;TargetPath:string):boolean;
      var
        F : Text;
      BEGIN
      result:=true;
      ...
      forcedirectories(ExtractFilePath(TargetPath));
       EXCEPT
             writeln('Fatal error in ' +     TargetPath);
        ...
        TRY
          ...
          exit(true);
        EXCEPT
              writeln('error I/O while Writing ' +     TargetPath);
