...
count:=0;
SYS:=Tsystem.Create;
     logs.Debuglogs('Starting......: Squid Memory = "'+IntTOStr(GetFreeMem())+'"');
  ...
  if SQUIDEnable=0 then begin
      logs.Debuglogs('Starting......: Squid is disabled aborting by SQUIDEnable...'+IntTostr(SQUIDEnable));
       pid:=SQUID_PID();
        if SYS.PROCESS_EXIST(pid) then begin
         SQUID_STOP();
         exit;
...
if not DirectoryExists('/var/lib/squidguard') then begin
   ...
   fpsystem('/bin/chmod 755 /var/lib/squidguard');
end;
zapper_install();
end;

if UrlFlilterBox then mounttmpfs();


squidconf:=SQUID_CONFIG_PATH();
http_port:=SQUID_GET_CONFIG('http_port');
options:='-sYC -a '+http_port +' -f ' +squidconf;
options:='-sYC -f ' +squidconf;


if not is31ver() then options:=' -D'+options;
...
if not UrlFlilterBox then begin
  if not WithoutConfig then begin
...
fpsystem(cmd);
 ...
 pid:=SQUID_PID();
  if SYS.PROCESS_EXIST(pid) then begin
   ...
   if not UrlFlilterBox then SQUID_RRD_EXECUTE();
  end else begin
...
killbin:=SYS.LOCATE_GENERIC_BIN('kill');
  ...
  if SYS.PROCESS_EXIST(pid) then begin
     ...
     if length(trim(pid))>0 then begin
        writeln('Stopping Squid...............: Send signal to "' + pid + '" PID');
   ...
   if SYS.PROCESS_EXIST(pid) then begin
       writeln('Stopping Squid...............: "'+pid+'" Send signal to kill it...');
   ...
   writeln('Stopping Squid...............: Success: stopped');
end;
...
while oldproc>5 do begin
     writeln('Stopping Squid...............: old process: ',oldproc,' pid ...');
     sleep(100);
     inc(count);
     if count>30 then break;
...
for i:=1 to 10 do begin
    ...
    if length(pid)>0 then begin
      writeln('Stopping Squid...............: Send kill signal to daemon "' +pid+'" pidof "'+binpath+'"');
...
for i:=1 to 10 do begin
    pid:=SYS.PIDOF(binpath);
    if length(pid)>0 then begin
      writeln('Stopping Squid...............: Force kill signal to daemon "' +pid+'" pidof "'+binpath+'"');
      logs.OutputCmd('/bin/kill -9 '+pid);
    end else begin
        break;
    end;
