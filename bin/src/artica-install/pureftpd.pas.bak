...
begin
     ...
     if RegExpr.Exec(PURE_FTPD_VERSION()) then begin
         TryStrToInt(RegExpr.Match[1],Major);
         TryStrToInt(RegExpr.Match[3],Minor);
           logs.DebugLogs('Starting......: pure-ftpd Major: '+IntToSTr(Major)+' Revision:'+IntToSTr(Minor));
           if Major<1 then Upgrade:=true;
           if Minor<25 then Upgrade:=true;
     end;

     if Upgrade then begin
        logs.DebugLogs('Starting......: pure-ftpd must upgrade...');
        fpsystem('/usr/share/artica-postfix/bin/artica-make APP_PUREFTPD');
        exit;
     end;
