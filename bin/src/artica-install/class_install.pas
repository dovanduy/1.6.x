...
uses
  ...
  apt_mirror,cluebringer,apachesrc,sabnzbdplus,fusermount,vnstat,munin,greyhole,iscsitarget,postfwd2,snort,greensql,amanda,yaffas,netatalk,
   ...
   if not SYS.BuildPids() then begin
       ...
       halt(0);
           ...
           if ParamStr(2)='haproxy' then begin
               haproxy:=thaproxy.Create(GLOBAL_INI.SYS);
               haproxy.STOP();
               haproxy.free;
               exit();
            end;
          ...
          if ParamStr(2)='p3scan' then begin
           ...
           if ParamStr(2)='winbindd' then begin
                 logs.Syslogs('Warning: Request to STOP Winbindd....');
                 zsamba.WINBIND_STOP();
                 exit();
            end;
            ...
            if length(ParamStr(2))>0 then begin
               ...
               writeln('|sendmail-openemm|fuppes|cgroups|arpd|yaffas|netatalk|loadbalance|memcached|UpdateUtility|ejabberd|haproxy|arkeia|klms|klmsdb');
               exit();
            end;
...
myFile.Add('#!/bin/sh');
myFile.add('### BEGIN INIT INFO');
myFile.Add('# Provides:          Artica boot service');
