...
begin
       ...
       if not FileExists('/etc/artica-postfix/settings/Daemons/ArticaImapIniConf') then begin
          ...
          l.free;
           ...
           ListenAddress:=ini.ReadString('CYRUS','ListenAddress','');
...
if length(ListenAddress)>2 then ListenAddress:=ListenAddress+':';
...
if DisableIMAPVerif=1 then Zarafa_installed:=false;
list2.Add('}');
list2.Add('');
list2.Add('SERVICES {');
...
list2.Add('	imap		cmd="'+basepath+imapd+' -U 30" listen="'+ListenAddress+listen+'" prefork=0 maxchild='+IntToStr(service_imap_maxchild));
if service_imapssl_enabed=1 then begin
   ...
   list2.Add('	imaps		cmd="'+basepath+imapd+' -s -U 30" listen="'+ListenAddress+service_imapssl_listen+'" prefork=0 maxchild='+IntToStr(service_imapssl_maxchild)+' '+provide_uuid);
end;
...
if service_pop3_enabed=1    then begin
      ...
      list2.Add('	pop3		cmd="'+basepath+pop3d+' -U 30" listen="'+ListenAddress+service_pop3_listen+'" prefork=0 maxchild='+IntToStr(service_pop3_maxchild));
end;
...
if service_pop3ssl_enabed=1 then begin
   ...
   list2.Add('	pop3s		cmd="'+basepath+pop3d+' -s -U 30" listen="'+ListenAddress+service_pop3ssl_listen+'" prefork=0 maxchild='+IntToStr(service_pop3ssl_maxchild));
end;


if service_nntpd_enabed=1   then begin
   logs.DebugLogs('Starting......: cyrus-imapd cyrus.conf: NNTP enabled');
   list2.Add('	nntp		cmd="'+basepath+nntpd+' -U 30" listen="'+ListenAddress+service_nntpd_listen+'" prefork=0 maxchild=100'+provide_uuid);
end;

if service_nntpds_enabed=1  then begin
      logs.DebugLogs('Starting......: cyrus-imapd cyrus.conf: NNTP SSL enabled');
      list2.Add('	nntps		cmd="'+basepath+nntpd+' -s -U 30" listen="'+ListenAddress+service_nntpds_listen+'" prefork=0 maxchild=100'+provide_uuid);
end;
