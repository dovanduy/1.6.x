...
private
     ...
     EnableFetchmailScheduler:integer;
...
begin
       ...
       if not TryStrToInt(SYS.GET_INFO('EnableFetchmailScheduler'),EnableFetchmailScheduler) then EnableFetchmailScheduler:=0;
...
begin
     ...
     result:=trim(SYS.GET_CACHE_VERSION('APP_FETCHMAIL'));
  ...
  for i:=0 to FileData.Count -1 do begin
         RegExpr.Expression:='fetchmail,.+?([0-9\.]+)';
          if RegExpr.Exec(FileData.Strings[i]) then  begin
            ...
            RegExpr.Expression:='fetchmail release\s+([0-9\.]+)';
            if RegExpr.Exec(FileData.Strings[i]) then  begin
               result:=RegExpr.Match[1];
               if length(result)>1 then begin
                  SYS.SET_CACHE_VERSION('APP_FETCHMAIL',result);
