...
begin
    ...
    bind9.STOP();
    if loglevel>8 then straces:=' --log-dns-details --log-failed-updates --loglevel='+IntToStr(loglevel);
    fpsystem(SYS.LOCATE_PHP5_BIN()+' /usr/share/artica-postfix/exec.pdns.php --mysql');
    fpsystem(BIN_PATH()+' --daemon --guardian=yes --recursor=127.0.0.1:1553 --config-dir=/etc/powerdns '+straces);
    count:=0;
...
if not TryStrToInt(SYS.GET_INFO('PdnsHotSpot'),PDNSHotSpot) then PDNSHotSpot:=0;
LaunchPipe:=0;
...
pipe:='';
if PDSNInUfdb=1 then LaunchPipe:=1;
if PDNSHotSpot=1 then LaunchPipe:=1;



if LaunchPipe=1 then pipe:='pipe,';
...
if PDNSHotSpot=1 then begin
   logs.DebugLogs('Starting......: PowerDNS starting the HotSpot engine...');
   fpsystem('/bin/chmod 777 /usr/share/artica-postfix/exec.pdns.pipe.php >/dev/null 2>&1');
   l.add('pipe-command=/usr/share/artica-postfix/exec.pdns.pipe.php');
   l.add('pipebackend-abi-version=2');
   l.add('distributor-threads=2');
end else begin
    logs.DebugLogs('Starting......: PowerDNS HotSpot engine is disabled...');
end;

l.add('lazy-recursion=yes');

l.add('#local-address=0.0.0.0');
fpsystem(SYS.LOCATE_PHP5_BIN()+' /usr/share/artica-postfix/exec.pdns.php --listen-ips');

z:=Tstringlist.Create;
z.LoadFromFile('/etc/powerdns/iplist');
cdirlist:=trim(z.Strings[0]);
l.add('local-address='+cdirlist);
...
L.Clear;
if length(RecursoripAllowFrom)>0 then RecursoripAllowFrom:=cdirlist+','+RecursoripAllowFrom else RecursoripAllowFrom:=cdirlist;
