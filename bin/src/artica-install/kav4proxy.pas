...
begin
       ...
       if not SYS.ISMemoryHiger1G() then begin
              if kavicapserverEnabled=1 then logs.Syslogs('Starting......: Fatal Memory is under 1G, Kav4Proxy will be disabled');
              SYS.set_INFO('kavicapserverEnabled','0');
              SYS.set_INFO('ArticaEnableKav4ProxyInSquid','0');
              kavicapserverEnabled:=0;
              SQUIDEnable:=0;
       end else begin
...
if not SYS.PROCESS_EXIST(result) then result:=SYS.PIDOF('/opt/kaspersky/kav4proxy/sbin/kav4proxy-kavicapserver');
end;
...
logs.DebugLogs('Starting......: kavicapserverEnabled = `'+InTtOStr(kavicapserverEnabled)+'`');
if kavicapserverEnabled=0 then begin
   logs.DebugLogs('Starting......: Kaspersky For Proxy server is disabled');
    if SYS.PROCESS_EXIST(KAV4PROXY_PID()) then KAV4PROXY_STOP();
    exit;
end;
