...
//#########################################################################################
 function ttomcat.DEB_VERSION():string;
var
   l:TstringList;
   i:integer;
   RegExpr:TRegExpr;
   ...
   cmd,sbinpath:string;
begin
    sbinpath:='/usr/share/tomcat6/bin/version.sh';
    if length(sbinpath)=0 then exit;
    if Not Fileexists(sbinpath) then exit;
    result:=SYS.GET_CACHE_VERSION('APP_TOMCAT6');
     if length(result)>2 then exit;
     if not FileExists(sbinpath) then begin
        if SYS.verbosed then writeln(sbinpath,' no such file');
         exit;
     end;
 ...
 SYS.SET_CACHE_VERSION('APP_TOMCAT6',result);
l.free;
RegExpr.free;
end;
//#########################################################################################
