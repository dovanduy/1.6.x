...
public
    ...
    function     APP_SPREED():string;
    function     APP_NETATALK_VERSION():string;
...
begin
     ...
     path:='/usr/bin/dstat';
     if not FileExists(path) then begin
        logs.Debuglogs('ttoolsversions.DSTAT_VERSION():: dstat is not installed');
        exit;
     end;
...
begin
     ...
     path:='/usr/bin/winexe';
     if not FileExists(path) then begin
        logs.Debuglogs('ttoolsversions.WINEXE_VERSION():: dstat is not installed');
        exit;
     end;
...
//########################################################################################
function ttoolsversions.ARKEIA_VERSION():string;
 var
    RegExpr:TRegExpr;
    filetemp:string;
    binpath:string;
...
begin
  SYS:=Tsystem.Create();
  logs:=Tlogs.Create;
  binpath:='/opt/arkeia/bin/arktrans';

     if not FileExists(binpath) then begin
        logs.Debuglogs('ttoolsversions.ARKEIA_VERSION():: not installed');
        exit;
     end;

  result:=SYS.GET_CACHE_VERSION('APP_ARKEIA');
  if length(result)>2 then exit;
  RegExpr:=TRegExpr.Create;
  filetemp:=LOGS.FILE_TEMP();
  fpsystem(binpath+' --version >' + filetemp + ' 2>&1');
  if not FileExists(filetemp) then exit;
  RegExpr.Expression:='Arkeia Network Backup\s+([0-9\.]+)';
    l:=TstringList.Create;
    L.LoadFromFile(filetemp);
  for i:=0 to l.Count-1 do begin
   if RegExpr.Exec(l.Strings[i]) then begin
      ...
      logs.Debuglogs('ttoolsversions.APP_ARKEIA():: Not found "'+l.Strings[i]+'"');
   end;
  end;
  RegExpr.free;
  SYS.SET_CACHE_VERSION('APP_ARKEIA',result);
...
//########################################################################################
function ttoolsversions.APP_HAPROXY_VERSION():string;
 var
    RegExpr:TRegExpr;
    filetemp:string;
    binpath:string;
...
begin
  ...
  binpath:=SYS.LOCATE_GENERIC_BIN('haproxy');

     if not FileExists(binpath) then begin
        logs.Debuglogs('ttoolsversions.APP_HAPROXY_VERSION():: not installed');
        exit;
     end;
  filetemp:=logs.FILE_TEMP();
  result:=SYS.GET_CACHE_VERSION('APP_HAPROXY');
  if length(result)>2 then exit;
  fpsystem(binpath+' -v >'+filetemp +' 2>&1');
  RegExpr:=TRegExpr.Create;
  RegExpr.Expression:='HA-Proxy version\s+([0-9\.a-z\-]+)';
  l:=TstringList.Create;
  L.LoadFromFile(filetemp);
  logs.DeleteFile(filetemp);
  for i:=0 to l.Count-1 do begin
   if RegExpr.Exec(l.Strings[i]) then begin
      ...
      logs.Debuglogs('ttoolsversions.APP_HAPROXY_VERSION():: Not found "'+l.Strings[i]+'"');
   end;
  end;
  RegExpr.free;
  l.free;
  SYS.SET_CACHE_VERSION('APP_HAPROXY',result);

end;
//########################################################################################
function ttoolsversions.APP_NETATALK_VERSION():string;
 var
    RegExpr:TRegExpr;
    filetemp:string;
    binpath:string;
...
begin
  ...
  binpath:=SYS.LOCATE_GENERIC_BIN('afpd');

     if not FileExists(binpath) then begin
        logs.Debuglogs('ttoolsversions.APP_NETATALK_VERSION():: not installed');
        exit;
     end;
  filetemp:=logs.FILE_TEMP();
  result:=SYS.GET_CACHE_VERSION('APP_NETATALK');
  if length(result)>2 then exit;
  fpsystem(binpath+' -V >'+filetemp +' 2>&1');
  RegExpr:=TRegExpr.Create;
  RegExpr.Expression:='afpd\s+([0-9\.]+)';
  l:=TstringList.Create;
  L.LoadFromFile(filetemp);
  for i:=0 to l.Count-1 do begin
   if RegExpr.Exec(l.Strings[i]) then begin
      ...
      logs.Debuglogs('ttoolsversions.APP_NETATALK_VERSION():: Not found "'+l.Strings[i]+'"');
   end;
  end;
  RegExpr.free;
  l.free;
  SYS.SET_CACHE_VERSION('APP_NETATALK',result);
...
//#####################################################################################
function ttoolsversions.MIMEDEFANG_VERSION():string;
var
   l:Tstringlist;
   tmpstr,binpath:string;
   RegExpr:TRegExpr;
...
begin
     ...
     D:=false;
     result:=SYS.GET_CACHE_VERSION('APP_MIMEDEFANG');
     if length(result)>2 then exit;
     binpath:=SYS.LOCATE_GENERIC_BIN('mimedefang');
     if not Fileexists(binpath) then exit;
...
for i:=0 to l.Count-1 do begin
    if RegExpr.Exec(l.Strings[i]) then begin
     ...
     SYS.SET_CACHE_VERSION('APP_MIMEDEFANG',result);
    l.free;
    RegExpr.free;
end;
//#####################################################################################
...
//##############################################################################
function ttoolsversions.APP_SPREED():string;
var
   l:Tstringlist;
   tmpstr,binpath:string;
   RegExpr:TRegExpr;
   ...
   fileToCheck:string;
begin
fileToCheck:='/usr/share/zarafa-webaccess/plugins/spreed/manifest.xml';
if not FileExists(fileToCheck) then exit;
result:=SYS.GET_CACHE_VERSION('APP_SPREED');
if length(result)>2 then exit;
l:=Tstringlist.Create;
l.LoadFromFile(fileToCheck);
RegExpr:=TRegExpr.Create;
if D then writeln('Lines:',l.Count);
RegExpr.Expression:='<version>([0-9\.]+)</version>';
for i:=0 to l.Count-1 do begin
    if RegExpr.Exec(l.Strings[i]) then begin
       result:=RegExpr.Match[1];
       break;
...
SYS.SET_CACHE_VERSION('APP_SPREED',result);
l.free;
RegExpr.free;
SYS.SET_CACHE_VERSION('APP_SPREED',result);
