...
var
  ...
  ldap_string:string;
  mds:string;
  KerbAuthDisableNsswitch,SquidPerformance,nsswitchEnableLdap,nsswitchEnableWinbind:integer;
...
if not TryStrToInt(SYS.GET_INFO('SquidPerformance'),SquidPerformance) then SquidPerformance:=0;
if not TryStrToInt(SYS.GET_INFO('nsswitchEnableLdap'),nsswitchEnableLdap) then nsswitchEnableLdap:=0;
if not TryStrToInt(SYS.GET_INFO('nsswitchEnableWinbind'),nsswitchEnableWinbind) then nsswitchEnableWinbind:=0;
...
if FileExists(SMBD_PATH()) then begin
   ...
   logs.Debuglogs('Starting......: SquidPerformance: '+IntToStr(SquidPerformance));
...
ldap_string:=' ldap';
if(SquidPerformance>2) then   ldap_string:=' files';
if nsswitchEnableLdap=0 then  ldap_string:=' files';
if nsswitchEnableWinbind=0 then  winbind:='';
...
if FileExists('/lib/x86_64-linux-gnu/security/pam_ldap.so') then exit ('/lib/x86_64-linux-gnu/security/pam_ldap.so');
end;
//##############################################################################
...
begin
  ...
  if FileExists('/usr/lib/x86_64-linux-gnu/libnss_ldap.so') then exit('/usr/lib/x86_64-linux-gnu/libnss_ldap.so');
end;
//##############################################################################
