...
if not FileExists(authlog) then if FIleExists('/var/log/auth.log') then authlog:='/var/log/auth.log';
if not FileExists(authlog) then if FIleExists('/var/log/syslog') then authlog:='/var/log/syslog';
if not FileExists(authlog) then if FIleExists('/var/log/messages') then authlog:='/var/log/messages';
...
writeln('Stopping OpenSSH watchdog....: Tail "',authlog,'" processe(s) number '+IntToStr(CountTail.Count));
     count:=0;
     for i:=0 to CountTail.Count-1 do begin;
          ...
          cmdline:=logs.ReadFromFile('/proc/'+pid+'/cmdline');
          writeln('Stopping OpenSSH watchdog....: Stop tail pid '+pid,' "',trim(cmdline),'"');
