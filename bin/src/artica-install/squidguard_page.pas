...
private
     ...
     SquidEnableStreamCache:integer;
...
begin
       ...
       if Not TryStrToInt(SYS.GET_INFO('SquidEnableStreamCache'),SquidEnableStreamCache) then SquidEnableStreamCache:=0;
       ...
       if SquidEnableStreamCache=1 then EnableSquidGuardHTTPService:=1;
       if EnableWebProxyStatsAppliance=1 then EnableSquidGuardHTTPService:=1;
...
SquidGuardStorageDir:=SYS.GET_INFO('SquidGuardStorageDir');
if length(SquidGuardStorageDir)=0 then SquidGuardStorageDir:='/home/artica/cache';
forceDirectories(SquidGuardStorageDir);
fpsystem('/bin/chown '+LighttpdUserAndGroup+' "'+SquidGuardStorageDir+'"');
fpsystem('/bin/chmod 755 "'+SquidGuardStorageDir+'"');
