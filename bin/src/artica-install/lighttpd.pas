...
begin
   ...
   logs.Debuglogs('Starting......: lighttpd: Checking sub-directories');
   CHECK_SUBFOLDER();
...
logs.Debuglogs('###################### LIGHTTPD #####################');
   ...
   if RegExpr.Exec(group) then group:=RegExpr.Match[1];
   if not SYS.isoverloaded() then begin
      logs.Debuglogs('Starting......: lighttpd: Cleaning PHP5 sessions...');
      CLEAN_PHP5_SESSIONS();
   end;
   SYS.THREAD_COMMAND_SET('/usr/share/artica-postfix/bin/artica-install --lighttpd-perms');
...
phpmyadminAllowNoPassword:integer;
database_password:string;
begin
 if not FileExists('/usr/share/phpmyadmin/index.php') then begin
    logs.DebugLogs('Starting......: lighttpd PhpMyAdmin: /usr/share/phpmyadmin/index.php no such file');
    exit;
 end;
...
if not TryStrToInt(SYS.GET_INFO('phpmyadminAllowNoPassword'),phpmyadminAllowNoPassword) then phpmyadminAllowNoPassword:=0;
database_password:=SYS.MYSQL_INFOS('database_password');
if database_password='!nil' then database_password:='';

logs.DebugLogs('Starting......: lighttpd PhpMyAdmin: AllowNoPassword='+IntTOstr(phpmyadminAllowNoPassword));
...
NoPHPMcrypt,EnableMemcached:integer;
begin
...
LighttpdArticaListenIP:=SYS.GET_INFO('LighttpdArticaListenIP');
...
l:=TstringList.Create;
l.Add('#artica-postfix saved by artica lighttpd.conf (Artica Install binary) v2.0');
l.Add('');
...
fpsystem(SYS.LOCATE_PHP5_BIN()+' /usr/share/artica-postfix/exec.lighttpd.nets.php --phpmyadmin');
if FileExists('/etc/artica-postfix/lighttpd_nets') then l.Add(logs.ReadFromFile('/etc/artica-postfix/lighttpd_nets'));

if FileExists('/proc/user_beancounters') then begin
      logs.Debuglogs('Starting......: lighttpd: VPS mode enabled, swith to socket mode for PHP');
...
if LighttpdUseUnixSocket=1 then begin
   ...
   l.Add('         "socket" => "/var/run/lighttpd/php.socket" + var.PID,');
end else begin
...
if FileExists('/etc/artica-postfix/lighttpd.phpmyadmin') then begin
   l.Add('');
   l.Add(logs.ReadFromFile('/etc/artica-postfix/lighttpd.phpmyadmin'));
   l.add('');
end;
...
l.Add('alias.url += ("/squid-rrd"=> "/opt/artica/share/www/squid/rrd")');
l.Add('alias.url += ("/artica-agent"=> "/usr/share/artica-postfix/ressources/artica-agent")');
