...
begin
  ...
  fpsystem(sys.LOCATE_PHP5_BIN()+' /usr/share/artica-postfix/exec.web-community-filter.php --uuid');
  result:=trim(SYS.GET_INFO('SYSTEMID'));
  if length(result)>3 then exit(result);
...
var
   ...
   RebootAfterArticaUpgrade:integer;
begin
...
if not TryStrToInt(SYS.GET_INFO('RebootAfterArticaUpgrade'),RebootAfterArticaUpgrade) then RebootAfterArticaUpgrade:=0;
Lastest:=CONF.ReadString('NEXT','artica-nightly','');
    logs.Debuglogs('NightlyBuild():: latest nightly build available version:'+Lastest);
    logs.Syslogs('Artica nightly build: latest nightly build available version:'+Lastest);
if length(Lastest)=0 then begin
   logs.Debuglogs('NightlyBuild():: No nightly build available.(artica-nightly key is null)');
   logs.Syslogs('Artica nightly build: No nightly build available.(artica-nightly key is null)');
end;
LastestBuild:=StripNumber(Lastest);
if LastestBuild=MyCurrentVersion then begin
    logs.Syslogs('Artica nightly build: up-to-date');
...
if LastestBuild<MyCurrentVersion then begin
  ...
  logs.Syslogs('Artica nightly build: Downloading new version ' + Lastest);
  ...
  logs.Syslogs('Artica nightly build: Downloading new version ' + Lastest+' done... starting installation');
  ...
  logs.Syslogs('Artica nightly build: decompressing ' + ArticaFileTemp);

  if not FileExists(ArticaFileTemp) then begin
     logs.Debuglogs('NightlyBuild():: Corrupted package '+ArticaFileTemp);
     logs.Syslogs('Artica nightly build: Corrupted  package:' + ArticaFileTemp);
     exit;
  end;
  ...
  if RebootAfterArticaUpgrade=1 then begin
    logs.NOTIFICATION('[ARTICA]:('+sys.HOSTNAME_g()+') success update new artica nightly build version ' + GLOBAL_INI.ARTICA_VERSION()+' system will be rebooted','your server is now up-to-date with the new version ' + GLOBAL_INI.ARTICA_VERSION(),'update');
...
begin
  ...
  logs.Debuglogs('Install_new_version() -> RebootAfterArticaUpgrade='+IntTostr(RebootAfterArticaUpgrade));
  current:=GetCurrentVersion();
...
fpsystem('/usr/share/artica-postfix/bin/process1 -perm');
 ...
 if RebootAfterArticaUpgrade=1 then begin
    logs.Debuglogs('Install_new_version() -> REBOOT !');
    logs.NOTIFICATION('[ARTICA]:('+sys.HOSTNAME_g()+') success update new official artica version ' + GLOBAL_INI.ARTICA_VERSION()+' system will be rebooted','your server is now up-to-date with the new version ' + GLOBAL_INI.ARTICA_VERSION(),'update');
    fpsystem(SYS.LOCATE_GENERIC_BIN('reboot'));
  end;
...
begin
 ...
 logs.Debuglogs('VerifAndReinstall():: Downloading new version ' + version);
  uri:='http://www.artica.fr/nightbuilds/artica-'+version+'.tgz';
