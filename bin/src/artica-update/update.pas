...
var  forcestr,debgustr:string;

begin
         D:=GLOBAL_INI.COMMANDLINE_PARAMETERS('--verbose');
  if FORCED then forcestr:=' --force';
  if D then debgustr:=' --verbose';
  fpsystem(SYS.LOCATE_PHP5_BIN()+' /usr/share/artica-postfix/exec.nightly.php'+forcestr+debgustr);
...
fpsystem('/etc/init.d/auth-tail restart');
fpsystem('/usr/share/artica-postfix/bin/process1 -perm');
if FileExists('/etc/init.d/nginx') then fpsystem('/etc/init.d/nginx restart');
  ...
  logs.Debuglogs('using "'+ uri+'"');
   ...
   if FileExists(localFile) then begin
      ...
      if minutestimed>60 then begin
         ...
         GLOBAL_INI.WGET_DOWNLOAD_FILE('http://www.artica.fr/download/old-squid/index.conf','/usr/share/artica-postfix/ressources/old-squid.ini');
         GLOBAL_INI.WGET_DOWNLOAD_FILE('http://www.artica.fr/download/old-samba/index.conf','/usr/share/artica-postfix/ressources/old-samba.ini');
      end;
   end else begin
        GLOBAL_INI.WGET_DOWNLOAD_FILE(uri+'?datas='+uriplus,localFile);
        GLOBAL_INI.WGET_DOWNLOAD_FILE('http://www.artica.fr/download/old-squid/index.conf','/usr/share/artica-postfix/ressources/old-squid.ini');
