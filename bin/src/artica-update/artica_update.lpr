...
begin
  ...
  if ParamStr(1)='-V' then begin
    writeln('Revision 24/08/2012');
     halt(0);
  end;

  if ParamStr(1)='--reinstall' then begin
    if not SYS.BuildPids() then begin
         zlogs.Debuglogs('Already instance executed');
         halt(0);
    end;
    if D then writeln(' -> VerifAndReinstall();...');
    XSETS.VerifAndReinstall();
    halt(0);
...
if not TryStrToInt(SYS.GET_INFO('EnableScheduleUpdates'),EnableScheduleUpdates) then EnableScheduleUpdates:=0;
 ...
 if not FileExists('/etc/init.d/artica-boot') then fpsystem('/usr/share/artica-postfix/bin/artica-install --artica-boot >/dev/null 2>&1');
 fpsystem('/etc/init.d/artica-boot start');
