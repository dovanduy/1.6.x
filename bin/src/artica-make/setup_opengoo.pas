...
function setupopengoo.PYAUTHENNTLM():boolean;
var
   CODE_NAME:string;
   cmd:string;
   pythonBin:string;
...
result:=false;
CODE_NAME:='APP_PYAUTHENNTLM';
if length(source_folder)=0 then source_folder:=libs.COMPILE_GENERIC_APPS('PyAuthenNTLM2');
  if not DirectoryExists(source_folder) then begin
     writeln('Install PyAuthenNTLM failed...');
     exit;
  end;
  install.INSTALL_STATUS(CODE_NAME,40);
  install.INSTALL_PROGRESS(CODE_NAME,'{compiling}');
  writeln('Install PyAuthenNTLM extracted on "'+source_folder+'"');

  pythonBin:=SYS.LOCATE_GENERIC_BIN('python');
  if not FileExists(pythonBin) then begin
     install.INSTALL_STATUS(CODE_NAME,110);
     writeln('python no such binary...');
     install.INSTALL_PROGRESS(CODE_NAME,'{failed}');
     result:=true;
     exit;
  end;
  ...
  if not FileExists(source_folder+'/setup.py') then begin
     install.INSTALL_STATUS(CODE_NAME,110);
     writeln(source_folder+'/setup.py no such file...');
     install.INSTALL_PROGRESS(CODE_NAME,'{failed}');
     result:=true;
     exit;
  end;
...
install.INSTALL_STATUS(CODE_NAME,70);
  install.INSTALL_PROGRESS(CODE_NAME,'{compiling}');
  cmd:=pythonBin+' setup.py install -f';
  fpsystem(cmd);
  ppackage:=SYS.LOCATE_PYTHON_PACKAGE('pyntlm.py');
  SetCurrentDir('/root');
  writeln(ppackage);

  if FileExists(ppackage) then begin
     install.INSTALL_STATUS(CODE_NAME,100);
     install.INSTALL_PROGRESS(CODE_NAME,'{installed}');
     result:=true;
  end else begin
