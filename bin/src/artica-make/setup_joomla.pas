...
procedure tsetup_joomla.cakephp_install();
 var
   CODE_NAME:string;
   cmd:string;
   RegExpr:TRegExpr;
   DirectoryDest:string;
begin
    CODE_NAME:='APP_CAKEPHP';
    DirectoryDest:='/var/www/c2';
    SetCurrentDir('/root');
    ForceDirectories(DirectoryDest);
  ...
  if not FileExists(SYS.LOCATE_APACHE_BIN_PATH()) then begin
     ...
     exit;
      ...
      install.INSTALL_PROGRESS(CODE_NAME,'{downloading}');
     if length(source_folder)=0 then source_folder:=libs.COMPILE_GENERIC_APPS('cakephp');
     if not DirectoryExists(source_folder) then begin
        writeln('Install '+CODE_NAME+' failed...');
        install.INSTALL_STATUS(CODE_NAME,110);
        exit;
     ...
     writeln('installing yficake....');
     libs:=tlibs.Create;
     source_folder:=libs.COMPILE_GENERIC_APPS('yficake');
     ...
     writeln('installing yfi....');
     DirectoryDest:='/usr/share/yfi';
     libs:=tlibs.Create;
     source_folder:=libs.COMPILE_GENERIC_APPS('yfi');
     ...
     writeln('installing coova_json....');
     DirectoryDest:='/usr/share/coova_json';
     libs:=tlibs.Create;
     source_folder:=libs.COMPILE_GENERIC_APPS('coova_json');
     forceDirectories(DirectoryDest);
     cmd:='/bin/cp -rf ' + source_folder+'/* '+DirectoryDest+'/';
     fpsystem(cmd);
     install.INSTALL_PROGRESS(CODE_NAME,'{checking}');
