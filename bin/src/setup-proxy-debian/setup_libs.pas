...
articaversion:='artica-latest.tgz';


if FileExists('/tmp/'+articaversion) then fpsystem('/bin/rm -f /tmp/'+articaversion);
if not WGET_DOWNLOAD_FILE('http://www.articatech.net/v10-officials/'+articaversion,'/tmp/'+articaversion) then begin
   writeln('Failed to get artica package...');
   exit;
end;

writeln('extracting  '+articaversion + ' artica version');
fpsystem('tar -xf /tmp/'+articaversion+' -C /usr/share');
...
ForceDirectories('/etc/artica-postfix/settings/Daemons');
writeln('Preparing  '+articaversion + ' for Proxy Appliance...');
fpsystem('echo "OK" >/etc/artica-postfix/SQUID_APPLIANCE');
fpsystem('echo "OK" >/etc/artica-postfix/FROM_ISO');
fpsystem('echo "OK" >/etc/artica-postfix/artica-iso-setup-launched');
fpsystem('echo "1" >/etc/artica-postfix/settings/Daemons/ProxyUseArticaDB');
fpsystem('echo "1" >/etc/artica-postfix/settings/Daemons/StatsPerfsSquidAnswered');
fpsystem('echo "1" >/etc/artica-postfix/settings/Daemons/CacheManagement2');
fpsystem('echo "0" >/etc/artica-postfix/settings/Daemons/EnablePHPFPM');
fpsystem('echo "0" >/etc/artica-postfix/settings/Daemons/EnableArticaFrontEndToNGninx');
fpsystem('echo "1" >/etc/artica-postfix/settings/Daemons/EnableArticaInNGINX');
fpsystem('echo "1" >/etc/artica-postfix/settings/Daemons/EnableArticaFrontEndToApache');
fpsystem('echo "0" >/etc/artica-postfix/settings/Daemons/EnableNginx');
...
fpsystem('php /usr/share/artica-postfix/exec.squid.php --build --force');
...
begin
     base:=trim(base);
      if length(base)>0 then begin
             writeln('Welcome on the Artica installation process 1.5 For Proxy appliance');
             writeln('');
...
if not FileExists('/usr/sbin/squid') then begin
   writeln('Downloading Squid-cache software 3.5.3...');
   WGET_DOWNLOAD_FILE('http://www.articatech.net/download/Debian7-squid/squid32-x64-debian7-3.5.3-20150415-r13798.tar.gz','/tmp/squid32.tar.gz');
   if not FileExists('/tmp/squid32.tar.gz') then begin
      Writeln('Failed !!!, aborting...');
      halt(0);
   end;
     writeln('Extracting squid32.tar.gz...');
     fpsystem('tar xf /tmp/squid32.tar.gz -C /');
     if not FileExists('/usr/sbin/squid') then begin
      Writeln('Failed !!!, aborting...');
      halt(0);
     end;
     fpsystem('rm -f /tmp/squid32.tar.gz');
end;



if not FileExists('/usr/sbin/smbd') then begin
     writeln('Downloading Samba 3.x software sambac-x64-debian7-3.6.22.tar.gz  (49MB)...');
     WGET_DOWNLOAD_FILE('http://www.articatech.net/download/Debian7-squid/sambac-x64-debian7-3.6.22.tar.gz','/tmp/sambac-x64-debian7-3.6.22.tar.gz');
     writeln('Extracting sambac-x64-debian7-3.6.22.tar.gz...');
     fpsystem('tar xf /tmp/sambac-x64-debian7-3.6.22.tar.gz -C /');
     fpsystem('rm -f /tmp/sambac-x64-debian7-3.6.22.tar.gz');
end;
