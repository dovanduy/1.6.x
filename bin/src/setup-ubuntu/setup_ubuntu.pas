...
begin
     ...
     if ParamStr(1)='--version' then begin
        writeln('Artica binary installer version 1.4 (August 2012) on '+distri.DISTRINAME,' version ',distri.DISTRI_MAJOR,'.',distri.DISTRI_MINOR);
        halt(0);
     end;
     ...
     if ParamStr(1)='--rpmforge' then begin
          if distri.DISTRINAME_CODE<>'CENTOS' then begin
             writeln('No supported');
             halt(0);
          end;
     ...
     if ParamStr(1)='--help' then begin
        ...
        if distri.DISTRINAME_CODE='CENTOS' then begin
        writeln('--rpmforge          : Install RPMForge');
        end;
        halt(0);
     end;
     ...
     if ParamStr(1)='--check-base-system' then begin
         ...
         if distri.DISTRINAME_CODE='DEBIAN' then  begin
            writeln('check-base-system:: Verify sources.list...');
            fpsystem('/usr/sbin/php5 /usr/share/artica-postfix/exec.apt-get.php --sources-list >/dev/null 2>&1');
            install:=tubuntu.Create;
            writeln('check-base-system:: Scanning softwares, please wait...');
            install.DEBUG:=true;
            install.InstallPackageListssilent(install.CheckBaseSystem());
            writeln('check-base-system:: Scanning softwares done, terminated');
            halt(0);
         end;
         ...
         writeln('check-base-system:: Fatal Operating system Not supported ' + distri.DISTRINAME+ '/' + distri.DISTRINAME_CODE);
         halt(0);
     end;
     ...
     if ParamStr(1)='--check-ejabberd' then begin
         writeln('check-postfix Starting checking ejabberd system on ' +distri.DISTRINAME + ' ' + distri.DISTRINAME_VERSION+' CODE=' +distri.DISTRINAME_CODE );
         ...
         if distri.DISTRINAME_CODE='FEDORA' then begin
           fedora:=tfedora.Create();
           fedora.InstallPackageListsSilent(fedora.ejabberd());
           halt(0);
         end;

         if distri.DISTRINAME_CODE='CENTOS' then begin
            centos:=tcentos.Create;
            centos.InstallPackageListsSilent(centos.ejabberd());
            halt(0);
         end;

         writeln('ejabberd Not supported on ' + distri.DISTRINAME+ '/' + distri.DISTRINAME_CODE);
         halt(0);
     end;
     ...
     if ParamStr(1)='--check-squid' then begin
      ...
      if ParamStr(1)='--check-krb5' then begin
         if distri.DISTRINAME_CODE='DEBIAN' then  begin
            install:=tubuntu.Create;
            install.InstallPackageListssilent(install.krb5());
         end;
 ...
 if ParamStr(1)='--install-proxy' then begin
         writeln('Install proxy Starting checking base system on ' +distri.DISTRINAME + ' ' + distri.DISTRINAME_VERSION+' CODE=' +distri.DISTRINAME_CODE );
         if distri.DISTRINAME_CODE='UBUNTU' then begin
            install:=tubuntu.Create;
            install.CheckProxyAppliance();
            halt(0);
         end;
         writeln('check-squid Not supported ' + distri.DISTRINAME+ '/' + distri.DISTRINAME_CODE);
         halt(0);
