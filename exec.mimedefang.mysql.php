...
function start(){
	...
	if ($handle = opendir("$storage_path")) {	
		while (false !== ($file = readdir($handle))) {
			if ($file == "." && $file == "..") {if($GLOBALS["VERBOSE"]){echo "skipped: `$file`\n";}continue;}
...
function CleanDatabase(){
	...
	$q->QUERY_SQL($sql);
	if(!$q->ok){system_admin_events("Fatal: $q->mysql_error.",__FUNCTION__,__FILE__,__LINE__,"postfix");return;}
	$filesEND=$q->COUNT_ROWS("storage");
	$diff=$filesStart-$filesEND;
	if($diff>0){system_admin_events("Success cleaned $diff files from attachments storage.",__FUNCTION__,__FILE__,__LINE__,"postfix");}
	@unlink($pidtime);
