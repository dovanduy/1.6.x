...
if(preg_match("#--verbose#",implode(" ",$argv))){$GLOBALS["VERBOSE"]=true;ini_set('display_errors', 1);ini_set('error_reporting', E_ALL);ini_set('error_prepend_string',null);ini_set('error_append_string',null);}
	...
	if(system_is_overloaded(basename(__FILE__))){writelogs("Overloaded system, aborting task","MAIN",__FILE__,__LINE__);die();}
	
	
if($argv[1]=="--users"){quota_users();die();}
if($argv[1]=="--user"){quota_users();die();}
if($argv[1]=="--quota-check"){quotaCheck();die();}
if($argv[1]=="--quota-sql"){quotas_mysql();die();}
