...
if(isset($_GET["verbose"])){$GLOBALS["VERBOSE"]=true;$GLOBALS["DEBUG_MEM"]=true;ini_set('display_errors', 1);ini_set('error_reporting', E_ALL);ini_set('error_prepend_string',null);ini_set('error_append_string',null);}
	...
	$user=new usersMenus();
	if(!$user->AsWebStatisticsAdministrator){echo FATAL_ERROR_SHOW_128("{ERROR_NO_PRIVS}");die();}
...
function group_popup() {
	...
	if($MAC<>null){
		if($IPclass->IsvalidMAC($MAC)){
			...
			while ($ligne2 = mysql_fetch_assoc($results)) {if(trim($ligne2["group"])==null){continue;}$GROUPS[$ligne2["group"]]=$ligne2["group"];}
...
function items_mac(){
	...
	while ($ligne = mysql_fetch_assoc($results)) {
			...
			"<a href=\"javascript:blur()\"
					OnClick=\"javascript:Loadjs('$MyPage?group-js=yes&table=webfilters_nodes&MAC=$MAC_encode');\"
...
function items_ip(){
	...
	while ($ligne = mysql_fetch_assoc($results)) {
		...
		<a href=\"javascript:blur();\"
		style=\"text-decoration:underline\"
		OnClick=\"javascript:Loadjs('squid.nodes.php?node-infos-js=yes&ipaddr=$ipaddrenc');\"
		>$proxy_alias</a>";
		...
		$data['rows'][] = array(
				...
				'cell' => array("<span style='font-size:26px'>$proxy_alias</span>",
						...
						"<a href=\"javascript:blur()\"
						OnClick=\"javascript:Loadjs('$MyPage?group-js=yes&table=webfilters_ipaddr&IP=$ipaddr_enc');\"
						style='text-decoration:underline;font-size:26px'>$group</a>",
						"<center style='font-size:26px'>$delete</center>",
