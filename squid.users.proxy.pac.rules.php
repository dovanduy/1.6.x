...
function js(){
	...
	<script>
		...
		function squid_proxy_pac_rules_load(){
			LoadAjax('$t','$page?popup-script=yes');
...
function popup_tabs(){
	...
	echo "
	<div id=main_config_proxypac style='width:100%;height:700px;overflow:auto'>
		<ul>". implode("\n",$html)."</ul>
	</div>
		<script>
...
function popup(){
	...
	$tpl=new templates();
	$q=new squidusers();
	$datas=$q->ProxyPacDatas;
	$ProxyPacRemoveProxyListAtTheEnd=$q->ProxyPacRemoveProxyListAtTheEnd;
...
function popup_add_proxy_save(){
	$q=new squidusers();
	$q->ProxyPacDatas["PROXYS"][]="{$_GET["proxy_addr"]}:{$_GET["listen_port"]}";
	unset($datas["PROYPACFINAL"]);$newdata=base64_encode(serialize($datas));
	$q->UpdateProxyPacDatas();
...
function popup_add_final_proxy_save(){
	$q=new squidusers();
	$q->ProxyPacDatas["FINAL_PROXY"][]="{$_GET["final_proxy_addr"]}:{$_GET["listen_port"]}";	
	$q->UpdateProxyPacDatas();
...
function popup_del_proxy_list(){
	$q=new squidusers();
	unset($q->ProxyPacDatas["PROXYS"][$_GET["del_proxy_addr"]]);
	$q->UpdateProxyPacDatas();
...
function popup_del_final_proxy_list(){
	$q=new squidusers();
	unset($q->ProxyPacDatas["FINAL_PROXY"][$_GET["del_proxy_final_addr"]]);
	$q->UpdateProxyPacDatas();
...
function popup_localHostOrDomainIs_add(){
	$q=new squidusers();
	$q->ProxyPacDatas["localHostOrDomainIs"][]=$_GET["localHostOrDomainIs-add"];
	$q->UpdateProxyPacDatas();
}
function popup_localHostOrDomainIs_del(){
	$q=new squidusers();
	unset($q->ProxyPacDatas["localHostOrDomainIs"][$_GET["localHostOrDomainIs-del"]]);
	$q->UpdateProxyPacDatas();
...
function popup_isInNet_add(){
	$q=new squidusers();
	$q->ProxyPacDatas["isInNet"][]=array($_GET["isInNet-add"],$_GET["isInNet-mask"]);
	$q->UpdateProxyPacDatas();
		
}
function popup_isInNet_del(){
	$q=new squidusers();
	unset($q->ProxyPacDatas["isInNet"][$_GET["isInNet-del"]]);
	$q->UpdateProxyPacDatas();
...
function popup_localHostOrDomainIs_list(){
	...
	$sql="SELECT ProxyPacDatas FROM usersisp WHERE userid='{$_SESSION["uid"]}'";
	...
	while (list ($num, $uri) = each ($datas["localHostOrDomainIs"])){
		...
		if(preg_match("#dnsDomainIs:(.+?)$#",trim($uri),$re)){
			$uri=$re[1].$tpl->_ENGINE_parse_body(" <i>{IE8_compatibility}</i>");
		}
	$data['rows'][] = array(
		'id' => "Proxy$num",
		'cell' => array(
...
function popup_add_proxy_list(){
	...
	$data['rows'][] = array(
		'id' => "Proxy1",
		'cell' => array(
		"<a href=\"javascript:blur();\" OnClick=\"$JSGROUP\"  style='font-size:16px;font-weight:bold;color:black;text-decoration:underline'>$SquidISPProxyServerAddress</a></span>",
		$conditions,"&nbsp;"
		)
		);	
...
function popup_add_condition(){
	$q=new squidusers();
	$proxyname=$q->ProxyPacDatas["PROXYS"][$_GET["add_condition"]];
...
function popup_add_condition_add(){
	...
	if(trim($_GET["dnsDomainIs"])<>null){
		$q->ProxyPacDatas["CONDITIONS"][$_GET["add_condition_plus"]][]["dnsDomainIs"]=$_GET["dnsDomainIs"];
	}

	if(trim($_GET["isPlainhost"])<>null){
		$q->ProxyPacDatas["CONDITIONS"][$_GET["add_condition_plus"]][]["isPlainhost"]=$_GET["isPlainhost"];
	}		
	
	if(trim($_GET["other_proxy_addr"])<>null){
		$q->ProxyPacDatas["CONDITIONS"][$_GET["add_condition_plus"]][]["FailOverProxy"]="{$_GET["other_proxy_addr"]}:{$_GET["other_proxy_port"]}";
	}	
...
function popup_del_condition(){
	$q=new squidusers();	
	unset($q->ProxyPacDatas["CONDITIONS"][$_GET["DeleteCondition"]][$_GET["DeleteConditionKey"]]);
	$q->UpdateProxyPacDatas();
...
function popup_script(){
	...
	$html="<textarea style='height:450px;overflow:auto;width:100%;font-size:14px'>$ppac->ProxyPacFinal</textarea>";
	echo $html;
