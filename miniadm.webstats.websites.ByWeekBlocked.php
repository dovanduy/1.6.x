...
function main_page(){
	...
	$content=str_replace("{SCRIPT}", "<script>LoadAjax('globalContainer','$page?content=yes&year={$_GET["year"]}&month={$_GET["month"]}&week={$_GET["week"]}&tablename={$_GET["tablename"]}&xtime={$_GET["xtime"]}')</script>", $content);
	echo $content;	
}
function content(){
	$page=CurrentPageName();
...
function webstats_middle_table_items(){
	...
	$subtable="( SELECT SUM(hits) as hits,client,hostname,website,category,rulename FROM `$tablename_prod` 
