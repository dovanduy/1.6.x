...
if(!$usersmenus->AsArticaMetaAdmin){
	$tpl=new templates();
...
function AddNewSchedule_js(){
	...
	echo "YahooWin{$YahooWin}('850','$page?AddNewSchedule-popup=yes&ID=$ID$YahooWinet&ForceTaskType={$_GET["ForceTaskType"]}&ForceType={$_GET["ForceTaskType"]}&uuid={$_GET["uuid"]}','$title')";
...
function AddNewSchedule_popup(){
	...
	<tr>
		<td class=legend style='font-size:18px' nowrap>{task_type}:</td>
		<td>". Field_array_Hash($taskz, "TaskType-$t",$ligne["TaskType"],"ExplainTaskType()",null,0,"font-size:18px")."</td>
	</tr>
	<tr>
	...
	<tr>
	<tr>
		<td class=legend style='font-size:18px'>{description}:</td>
		<td>". Field_text("TimeDescription", $ligne["TimeDescription"],"font-size:18px")."</td>
	</tr>
	<tr>
		<td class=legend style='font-size:18px'>{schedule}:</td>
		<td><input type='hidden' id='TimeText-$t' value='{$ligne["TimeText"]}' style='font-size:18px'>
		". button("{browse}...","Loadjs('cron.php?field=TimeText-$t')",18)."</td>
	</tr>	
	<tr>
		<td colspan=2 align='right'><hr>". button($buttontext,"SaveTaskSystem$t()",22)."
...
function AddNewSchedule_save(){
	...
	if(!$q->TABLE_EXISTS("system_schedules")){$q->BuildTables();}
	$q->QUERY_SQL($sql); 
...
$('#$t').flexigrid({
	url: '$page?search=yes&minisize={$_GET["minisize"]}&ForceTaskType={$_GET["ForceTaskType"]}&uuid={$_GET["uuid"]}',
	dataType: 'json',
	colModel : [
		{display: '&nbsp;', name : 'ID', width : 32, sortable : true, align: 'center'},
		{display: '$task', name : 'TaskType', width : 319, sortable : false, align: 'left'},
		{display: '$description', name : 'TimeDescription', width : 723, sortable : false, align: 'left'},
...
buttons : [
	...
	function AddNewSchedule(category){
			Loadjs('$page?AddNewSchedule-js=yes&ID=0&ForceTaskType={$_GET["ForceTaskType"]}&uuid={$_GET["uuid"]}');
	}
...
function search(){
	...
	$data['rows'][] = array(
		...
		<div style='font-size:11px'><i>$explainTXT</i></div>",
