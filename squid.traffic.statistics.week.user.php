...
function visited(){
	...
	$html="<div style='font-size:16px;font-weight:bold'>{$_GET["field"]}:{$_GET["user"]} $titleW</div>	<table class='events-table-$t' style='display: none' id='events-table-$t' style='width:99%'></table>
<script>
...
function users_form(){
	...
	$tpl=new templates();		
	$html="<center>
			". Field_text("member-$t-search",null,"font-size:16px;width:220px",null,null,null,false,"WeekMembersSearchCheck$t(event)")."
		</center>
	<div id='member-$t-list' style='margin-top:10px'></div>
	<script>
		function WeekMembersSearchCheck$t(e){
			if(checkEnter(e)){WeekMembersSearch();}
		}
	
	
		function WeekMembersSearch(){
...
function users_search(){
	...
	$sql="SELECT $field from $table GROUP BY $field HAVING `$field` LIKE '$query' ORDER BY $field LIMIT 0,25";
	...
	while($ligne=@mysql_fetch_array($results,MYSQL_ASSOC)){
		$FIELD_SRC=$ligne[$field];
		$usertext=null;
		
		if($field=="MAC"){
			$usertext=$qA->UID_FROM_MAC($FIELD_SRC);
		}
		if($usertext==null){$usertext=$FIELD_SRC;}
...
function paragraphe2(){
	...
	$html="<div style='color:#CF1717;font-weight:bold;margin-top:5px;font-size:13.5px'>{top_visited_categories} :</div>
	$p0
