...
function getobjects($ID){
	$q=new mysql();
	$ipaddr=array();
	$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT ipaddr FROM arpspoof_objects WHERE ID=$ID AND ipaddr='*'","artica_backup"));
...
function init_debian(){
	...
	$f[]="	if [ \$ENABLED -eq 0 ]";
	$f[]="  	then";
	$f[]="   		$php5 ".dirname(__FILE__)."/exec.arpspoof.php --stop";
	$f[]="  		return 0";
	$f[]="  	fi";	
	
	$f[]="	if [ \$ENABLED -eq 1 ]";
	$f[]="  	then";
	$f[]="   		$php5 ".dirname(__FILE__)."/exec.arpspoof.php --start";
	$f[]="  	fi";
	$f[]="  return 0";
	$f[]="}";
	$f[]="";
	...
	$f[]="#";
	$f[]="do_reload() {";
	$f[]="   $php5 ".dirname(__FILE__)."/exec.arpspoof.php --restart";
	$f[]="    return 0";
	$f[]="}";
	$f[]="";
	$f[]="case \"\$1\" in";
