<?php
	...
	if(isset($_POST["www_port_enabled"])){SaveGeneralSettings();exit;}
	...
	if(isset($_GET["config-parameters"])){parameters();exit;}
...
function tabs(){
	...
	$array["config-parameters"]="{parameters}";
...
function SaveGeneralSettings(){
	...
	$arrayConf["GLOBAL_SETTINGS"]["www_port_enabled"]=$_POST["www_port_enabled"];
	$arrayConf["GLOBAL_SETTINGS"]["www_port"]=$_POST["www_port"];
	$arrayConf["GLOBAL_SETTINGS"]["www_password"]=$_POST["www_password"];
	$arrayConf["GLOBAL_SETTINGS"]["www_user"]=$_POST["www_user"];
	$arrayConf["GLOBAL_SETTINGS"]["buffer"]=$_POST["buffer"];
...
function parameters(){
	...
	$ligne=mysql_fetch_array($q->QUERY_SQL($sql,"artica_backup"));	
	$arrayConf=unserialize($ligne["parameters"]);
	$Parameters=$arrayConf["GLOBAL_SETTINGS"];
	if(!is_numeric($Parameters["buffer"])){$Parameters["buffer"]=2048;}
	$t=time();
	$html="<table style='width:99%' class=form>
	<tbody>
	</tr>
		<td class=legend style='font-size:14px'>{buffer}:</td>
		<td style='font-size:13px'>". Field_text("buffer-$t",$Parameters["buffer"],"font-size:14px;padding:3px;width:120px",null,null,null,false,"")."&nbsp;Bytes</td>
		<td width=1%>&nbsp;</td>
	</tr>		
	</tr>
		<td class=legend style='font-size:14px'>{web_interface_port}: {enable}</td>
		<td style='font-size:13px'>". Field_checkbox("www_port-enabled-$t",1,$Parameters["www_port_enabled"],"BckGenFormCheck()")."</td>
		<td width=1%>&nbsp;</td>
	</tr>	
	</tr>
		<td class=legend style='font-size:14px'>{web_interface_port}:</td>
		<td style='font-size:13px'>". Field_text("www_port-$t",$Parameters["www_port"],"font-size:14px;padding:3px;width:90px",null,null,null,false,"BckGenFormCheck(event)")."</td>
		<td width=1%>". help_icon("{xr_web_interface_port}")."</td>
	</tr>	
	</tr>
		<td class=legend style='font-size:14px'>{username}:</td>
		<td style='font-size:13px'>". Field_text("www_user-$t",$Parameters["www_user"],"font-size:14px;padding:3px;width:128px",null,null,null,false,"BckGenFormCheck(event)")."</td>
		<td width=1%>&nbsp;</td>
	</tr>	
	</tr>
		<td class=legend style='font-size:14px'>{password}:</td>
		<td style='font-size:13px'>". Field_password("www_password-$t",$Parameters["www_password"],"font-size:14px;padding:3px;width:128px",null,null,null,false,"BckGenFormCheck(event)")."</td>
		<td width=1%>&nbsp;</td>
	</tr>			
	<tr>
		<td colspan=3 align='right'><hr>". button("{apply}","SaveGenSettCrossx()",14)."</td>
	</tr>
...
<script>
	...
	var x_SaveGenSettCrossx= function (obj) {
		var results=obj.responseText;
		if(results.length>2){alert(results);return;}
		RefreshTab('main_config_postfixmultipeCrossRoads');
	}	

	function SaveGenSettCrossx(){
		var XHR = new XHRConnection();
		XHR.appendData('client-settings-save','yes');
		XHR.appendData('ipaddr','$ipaddr');
		if(document.getElementById('www_port-enabled-$t').checked){XHR.appendData('www_port_enabled',1);}else{XHR.appendData('www_port_enabled',0);}
		XHR.appendData('www_port',document.getElementById('www_port-$t').value);
		XHR.appendData('www_user',document.getElementById('www_user-$t').value);
		XHR.appendData('www_password',document.getElementById('www_password-$t').value);
		XHR.appendData('buffer',document.getElementById('buffer-$t').value);
		XHR.sendAndLoad('$page', 'POST',x_SaveGenSettCrossx);	
	...
	var X_fillStatusPort= function (obj) {
		var results=obj.responseText;
		if(results.length>2){
			document.getElementById('www_port-$t').value=results;
			return;
		}
	}	
	
	function fillStatusPort(){
		var port=document.getElementById('www_port-$t').value;
		if(port.length>0){return;}
		var XHR = new XHRConnection();
		XHR.appendData('find-newport','yes');
		XHR.sendAndLoad('crossroads.balancers.php', 'POST',X_fillStatusPort);	
	}	
	
	function BckGenFormCheck(){
		document.getElementById('www_port-$t').disabled=true;
		document.getElementById('www_user-$t').disabled=true;
		document.getElementById('www_password-$t').disabled=true;
		if(document.getElementById('www_port-enabled-$t').checked){
			document.getElementById('www_port-$t').disabled=false;
			document.getElementById('www_user-$t').disabled=false;
			document.getElementById('www_password-$t').disabled=false;
		}
