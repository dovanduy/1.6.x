<?php
	...
	if(isset($_POST["RemoveInstallFiles"])){RemoveInstallFiles();exit;}
	
	
	if(isset($_GET["ocs-search"])){echo ocs_search();exit;}
...
function ocs_search(){
	...
	echo "
	<div id='$time'></div>
	<script>LoadAjax('$time','ocs.search.php?js-in-front-popup=yes')</script>";
}
...
function js(){
	...
	function {$prefix}LoadMain(){
		LoadWinORG('890','$page?popup=yes$newinterface','$title');
		}	
		
	function {$prefix}LoadMainAjax(){
		$('#$divFill').load('$page?popup=yes$newinterface');
	}		
		
	function ocs_web_params(){
		YahooWin2('500','$page?params-web=yes$newinterface');
	}
...
function popup(){
	...
	$array["ocs-search"]='{browse_computers}';
	$array["popup-status"]='OCS:{status}';
	$array["params-web"]='{ocs_web_params}';
	$array["connected"]='OCS:{computers}';
...
if(isset($_GET["newinterface"])){$newinterface="style='font-size:14px'";}
	...
	while (list ($num, $ligne) = each ($array) ){
		...
		$html[]=$tpl->_ENGINE_parse_body("<li><a href=\"$page?$num=yes\"><span $newinterface>$ligne</span></a></li>\n");
	}
	
	
	echo "
	<div id=main_config_ocsi style='width:99.5%;height:100%;overflow:auto'>
		<ul>". implode("\n",$html)."</ul>
	</div>
		<script>
...
function popup_status(){
	...
	$html="
	<div style='background-color:#005447'>
	<img src='img/ocs-logo.png'>
	</div>
...
function params_web(){
	...
	if(trim($sock->getFrameWork("ocs.php?installExists=yes"))=="TRUE"){
		$removeInstall="<center style='margin:10px;font-size:14px'>".button("{remove_installation_files}","RemoveInstallFiles()",16)."</center>";
...
$removeInstall
	...
	var x_RemoveInstallFiles= function (obj) {
		var results=obj.responseText;
	...
	function RemoveInstallFiles(){
		var XHR = new XHRConnection();
		XHR.appendData('RemoveInstallFiles','yes');
		AnimateDiv('ocswebservername_div');
		XHR.sendAndLoad('$page', 'POST',x_RemoveInstallFiles);	
...
function popup_connected(){
	...
	<tbody>
	<tr>
		<td class=legend style='font-size:13px'>{computers}:</td>
		<td>". Field_text("searchocs",null,"font-size:16px;padding:3px",null,null,null,false,"OCSSearchPCCheck(event)")."</td>
	</tr>
	</tbody>
	</table>
	</center>
...
function RemoveInstallFiles(){
	...
	$sock=new sockets();
	$sock->getFrameWork("ocs.php?RemoveInstallFiles=yes");
}
