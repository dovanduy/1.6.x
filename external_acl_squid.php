...
while (!feof(STDIN)) {
 ...
 if($url<>null){
 	$array=parseURL($url);
 	WLOG($url." str:".strlen($url)." LOGIN:{$array["LOGIN"]},IPADDR:{$array["IPADDR"]} MAC:{$array["MAC"]} HOST:{$array["HOST"]}");
...
function GetMacToUid($mac){
	if($mac==null){return;}
	if(isset($GLOBALS["GetMacToUidMD5"])){$md5file=md5_file("/etc/squid3/MacToUid.ini");if($md5file<>$GLOBALS["GetMacToUidMD5"]);unset($GLOBALS["GetMacToUid"]);}
