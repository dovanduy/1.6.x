...
function start($ID){
	...
	$unix=new unix();
	$pidfile="/etc/artica-postfix/pids/ftpproxystart.$ID.pid";
...
function build(){
	if($GLOBALS["OUTPUT"]){echo "Reconfigure...: ".date("H:i:s")." [INIT]: Remove init.d scripts\n";}
	remove_init();
	$q=new mysql_squid_builder();
	$sql="SELECT * FROM proxy_ports WHERE FTP=1 AND enabled=1";
	
	if($GLOBALS["OUTPUT"]){echo "Reconfigure...: ".date("H:i:s")." [INIT]: Search parameters in MySQL Database\n";}
	$results = $q->QUERY_SQL($sql);
	while ($ligne = mysql_fetch_assoc($results)) {
		if($GLOBALS["OUTPUT"]){echo "Reconfigure...: ".date("H:i:s")." [INIT]: Building Port ID {$ligne["ID"]}\n";}
		build_conf($ligne);
		stop($ligne["ID"]);
		start($ligne["ID"]);
	}
...
function remove_init(){
	...
	while (list ($basename, $value) = each ($files) ){
		...
		shell_exec("$filepath --stop --force");
