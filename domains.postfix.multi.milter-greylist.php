<?php
	...
	if(isset($_GET["script-dump"])){script_dump();exit;}
	...
	if(isset($_GET["SrvMilterServ"])){SrvMilterServ();exit;}
...
function js(){
	...
	function LoadMilterGreyListAclMulti(index){
		YahooWin3(450,'$page?add_acl=true&hostname=$hostname&ou={$_GET["ou"]}&num='+index,'$acl::N.'+index);	
		}	
	...
	function MilterGreyListMultiAclID(ID){
		YahooWin3(450,'$page?add_acl=true&num='+ID+'&hostname={$_GET["hostname"]}&ou={$_GET["ou"]}','$acl N.$num');
	}
...
function main_tabs(){
	...
	$tpl=new templates();
	$array["index"]='{index}';
	$array["main_settings"]='{main_settings}';
	$array["popup-acl"]='{acls}';
	$array["popup-dumpdb"]='{delayed_items}';
	$array["script-dump"]='{file}';
	$hostname=$_GET["hostname"];
 ...
 * function PostfixMultiMilterGreymain_settings_greylist(){
			YahooWin3(\"500\",\"$page?popup-settings=yes&hostname=$hostname&ou={$_GET["ou"]}\",\"$title $main_settings\")
		}
		
		function PostfixMultiMilterGreymain_accesslist_greylist(){
			YahooWin3(\"600\",\"$page?popup-acl=yes&hostname=$hostname&ou={$_GET["ou"]}\",\"$acl $main_settings\")
		}	
		
		function PostfixMultiMilterGreymain_events_greylist(){
			YahooWin3(\"600\",\"$page?popup-logs=yes&hostname=$hostname&ou={$_GET["ou"]}\",\"$title\")
		}		
...
$style="style='font-size:14px'";
	while (list ($num, $ligne) = each ($array) ){
		...
		if($num=="main_settings"){
			$html[]=$tpl->_ENGINE_parse_body("<li><a href=\"$page?popup-settings=yes&hostname=$hostname&ou={$_GET["ou"]}\"><span $style>$ligne</span></a></li>\n");
			continue;
			
		}

		if($num=="main_settings"){
			$html[]=$tpl->_ENGINE_parse_body("<li><a href=\"$page?popup-logs=yes&hostname=$hostname&ou={$_GET["ou"]}\"><span $style>$ligne</span></a></li>\n");
			continue;
			
		}			
		
		if($num=="popup-acl"){
			$html[]=$tpl->_ENGINE_parse_body("<li><a href=\"milter.greylist.index.php?$num=yes&hostname={$_GET["hostname"]}&ou={$_GET["ou"]}\"><span $style>$ligne</span></a></li>\n");
			continue;
			
		}
		
		if($num=="popup-dumpdb"){
			$html[]=$tpl->_ENGINE_parse_body("<li><a href=\"milter.greylist.dumpdb.php?hostname={$_GET["hostname"]}&ou={$_GET["ou"]}\"><span $style>$ligne</span></a></li>\n");
			continue;			
		}
		
		if($num=="events"){
			$html[]=$tpl->_ENGINE_parse_body("<li><a href=\"milter.greylist.index.php?popup-logs=yes&hostname={$_GET["hostname"]}&ou={$_GET["ou"]}\"><span $style>$ligne</span></a></li>\n");
			continue;			
		}		
		...
		$html[]=$tpl->_ENGINE_parse_body("<li><a href=\"$page?$num=yes&hostname={$_GET["hostname"]}&ou={$_GET["ou"]}\"><span $style>$ligne</span></a></li>\n");
	}
...
function popup(){
	...
	$t=time();
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("{APP_MILTER_GREYLIST}");
	...
	<center>$imgG</center>
		...
		function SrvMilterServ$t(type){
			YahooWin5('650','$page?SrvMilterServ=yes&hostname={$_GET["hostname"]}&ou={$_GET["ou"]}&type='+type,'{APP_MILTERGREYLIST}&raquo;{service}&raquo;'+type);
		}
...
function dumpfile_js(){
	...
	function StartMilterGreylistDumpPage(){
		YahooWin3('650','$page?dumpfile-popup=yes&hostname={$_GET["hostname"]}&ou={$_GET["ou"]}','$title');
		}
...
function greylist_config_tab(){
	...
	while (list ($num, $ligne) = each ($array) ){
			if($num=="multiples-mx"){
			$html[]= $tpl->_ENGINE_parse_body("<li><a href=\"domains.postfix.multi.milter-greylist.mxs.php?hostname=$hostname&ou={$_GET["ou"]}\"><span style='font-size:16px'>$ligne</span></a></li>\n");
			continue;
		}
		$html[]= $tpl->_ENGINE_parse_body("<li><a href=\"$page?$num=yes&hostname=$hostname&ou={$_GET["ou"]}\"><span style='font-size:16px'>$ligne</span></a></li>\n");
...
function greylist_config($noecho=0){
	...
	<table style='width:99%' class=form>	
<tr>
	<td $style align='right' nowrap valign='top' class=legend style='font-size:14px'>{remove_tuple}:</strong></td>
	<td $style valign='top'>" . Field_checkbox("$t-lazyaw",1,$pure->main_array["lazyaw"])."</td>
	<td $style valign='top'>". help_icon("{remove_tuple_text}")."</td>
</tr>	
<tr>
	<td $style align='right' nowrap valign='top' class=legend style='font-size:14px'>{add_default_nets}:</strong></td>
	<td $style valign='top'>" . Field_checkbox("$t-MiltergreyListAddDefaultNets",1,$pure->MiltergreyListAddDefaultNets)."</td>
	<td $style valign='top'>". help_icon("{milter_greylist_add_default_net_explain}")."</td>
</tr>	
<tr>
	<td  align='right' nowrap valign='top' class=legend style='font-size:14px'>{timeout}:</strong></td>
	<td  valign='top' colspan=2>" . Field_text("$t-timeout",$pure->main_array["timeout"],'width:40px;font-size:14px',null,null,'{mgreylisttimeout_text}')."&nbsp;".
		Field_array_Hash($arraytime,"$t-timeout_TIME",$pure->main_array["timeout_TIME"],"style:font-size:14px")."</td>
</tr>
		

	<tr>
	<td  align='right' nowrap valign='top' class=legend style='font-size:14px'>{greylist}:</strong></td>
	<td  valign='top' colspan=2>
	
	" . Field_text("$t-greylist",$pure->main_array["greylist"],'width:40px;font-size:14px',null,null,'{greylist_text}')."&nbsp;".
		Field_array_Hash($arraytime,"$t-greylist_TIME",$pure->main_array["greylist_TIME"],"style:font-size:14px")."
	
	</td>
	</tr>
	
	<tr>
	<td  align='right' nowrap valign='top' class=legend style='font-size:14px'>{autowhite}:</strong></td>
	<td  valign='top' colspan=2>" . Field_text("$t-autowhite",$pure->main_array["autowhite"],'width:40px;font-size:14px',null,null,'{autowhite_text}')."&nbsp;".
		Field_array_Hash($arraytime,"$t-autowhite_TIME",$pure->main_array["autowhite_TIME"],"style:font-size:14px")."</td>
	</tr>
	<td  colspan=3 align='right' valign='top'>
	<hr>". button("{apply}","MilterGreyListConfigGeneSave$t()",16)."
	</td>
	</tr>
	...
	var x_MilterGreyListConfigGeneSave$t=function(obj){
	      var tempvalue=obj.responseText;
	      if(tempvalue.length>3){alert(tempvalue);}
	...
	function MilterGreyListConfigGeneSave$t(){
		 ...
		 XHR.appendData('timeout',document.getElementById('$t-timeout').value);
		 XHR.appendData('timeout_TIME',document.getElementById('$t-timeout_TIME').value);
		 XHR.appendData('greylist',document.getElementById('$t-greylist').value);
...
echo "<div id='$t'></div>
<script>
	LoadAjax('$t','milter.greylist.index.php?popup-logs=yes&hostname={$_GET["hostname"]}&ou={$_GET["ou"]}');
</script>
...
function script_dump(){
	$sock=new sockets();
	$datas=unserialize(base64_decode($sock->getFrameWork("postfix.php?mgreylist-config=yes&hostname={$_GET["hostname"]}")));
	while (list ($num, $ligne) = each ($datas) ){
		$ligne=trim($ligne);
		if($ligne==null){continue;}
		if($ligne[0]=="#"){continue;}
		$html=$html."<div><code style='font-size:14px'>$ligne</code></div>";
