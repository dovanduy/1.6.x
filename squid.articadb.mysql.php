...
function selectDB_js(){
	...
	echo "YahooWin5(600,'$page?selectDB-popup=yes&host={$_GET["host"]}&user={$_GET["user"]}&instance-id={$_GET["instance-id"]}&t={$_GET["t"]}','$title');";
}
...
function selectDB_save(){
	...
	if(!is_numeric($_POST["instance-id"])){$_POST["instance-id"]=0;}
	 ...
	 $sql_delete="DELETE FROM `db` WHERE `Host`='{$_POST["host"]}' AND `Db`='{$_POST["database"]}' AND `User`='{$_POST["user"]}'";
...
$('#mysql-users-$t').flexigrid({
	...
	colModel : [
		{display: '$member', name : 'User', width : 320, sortable : true, align: 'left'},
	...
	function AddMysqlUser$t(){
		YahooWin3('443','$page?members-add=yes&instance-id={$_GET["instance-id"]}&t=$t','$add_user');
	}
...
function members_add_popup(){
	...
	<tr>
		...
		<td>". Field_password("password",$password,"font-size:16px;padding:3px;width:160px")."</td>
	</tr>	
	<tr>
