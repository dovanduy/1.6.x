...
function LinkAddAclItem(){
	Loadjs('squid.acls.groups.php?AddGroup-js=-1&link-acl={$_GET["aclid"]}&table-acls-t=$t&TransParentMode=1');
}
...
function items_negation(){
	...
	$sql="UPDATE transparent_networks_groups SET enabled={$_POST["value"]} WHERE zmd5='$md5'";
...
function items_unlink(){
	...
	$sql="DELETE FROM transparent_networks_groups WHERE zmd5='$md5'";
	$q=new mysql_squid_builder();
	$q->QUERY_SQL($sql);
	if(!$q->ok){echo $q->mysql_error;return;}
...
function items_list(){
	...
	$search='%';
	$table="(SELECT transparent_networks_groups.gpid,
	transparent_networks_groups.zmd5 as mkey,transparent_networks_groups.enabled as zenabled,
	webfilters_sqgroups.* FROM transparent_networks_groups,webfilters_sqgroups 
	WHERE transparent_networks_groups.gpid=webfilters_sqgroups.ID AND transparent_networks_groups.ruleid=$ID
	) as t";
	
	$page=1;

	if($q->COUNT_ROWS("transparent_networks_groups")==0){json_error_show("No datas");}
	...
	while ($ligne = mysql_fetch_assoc($results)) {
		...
		$negation=Field_checkbox("negation-$mkey", 1,$ligne["zenabled"],"ChangeNegation('$mkey')");
