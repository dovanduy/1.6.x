...
function js(){
	...
	$title=$rp->report."&nbsp;&raquo;{$_GET["category"]}";
	echo "YahooWin('962','$page?popup=yes&ID=$ID&category=".urlencode($_GET["category"])."&t=$t','$title')";
...
function popup(){
	...
	$rp=new squid_report($ID);
	$titleG=$rp->report."&nbsp;&raquo;{$_GET["category"]}";
	$sql="SELECT SUM(hits) as thits, zDate,category FROM $tablename GROUP BY zDate,category HAVING category='$category' ORDER BY zDate";
	...
	if(mysql_num_rows($results)>0){
		...
		if(is_file($targetedfile)){
			echo "<center>
			<div style='font-size:18px;margin-bottom:10px'>$titleG&raquo;".$tpl->_ENGINE_parse_body("{hits}/{day}")."</div>
			<img src='$targetedfile'></center>";
		}
	
	}	
	$sql="SELECT SUM(size) as thits, zDate,category FROM $tablename GROUP BY zDate,category HAVING category='$category' ORDER BY zDate";
	...
	if(mysql_num_rows($results)>0){
		...
		if(is_file($targetedfile)){
			echo "<center>
			<div style='font-size:18px;margin-bottom:10px'>$titleG&raquo;".$tpl->_ENGINE_parse_body("{size}/{day} (MB)")."</div>
			<img src='$targetedfile'></center>";
		}
	...
	<script>
		LoadAjax('$t','$page?table=yes&ID=$ID&category=".urlencode($_GET["category"])."');
	</script>
...
//	$title=$tpl->_ENGINE_parse_body("$attachments_storage {items}:&nbsp;&laquo;$size&raquo;");
	...
	$titletable=$tpl->_ENGINE_parse_body("{category}:{$_GET["category"]}");
	$buttons="
	buttons : [
...
$('#flexRT$t').flexigrid({
	url: '$page?items=yes&t=$t&ID=$ID&category=".urlencode($_GET["category"])."',
	dataType: 'json',
	colModel : [
		{display: '$date', name : 'zDate', width :84, sortable : true, align: 'left'},
		{display: '$sitename', name : 'sitename', width :586, sortable : true, align: 'left'},		
		{display: '$size', name : 'size', width :85, sortable : true, align: 'left'},
		{display: '$hits', name : 'hits', width :85, sortable : true, align: 'left'},
...
function items(){
	...
	$data['rows'][] = array(
		...
		'cell' => array(
			...
			"<span style='font-size:14px;color:$color'>$urljsSIT{$ligne["sitename"]}</a>&nbsp;($urljsFAM$family</a>)</span>",
