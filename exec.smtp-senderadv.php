...
if($GLOBALS["WAITFORATIME_SEQUENCE"]>9){die();}
...
$GLOBALS["MultipleAdvLoad"]=trim(@file_get_contents("/etc/artica-postfix/settings/Daemons/MultipleAdvLoad"));
$GLOBALS["MultipleAdvUseMemory"]=trim(@file_get_contents("/etc/artica-postfix/settings/Daemons/MultipleAdvUseMemory"));
$GLOBALS["MultipleAdvMaxRunningProcs"]=trim(@file_get_contents("/etc/artica-postfix/settings/Daemons/MultipleAdvMaxRunningProcs"));
...
function QueueDirectoryConfig(){
	...
	$MultipleAdvMemorySize=trim(@file_get_contents("/etc/artica-postfix/settings/Daemons/MultipleAdvMemorySize"));
...
function Scan_Pools(){
	...
	if(!is_file($CacheFile)){
		...
		$countInstances=mysql_num_rows($results);
		...
		$t=time();
		while ($ligne = mysql_fetch_assoc($results)) {$f[]=$ligne;ScanSpool($ligne);}
