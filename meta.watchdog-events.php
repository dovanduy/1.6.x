...
function ShowID_js(){
	...
	$sql="SELECT subject FROM meta_admin_hosts WHERE ID=$id";
	$q=new mysql_meta();
...
$tpl=new templates();
$sql="SELECT content FROM meta_admin_hosts WHERE ID={$_GET["ShowID"]}";
$q=new mysql_meta();
$ligne=@mysql_fetch_array($q->QUERY_SQL($sql,"artica_events"));
...
function empty_table(){
	$q=new mysql_meta();
	$q->QUERY_SQL("TRUNCATE TABLE meta_admin_hosts","artica_events");
...
function BuildTable$t(){
	$('#events-table-$t').flexigrid({
		...
		{display: '$hostname', name : 'hostname', width :185, sortable : true, align: 'left'},
...
function events_table(){
	...
	$table="(SELECT meta_admin_hosts.ID,meta_admin_hosts.subject,meta_admin_hosts.severity,meta_admin_hosts.line,meta_admin_hosts.filename,meta_admin_hosts.function,meta_admin_hosts.zDate,metahosts.hostname FROM meta_admin_hosts,metahosts WHERE metahosts.uuid=meta_admin_hosts.uuid) as t";
	$page=1;
	...
	if(!$q->ok){json_error_show($q->mysql_error." $sql",2);}
	...
	if(mysql_num_rows($results)==0){json_error_show("no data<br>$sql",2);}
	...
	while ($ligne = mysql_fetch_assoc($results)) {
		...
		$data['rows'][] = array(
				...
				'cell' => array(
						...
						"$spanzdate{$ligne["zDate"]}</span>",$text,$ligne["hostname"],$ligne["filename"] )
		);
	}
