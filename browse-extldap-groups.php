...
function js(){
	...
	echo "YahooWinBrowse(966,'$page?popup=yes&MainFunction={$_GET["MainFunction"]}&field-user={$_GET["field-user"]}&field-type={$_GET["field-type"]}&function={$_GET["function"]}&t=$t&CallBack2={$_GET["CallBack2"]}','Browse::$title');";
}
...
function popup_search(){
	...
	while (list ($dn, $itemname) = each ($Array) ){
		...
		$data['rows'][] = array(
				...
				'cell' => array(
						"<center>$image</center>",
						"<span style='font-size:20px;'>$GroupxName</a></span>",
