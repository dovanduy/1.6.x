...
function start(){
	...
	while($ligne=@mysql_fetch_array($results,MYSQL_ASSOC)){
		...
		if(!$q->ok){ufdbguard_admin_events("Fatal, MySQL error $q->mysql_error on tables_day", __FUNCTION__, __FILE__, __LINE__, "backup");return;}
		...
		if(system_is_overloaded(basename(__FILE__))){			
			...
			if($GLOBALS["TABLECOUNT"]>0){
				...
				ufdbguard_admin_events("Success backuped {$GLOBALS["TABLECOUNT"]} tables {$GLOBALS["BACKUPED_SIZET"]} added in backuped directory", __FUNCTION__, __FILE__, __LINE__, "backup");return;
			}
		 return;
		}
	...
	if($GLOBALS["TABLECOUNT"]>0){
		$GLOBALS["BACKUPED_SIZET"]=FormatBytes($GLOBALS["BACKUPED_SIZE"]/1024);
		ufdbguard_admin_events("Success backuped {$GLOBALS["TABLECOUNT"]} tables {$GLOBALS["BACKUPED_SIZET"]} added in backuped directory took:$took", __FUNCTION__, __FILE__, __LINE__, "backup");return;
	}
...
function backupTable($tablename,$filename){
	...
	if(!$q->ok){ufdbguard_admin_events("Fatal, MySQL error $q->mysql_error on webstats_backup", __FUNCTION__, __FILE__, __LINE__, "backup");return false;}
...
function week_visited(){
	...
	$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT WEEK( NOW( ) ) AS tweek, YEAR( NOW( ) ) AS tyear","artica_events"));
