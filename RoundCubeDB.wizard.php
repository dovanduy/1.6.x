...
function MySQLSyslogType_save(){
	$sock=new sockets();
	if(isset($_POST["RoundCubeMySQLServiceType"])){
		writelogs("SET_INFO RoundCubeMySQLServiceType {$_POST["RoundCubeMySQLServiceType"]}",__FUNCTION__,__FILE__,__LINE__);
		$sock->SET_INFO("RoundCubeMySQLServiceType", $_POST["RoundCubeMySQLServiceType"]);
	}
	if(isset($_POST["ListenPort"])){
		...
		$sock->SaveConfigFile($TuningParametersEnc, "RoundCubeTuningParameters");
	}
	if(isset($_POST["MySQLSyslogWorkDir"])){$sock->SET_INFO("RoundCubeDedicateMySQLWorkDir", $_POST["MySQLSyslogWorkDir"]);}
...
function remote_save(){
	...
	$sock->SET_INFO("RoundCubeRemoteMySQLServer", $_POST["mysqlserver"]);
	$sock->SET_INFO("RoundCubeRemoteMySQLServerPort", $_POST["RemotePort"]);
	$sock->SET_INFO("RoundCubeRemoteMySQLServerAdmin", $_POST["username"]);
	$sock->SET_INFO("RoundCubeRemoteMySQLServerPassword", $_POST["password"]);
}
...
function Next1(){
	...
	<script>
		...
		function Next$tt(){
			var XHR = new XHRConnection();
			XHR.appendData('RoundCubeMySQLServiceType',document.getElementById('MySQLSyslogType-$t').value);
...
function apply(){
	...
	if($MySQLSyslogType==3){
		$sock->SET_INFO("RoundCubeDedicateMySQLServer", 1);
		$sock->getFrameWork("roundcube.php?mysqldb-restart=yes");
