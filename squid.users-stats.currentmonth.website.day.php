...
function js(){
	...
	$title=$tpl->_ENGINE_parse_body("$time_text::{$_GET["familysite"]}:: {$_GET["field"]}::{$_GET["value"]}");
	$page=CurrentPageName();
	$html="
	function Start$t(){
	YahooWin4('1019','$page?tabs=yes$suffix','$title')
}
...
function suffix(){
	...
	return "&field={$_GET["field"]}&value=".urlencode($_GET["value"])."&time={$_GET["time"]}&familysite=".urlencode($_GET["familysite"]);
}
...
function tabs(){
	...
	$suffix=suffix();
	$array["flow"]="{$_GET["familysite"]}: {flow_by_hour}";
...
function flow(){
	...
	$sql="SELECT hits as hits,size as size,{$_GET["field"]},`hour` 
	FROM {$date}_maccess 
	WHERE {$_GET["field"]}='{$_GET["value"]}' AND familysite='{$_GET["familysite"]}' AND zDate='$zDate' ORDER BY `hour` ";
	$time=time();
	...
	$f1[]="<div style='width:955px;font-size:28px'>{$_GET["familysite"]} / {$_GET["field"]} / {$_GET["value"]}</div>";
...
function websites_table(){
	...
	$zDate=$tpl->javascript_parse_text("{hour}");
	...
	$title=$tpl->javascript_parse_text("$time_text {$_GET["familysite"]}:: {$_GET["field"]}:: &laquo;{$_GET["value"]}&raquo;");
	$suffixMD=md5($suffix);
	$buttons="
	buttons : [
	...
	dataType: 'json',
	colModel : [
	{display: '$zDate', name : 'hour', width : 641, sortable : true, align: 'left'},
...
function websites_search(){
	...
	$table="(SELECT hits,size,familysite,`hour`,{$_GET["field"]}
	FROM {$date}_maccess WHERE {$_GET["field"]}='{$_GET["value"]}' AND familysite='{$_GET["familysite"]}'
	AND zDate='$zDate') as t";
	...
	$time_text=$tpl->_ENGINE_parse_body($q->time_to_date($time));
		...
		$FILTER="Loadjs('squid.users-stats.currentmonth.website.day.php?".
		"field={$_GET["field"]}&value=".urlencode($_GET["value"]).
		"familysite={$_GET["familysite"]}&time=$time')";
		...
		OnClick=\"javascript:$FILTER\" $styleHref>";
				...
				$cell=array();
				$cell[]="<span $style>{$ligne["hour"]}H</a></span>";
