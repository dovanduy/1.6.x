...
function main_network(){
	if(GET_CACHED(__FILE__,__FUNCTION__.$_GET["newinterface"],null)){return;}
	$users=new usersMenus();
	...
	$ntpd_server=Paragraphe("folder-64-ntp-grey.png","{APP_NTPD}","{APP_NTPD_TEXT}");
	if($users->NTPD_INSTALLED){
		$js="Loadjs('ntpd.index.php')";
		$img="folder-64-ntp.png";
		$ntpd_server=Paragraphe($img,"{APP_NTPD}","{APP_NTPD_TEXT}","javascript:$js");		
	}	

	if(!$users->POWER_DNS_INSTALLED){$pdns=Paragraphe('dns-64-grey.png','{APP_PDNS}','{feature_not_installed}');}
	...
	if(isset($_GET["newinterface"])){$network=null;$openvpn=null;$dhcp=null;}	
	...
	if(isset($_GET["newinterface"])){
		$html="<center style='width:100%'><div style='width:80%' class=form>$html</div></center>";
	}
	
	$html=$tpl->_ENGINE_parse_body($html);
	SET_CACHED(__FILE__,__FUNCTION__.$_GET["newinterface"],null,$html);
	echo $html;
}
...
$html="<div id='rdr'>
	...
	<td valign='top' width=100%>
		...
		<tr>
			...
			<td valign='top'>
				". button("{apply}","AddressBookSave()",16)."
			</td>
		</tr>
		</table>
		...
		<tr>
			...
			<td valign='top'>". button("{apply}","AddressBookSave()",16)."</td>
				<hr>
				<table style='width:100%'>
					<tr>
...
function AdressBookPopup_save(){
		...
		$sock->SET_INFO('EnablePerUserRemoteAddressBook',$_GET["EnablePerUserRemoteAddressBook"]);	
