<?php
	...
	if(isset($_POST["ICQ_GATEWAY"])){SaveMSNConf();exit;}
...
function js(){
	...
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("{APP_PYICQT}");
...
function popup(){
	...
	$html="
	<div id='$t'>
	<div class=text-info>{PYICQT_HOWTO}</div>
	<table class=form style='width:99%'>
	<tr>
		<td class=legend style='font-size:14px'>{hostname}:</td>
		<td>". Field_text("ICQ_GATEWAY",$PYMSNT["ICQ_GATEWAY"],"font-size:14px;width:220px")."</td>
	</tr>
	<tr>
	...
	var x_SaveMSNConfig=function (obj) {
		...
		function SaveMSNConfig(){
			...
			XHR.appendData('ICQ_GATEWAY',document.getElementById('ICQ_GATEWAY').value);
			AnimateDiv('$t');
