...
if(isset($_GET["VERBOSE"])){$GLOBALS["VERBOSE"]=true;ini_set('html_errors',0);ini_set('display_errors', 1);ini_set('error_reporting', E_ALL);ini_set('error_prepend_string','');ini_set('error_append_string','');}
	...
	if(isset($_POST["addwww"])){save_items_www();exit;}
...
function popup(){
	...
	$add_websites_meta=$tpl->_ENGINE_parse_body("{add_websites_meta}");
	...
	$RemoveEnabled="{name: '<strong style=font-size:18px>$removedisabled</strong>', bclass: 'Delz', onpress : RemoveDisabled$t},";
	$BUTON_ADD_WEBSITES="{name: '<strong style=font-size:18px>$add_websites</strong>', bclass: 'Add', onpress : AddWebSites$t},";
...
$give_the_website=$tpl->javascript_parse_text("{add_websites_meta}");
...
$('#$t').flexigrid({
	...
	dataType: 'json',
	colModel : [
			{display: '<span style=font-size:18px>$date</span>', name : 'zDate', width : 188, sortable : true, align: 'left'},	
			{display: '<span style=font-size:18px>$website</span>', name : 'pattern', width :431, sortable : true, align: 'left'},
			{display: '&nbsp;', name : 'none2', width : 55, sortable : false, align: 'left'},
...
$searchitem
	...
	sortorder: 'desc',
	usepager: true,
	title: '<span style=font-size:22px>$table_title</span>',
	useRp: true,
	rp: 15,
...
var xAddWebSites$t= function (obj) {
	var results=obj.responseText;
...
function AddWebSites$t(){
	var pp=prompt('$give_the_website');
	if(!pp){return;}
	var XHR = new XHRConnection();
	var ppb=encodeURIComponent(pp);
	XHR.appendData('addwww',ppb);
	XHR.appendData('category','{$_GET["category"]}');
	XHR.sendAndLoad('$page', 'POST',xAddWebSites$t);
}
	...
	function ReallyDeleteCategorizedWebsite(zmd5){
		MEMMD=zmd5;
		var XHR = new XHRConnection();
...
function query(){
	...
	if($COUNT_ROWS==0){json_error_show("category/COUNT_ROWS:$category ,$nowebsites",1);}
...
function DeleteCategorizedWebsiteReally(){
	...
	$q->QUERY_SQL("DELETE FROM webfiltering_categories_items WHERE zmd5='$md5'");
	if(!$q->ok){echo $q->mysql_error;}
	$sock=new sockets();
...
function save_items_www(){

	$category=url_decode_special_tool($_POST["category"]);
	$_POST["addwww"]=url_decode_special_tool($_POST["addwww"]);
	if(strpos($_POST["addwww"], ",")>0){
		$f=explode(",",$_POST["addwww"]);
	}else{
		$f[]=$_POST["addwww"];
	}
	...
	while (list ($num, $www) = each ($f) ){
		...
		if(preg_match("#http.*?:\/\/(.+?)[\/\s]+#",$www,$re)){$www=$re[1];}
	...
	while (list ($num, $www) = each ($websitesToscan) ){
		...
		$t[]="('$date','$md5','$category','$www',1)";
		
		
	}
	$q=new mysql_meta();
	if(count($t)>0){
		$sql="INSERT IGNORE INTO `webfiltering_categories_items` (zDate,zmd5,category,pattern,enabled) VALUES ".@implode(",", $t);
		$q->QUERY_SQL($sql);
		if(!$q->ok){echo $q->mysql_error;}
	}else{
		echo "No Website to add\n";
	}
	$sock=new sockets();
	$sock->getFrameWork("artica.php?scan-categories=yes");
}
