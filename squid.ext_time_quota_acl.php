...
function session_rule_js(){
	...
	if($_GET["ID"]>0){
		$q=new mysql();
		$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT * FROM ext_time_quota_acl WHERE ID={$_GET["ID"]}","artica_backup"));
...
function session_rule_settings(){
	...
	if($ID>0){
		$btname="{apply}";
		$q=new mysql();
		$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT * FROM ext_time_quota_acl WHERE ID=$ID","artica_backup"));
...
function rules_list(){
		...
		while ($ligne = mysql_fetch_assoc($results)) {
			...
			$data['rows'][] = array(
					...
					OnClick=\"javascript:Loadjs('$MyPage?session-rule-js=yes&ID={$ligne['ID']}&t=$t&tOrg=$tOrg');\"
