...
if(isset($_GET["verbose"])){$GLOBALS["VERBOSE"]=true;ini_set('display_errors', 1);ini_set('error_reporting', E_ALL);ini_set('error_prepend_string',null);ini_set('error_append_string',null);}
	...
	if(!isset($_GET["uid"])){if(isset($_POST["uid"])){$_GET["uid"]=$_POST["uid"];}}
	...
	if(isset($_GET["zarafaSendAsPrivilege"])){zarafaSendAsPrivilege_popup();exit;}
	if(isset($_GET["zarafaSendAsPrivilege-search"])){zarafaSendAsPrivilege_search();exit;}
	if(isset($_POST["zarafaSendAsPrivilege-add"])){zarafaSendAsPrivilege_add();exit;}
	if(isset($_POST["zarafaSendAsPrivilege-del"])){zarafaSendAsPrivilege_del();exit;}
...
function tabs(){
	...
	$users=new usersMenus();
	$tpl=new templates();
	$array["popup"]='{sender_parameters}';
	if($users>ZARAFA_INSTALLED){
		$array["zarafaSendAsPrivilege"]='{zarafaSendAsPrivilege}';
	}
...
while (list ($num, $ligne) = each ($array) ){
		$html[]= "<li><a href=\"$page?$num=yes&uid={$_GET["uid"]}\"><span style='font-size:13px;'>$ligne</span></a></li>\n";
	}
	
	$t=time();
	echo $tpl->_ENGINE_parse_body( "
	<div id=$t style='width:101.1%;font-size:14px;margin-left:-10px;margin-right:-15px;margin-top:-5px'>
		<ul>". implode("\n",$html)."</ul>
	</div>
		<script>
...
function zarafaSendAsPrivilege_search(){
	$page=CurrentPageName();
	$tpl=new templates();
	$u=new user($_GET["uid"]);
...
function zarafaSendAsPrivilege_add(){
	$user=new user($_POST["uid"]);
	$user2=new user($_POST["zarafaSendAsPrivilege-add"]);
	writelogs("zarafaSendAsPrivilege: adding UidNumber $user2->uidNumber",__FUNCTION__,__FILE__,__LINE__);
	$user->zarafaSendAsPrivilege_add($user2->uidNumber);
	
}
function zarafaSendAsPrivilege_del(){
	$user=new user($_POST["uid"]);
	$_POST["zarafaSendAsPrivilege-del"]=base64_decode($_POST["zarafaSendAsPrivilege-del"]);
	$UidNumber=$_POST["zarafaSendAsPrivilege-del"];
	$user->zarafaSendAsPrivilege_del($UidNumber);	
}
...
function {$prefix}Load(){
		YahooWin3(780,'$page?tabs=yes&uid={$_GET["uid"]}','$title');
