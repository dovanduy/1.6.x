...
function move(){
	...
	if($unix->process_exists($oldpid,basename(__FILE__))){
		...
		if($GLOBALS["OUTPUT"]){echo "Starting Already Artica task running PID $oldpid since {$time}mn\n";}
		return;
	}
	...
	while (list ($directory, $val) = each ($GLOBALS["SYSDIRS"]) ){
		...
		$cmd="$cp -rfb $directory/* $nextdirectory/";
		echo "$cmd\n";
		system($cmd);
		...
		if(!is_link($directory)){
			...
			system("$cp -rfb  $nextdirectory/* $directory/");
			continue;
		}
		
		$DirExploded=explode("/",$nextdirectory);
		while (list ($index, $subdir) = each ($DirExploded) ){if($subdir==null){continue;}$dir=$dir."/$subdir";@chmod($dir,0755);}
