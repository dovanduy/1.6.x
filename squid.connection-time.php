...
function EditTimeRule_js(){
	...
	if($ID>0){
			...
			$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT TimeName FROM webfilters_sqtimes_rules WHERE ID='$ID'"));
...
function EditTimeRule_groups_add(){
	...
	$ID=$_POST["ID"];
	$zmd5=md5($gpid.$ID);
	$sql="INSERT IGNORE INTO webfilters_sqtimes_assoc (gpid,zMD5,TimeRuleID) VALUES($gpid,'$zmd5',$ID)";
	$q=new mysql_squid_builder();
	$q->QUERY_SQL($sql);
