...
if(!$GLOBALS["NO_COMPILE"]){
	...
	if(!is_file("/root/{$GLOBALS["SRCNAME"]}")){
		system_admin_events("Detected new version {$GLOBALS["VERSION"]}", __FUNCTION__, __FILE__, __LINE__, "software");
	...
	shell_exec("$tar -xf /root/{$GLOBALS["SRCNAME"]} -C {$GLOBALS["WORKDIR"]}");
...
if(!is_file("/usr/local/sbin/vsftpd")){system_admin_events("Installing the new vsftpd {$GLOBALS["VERSION"]} failed", __FUNCTION__, __FILE__, __LINE__, "software"); echo "Failed\n";}
create_package();	
