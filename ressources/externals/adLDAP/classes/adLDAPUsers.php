...
class adLDAPUsers {
    ...
    */
    public function groups($username, $recursive = NULL, $isGUID = false)
    {
    	...
    	if($GLOBALS["VERBOSE"]){echo "Username: $username\n";}
    	$GLOBALS["CLASS_ACTV"][]=__FUNCTION__.": LINE:".__LINE__.": Username: $username\n";
    	if ($username === NULL) { return false; }
        if ($recursive === NULL) { $recursive = $this->adldap->getRecursiveGroups(); } // Use the default option if they haven't set it
        if (!$this->adldap->getLdapBind()) { 
        	if($GLOBALS["VERBOSE"]){echo "getLdapBind() return false\n";}
        ...
        $GLOBALS["CLASS_ACTV"][]=__FUNCTION__.": LINE:".__LINE__.": memberof:\"{$info[0]["memberof"]}\"\n";
