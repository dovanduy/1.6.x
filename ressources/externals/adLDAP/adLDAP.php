...
class adLDAP {
    ...
    */
    function __construct($options = array()) {
		...
		if($GLOBALS["VERBOSE"]){echo " ->Connect()\n";}
		$GLOBALS["CLASS_ACTV"][]=__FUNCTION__.": LINE:".__LINE__.": ->Connect() [".basename(__FILE__)."]";
        return $this->connect();
    }
    ...
    */
    public function connect() 
    {
        ...
        if(count($GLOBALS["CLASS_ACTV"])>250){$GLOBALS["CLASS_ACTV"]=array();}
        $domainController = $this->randomController();
        if ($this->useSSL) {
            $this->ldapConnection = ldap_connect("ldaps://" . $domainController, $this->adPort);
        } else {
        	$GLOBALS["CLASS_ACTV"][]=__FUNCTION__.": LINE:".__LINE__.": ->ldap_connect($domainController,$this->adPort)";
            $this->ldapConnection = ldap_connect($domainController, $this->adPort);
            if(!$this->ldapConnection){
            	$GLOBALS["CLASS_ACTV"][]=__FUNCTION__.": LINE:".__LINE__.": ->ldap_connect(... failed [".basename(__FILE__)."]";
            }else{
            	$GLOBALS["CLASS_ACTV"][]=__FUNCTION__.": LINE:".__LINE__.": ->ldap_connect(... success [".basename(__FILE__)."]";
            }
        }
        ...
        if ($this->adminUsername !== NULL && $this->adminPassword !== NULL) {
        	$GLOBALS["CLASS_ACTV"][]=__FUNCTION__.": LINE:".__LINE__.": -> @ldap_bind($this->ldapConnection, $this->adminUsername . $this->accountSuffix, $this->adminPassword.. [".basename(__FILE__)."]";
            ...
            if (!$this->ldapBind) {
                if ($this->useSSL && !$this->useTLS) {
                	...
                	$GLOBALS["CLASS_ACTV"][]=__FUNCTION__.": LINE:".__LINE__.": -> Bind to Active Directory failed. Check the login credentials and/or server details [".basename(__FILE__)."]";
        ...
        $GLOBALS["CLASS_ACTV"][]=__FUNCTION__.": LINE:".__LINE__.": $domainController OK [".basename(__FILE__)."]";
        if($GLOBALS["VERBOSE"]){echo "$domainController:: $this->adminUsername . $this->accountSuffix OK\n";}
        return true;
    }
    ...
    */
    public function close() {
        if ($this->ldapConnection) {
        	$GLOBALS["CLASS_ACTV"][]=__FUNCTION__.": LINE:".__LINE__.": ->ldap_close()";
