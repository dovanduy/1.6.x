...
if($usersmenus->AsArticaAdministrator==true){}else{die();exit;}		
	...
	if(isset($_POST["freshclam-mirror-add"])){freshclam_addmirror();exit;}
...
function freshclam_js(){
	...
	$html="
		function FreshclamLoad(){
			YahooWin2(550,'$page?freshclam-popup=yes','$title');
	...
	var x_AddFreshClamMirror= function (obj) {
		...
		function FreshClamSaveSettings(){
			var XHR = new XHRConnection();
			if(document.getElementById('EnableFreshClam').checked){XHR.appendData('EnableFreshClam',1);}else{XHR.appendData('EnableFreshClam',0);}
...
function freshclam_save(){
	$clam=new clamav();
	$sock=new sockets();
	$sock->SET_INFO("EnableFreshClam", $_GET["EnableFreshClam"]);
...
function freshclam_addmirror(){
	...
	$mirror=$_POST["freshclam-mirror-add"];
	$clam=new clamav();
