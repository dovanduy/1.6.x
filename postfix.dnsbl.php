...
function popup(){
	...
	<tr>
		<td class=legend style='font-size:22px'>{dnsbl_service}</td>
		<td>$list</td>
	</tr>
	<tr>
...
var xSave$t= function (obj) {
	...
	YahooWin3Hide();
	$('#POSTFIX_DNSBL_TABLE').flexReload();
}
...
function Save(){
	$q=new mysql();
	$q->QUERY_SQL("INSERT IGNORE INTO postfix_dnsbl (dnsbl,hostname) VALUE ('{$_POST["iii_DNSBL"]}','{$_POST["hostname"]}')","artica_backup");
...
function remove(){
	$q=new mysql();
	$q->QUERY_SQL("DELETE FROM postfix_dnsbl WHERE ID='{$_POST["remove"]}'","artica_backup");
	if(!$q->ok){echo $q->mysql_error;}
...
function table(){
	...
	$server=$tpl->_ENGINE_parse_body("{dnsbl_service}");
	...
	$title=$tpl->javascript_parse_text("{dnsbl_service}");
...
$buttons="
	buttons : [
		...
		$html="
		<table class='POSTFIX_DNSBL_TABLE' style='display: none' id='POSTFIX_DNSBL_TABLE' style='width:100%'></table>
		<script>
		var xsite='';
		$(document).ready(function(){
		$('#POSTFIX_DNSBL_TABLE').flexigrid({
...
var xRemovePostfixDNSBL= function (obj) {
	var tempvalue=obj.responseText;
...
function RemovePostfixDNSBL(ID){
	var XHR = new XHRConnection();
	XHR.appendData('remove',ID);
	XHR.sendAndLoad('$page', 'POST',xRemovePostfixDNSBL);
	}
...
function search(){
	...
	if($sql_search<>null){
		...
		$sql="SELECT COUNT(*) AS TCOUNT FROM postfix_dnsbl WHERE hostname='{$_GET["hostname"]}' $sql_search";
		$ligne=mysql_fetch_array($q->QUERY_SQL($sql,"artica_backup"));
		$total = $ligne["TCOUNT"];
	
	}else{
		$sql="SELECT COUNT(*) AS tcount FROM postfix_dnsbl WHERE hostname='{$_GET["hostname"]}'";
		$ligne=mysql_fetch_array($q->QUERY_SQL($sql,"artica_backup"));
		$total = $ligne["TCOUNT"];
	}
	...
	$sql="SELECT * FROM postfix_dnsbl WHERE hostname='{$_GET["hostname"]}' $sql_search $ORDER $limitSql";
	...
	while ($ligne = mysql_fetch_assoc($results)) {
		$ID=$ligne["ID"];
		$dnsbl=$ligne["dnsbl"];
		$delete=imgsimple("delete-42.png",null,"RemovePostfixDNSBL($ID)");
