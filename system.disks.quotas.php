...
function manage_quotas_popup_add(){
	...
	$member="	<tr>
		...
		<td>". Field_text("QuotaUser",null,"font-size:18px;font-wieght:bolder;width:220px")."</td>
		<td width=1%><input type='button' value='{browse}...' 
		OnClick=\"javascript:Loadjs('SambaBrowseUsers.php?field-user=QuotaUser&prepend=yes')\"
	...
	$member
	<tr>
		<td colspan=3><span style='font-size:18px'>{size}:</span></td>
	</tr>
	...
	<tr>
		<td colspan=3><hr><span style='font-size:18px'>{files}:</span></td>
	</tr>	
	<tr>
		<td class=legend>{quota_soft}:</td>
	...
	<tr>
		<td colspan=3 align='right'><hr>". button("{apply}","SaveMembersQuotas()","18px")."</td>
	</tr>
	</table>
...
<tbody class='tbody'>";	
	if(is_array($disks)){
		while (list ($part, $line) = each ($disks) ){
			...
			$icon=imgtootltip("Database32.png","{settings}","Loadjs('$page?manage-quotas=yes&mount=$upart')");
...
<table>
<tr>
<td align='left'>". button("{add}","AddQuotaUserGroup()","16px")."</td>
