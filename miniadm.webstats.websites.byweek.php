...
function main_page(){
	...
	$content=str_replace("{SCRIPT}", "<script>LoadAjax('globalContainer','$page?content=yes&year={$_GET["year"]}&month={$_GET["month"]}&week={$_GET["week"]}')</script>", $content);
	echo $content;	
}
function content(){
...
function webstats_middle_table_items(){
	...
	$table="(SELECT sitename, SUM(hits) as hits, SUM(size) as size FROM `$table_main` GROUP BY sitename) as t";
	
	$page=1;
	$FORCE_FILTER=null;
	
	
	if(!$q->TABLE_EXISTS($table_main, $database)){json_error_show("$table doesn't exists...");}
	if($q->COUNT_ROWS($table_main, $database)==0){json_error_show("No data");}
	...
	OnClick=\"javascript:Loadjs('squid.website-zoom.php?js=yes&sitename={$ligne["sitename"]}&week={$_GET["week"]}&year={$_GET["year"]}');\"
...
function generate_graph(){
	...
	$sql="SELECT SUM(hits) as thits, day FROM $tablename GROUP BY day ORDER BY day";
	...
	if(mysql_num_rows($results)>0){
			...
			$t=time();
			$targetedfile="ressources/logs/".md5(basename(__FILE__).".".__FUNCTION__.".week.$tablename").".png";
