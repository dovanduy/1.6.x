...
function page_old(){
	...
	<tr>
		...
		<td>". Field_text("SearchComputers",$_GET["def"],"font-size:16px",null,null,null,null,"SearchComputersCheck(event)")."</td>
		<td class=legend>{OnlyActive}:</td>
		<td>". Field_checkbox("OnlyActive",1,null,"SearchComputersCheckIsActive()")."</td>
	</tr>
	</table>
	...
	<script>
		...
		function SearchComputersCheckIsActive(){
		SearchComputers();
		}
		...
		function SearchComputers(){
			...
			if(document.getElementById('OnlyActive').checked){IsActive=1;}
			LoadAjax('SearchComputers-list','$page?SearchComputers='+se+u_f_FilterByDate+'&mode={$_GET["mode"]}&value={$_GET["value"]}&callback={$_GET["callback"]}&CorrectMac={$_GET["CorrectMac"]}&fullvalues={$_GET["fullvalues"]}&IsActive='+IsActive);
		}
		...
		function CheckIsActive(){
			var EnableScanComputersNet=$EnableScanComputersNet;
			if(EnableScanComputersNet==0){
				document.getElementById('OnlyActive').disabled=true;
				if(document.getElementById('OnlyActive').checked){document.getElementById('OnlyActive').checked=false;}
				}
		}		
