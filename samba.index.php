...
function SharedFolderListJS(){
	...
	echo "YahooWin2('750','$page?main=shared_folders&bypopup=yes','$title')";
}
...
function popup_js(){
	...
	$start="load();";
	if(isset($_GET["behavior"])){$start="Loadjs('samba.neighborhood.php');";}
...
function main_smb_config(){
	...
	"javascript:Loadjs('samba.neighborhood.php');",'{windows_network_neighborhood}',260,null,1);
...
<tr>
	...
	<td valign='top' style='$styleTD'>" . Field_array_Hash(array("no"=>"{no}","yes"=>"{yes}",
	"auto"=>"{auto}"),"domain master",$smb->main_array["global"]["domain master"],null,null,0,"width:100px;$styleTD;padding:3px")."</td>
...
<tr>
	...
	<td valign='top' style='$styleTD'>" . Field_text("workgroup",$smb->main_array["global"]["workgroup"],
	"width:{$styleFieldWith}px;$styleTD;padding:3px",null,null,null,false,null,$workgroup_disabled)."</td>
...
<tr>
	...
	<td valign='top' style='$styleTD'>" . Field_text("netbiosname",$smb->main_array["global"]["netbios name"],
	"width:{$styleFieldWith}px;$styleTD;padding:3px")."</td>
...
<tr>
	...
	<hr>
	". button("{apply}","SaveSambaMainConfiguration()","16px")
	."</td>
	</tr>
</table>
</div>
...
function jsaddons(){
	...
	var x_SaveSambaMainConfiguration=function (obj) {
		...
		if(document.getElementById('main_config_samba')){RefreshTab('main_config_samba');}
		if(document.getElementById('main_samba_quicklinks_config')){RefreshTab('main_samba_quicklinks_config');}
		if(!document.getElementById('main_config_samba')){if(document.getElementById('MainSambaConfigDiv')){
		Loadjs('samba.index.php?main-params-js=yes');}}
	   }
...
function shared_folders(){
	...
	$add_usb=$tpl->_ENGINE_parse_body("{external_devices}");
...
<table class='SAMBA_TABLE_SHARED_LIST' style='display: none' id='SAMBA_TABLE_SHARED_LIST' style='width:100%'></table>

	
<script>
var IDTMP=0;
$(document).ready(function(){
$('#SAMBA_TABLE_SHARED_LIST').flexigrid({
	url: '$page?SharedFoldersList=yes',
	dataType: 'json',
	colModel : [
		{display: '&nbsp;', name : 'icon1', width :56, sortable : false, align: 'center'},
...
function AddShared$t(){
	Loadjs('browse-disk.php?with-samba=yes');
}
function Defsets$t(){
...
function AddUsb$t(){
	Loadjs('samba.usb.php?t=$t');

}

Loadjs('js/samba.js');
QuickLinkShow('quicklinks-shared_folders');
</script>
...
function shared_folders_list(){
	...
	if(!is_array($folders)){json_error_show("No shared folders",1);}
	...
	while (list ($FOLDER, $ligne) = each ($folders) ){
		...
		if(strpos("   ".$samba->main_array[$FOLDER]["path"], "/automounts/")>0){$icon="usb-share-48.png";}	
		...
		$data['rows'][] = array(
			'id' => $md,
			'cell' => array(
			 imgsimple($icon,null,$propertiesjs),
			 "<div style='padding-top:10px'>$href$FOLDER</a></span>",
			$trash
			,"<div style='padding-top:10px'><strong>
...
function folder_properties(){
	...
	<td valign='top'>
		...
		". imgtootltip('share-computer-48.png','{simple_share}',"Loadjs('samba.simple.share.php?path=$path_encoded')")."<hr>
		". imgtootltip('good-files-48.png','{banned_files}',"Loadjs('samba.bannedfiles.php?path=$path_encoded')")."
	</td>
	<td valign='top' width=99%>
...
function SambaSaveFolderProperties(){
	...
	if(document.getElementById('dos_filemode').checked){XHR.appendData('dos_filemode','yes');}else{XHR.appendData('dos_filemode','no');}
...
var x_ChangeShareName=function (obj) {
	...
	function ChangeShareName(share){
		...
		AnimateDiv('main_config_folder_properties');
...
function folder_security(){
	...
	$browseMembers="YahooWin5(600,'$page?security=$folder_uri','{security} $folder');";
	...
	<div style='padding:5px;margin:5px;width:380px;' id='UserSecurityInfos'></div>
	<script>
		LoadAjax('userlists','$page?userlists=yes&prop=$folderEnc');
	</script>
	";
...
function folder_UserSecurityInfos(){
	...
	<script>
		function ChekzScurityUsrs(){
		var mustChecked=0;
		var mustread=0;
		if(document.getElementById('write list').checked){mustChecked=1;mustread=1}
		if(document.getElementById('read list').checked){mustChecked=1;}
		if(mustChecked==1){
			document.getElementById('valid users').checked=true;
		}
		
		if(mustread==1){
			document.getElementById('read list').checked=1;
		}
	...
	function ChekzScurityUsrs1(){
		if(!document.getElementById('valid users').checked){
			document.getElementById('write list').checked=false;
			document.getElementById('read list').checked=false;
		}
	}
	</script>
	<input type='hidden' name='SaveFolderProp' id='SaveFolderProp' value='$folder'>
	...
	<table style='width:100%'>
		<tr>
			...
			<td align='center' style='font-size:14px;font-weight:bold'>{write}</th>
			<td align='center' style='font-size:14px;font-weight:bold'>{read}</th>
		</tr>";
	
		$hash=$smb->hash_privileges($folder);
		$html=$html ."<tr>";
		$html=$html ."<td align='center'><div id='waitfolderprop'></div></td>";
		$html=$html ."<td align='center'>" .Field_yesno_checkbox('valid users',$hash[$_GET["UserSecurityInfos"]]["valid users"],"ChekzScurityUsrs1()") . "</td>";
		$html=$html ."<td align='center'>" .Field_yesno_checkbox('write list',$hash[$_GET["UserSecurityInfos"]]["write list"],"ChekzScurityUsrs()") . "</td>";
		$html=$html ."<td align='center'>" .Field_yesno_checkbox('read list',$hash[$_GET["UserSecurityInfos"]]["read list"],"ChekzScurityUsrs()") . "</td>";
		$html=$html ."</tr><tr>
...
function folder_properties_tab(){
	...
	$fontsize=14;
		...
		<a href=\"$page?propTab=$num&prop={$_GET["prop"]}\">
			<span style='font-size:{$fontsize}px'>$ligne</span>
		</a>
		</li>\n");
	}
...
function folder_security_users(){
	...
	<input type='hidden' id='IsTimeCapsule' value='{$_GET["TimeCapsule"]}'>
	<table style='width:100%'>
	<td>" . Field_text('query',null,'width:400px;font-size:14px;padding:5px',null,null,null,null,"FindUserGroupClick$t(event);")."</td>
	<td align='right'><input type='button' OnClick=\"javascript:FindUserGroup$t();\" value='{search}&nbsp;&raquo;'></td>
	</table>";
	...
	<script>
	function FindUserGroup$t(){
			var query=escape(document.getElementById('query').value);
			LoadAjax('finduserandgroupsid','$page?finduserandgroup=yes&query='+query+'&IsNFS={$_GET["nfs"]}&IsTimeCapsule={$_GET["TimeCapsule"]}');
		}
		
		
	function FindUserGroupClick$t(e){
		if(checkEnter(e)){FindUserGroup$t();}
...
function folder_security_list_users(){
	$folder=$_GET["prop"];
	$folderEnc=urldecode($folder);
	$page=CurrentPageName();
	$tpl=new templates();	
	$members=$tpl->_ENGINE_parse_body("{members}");
	$title=$tpl->_ENGINE_parse_body("{users_and_groups}");
	$t=time();
	$browseMembers="Loadjs('MembersBrowse.php?prepend=0&callback=AddUserToFolder&NOComputers=1')";
	$link_members=$tpl->_ENGINE_parse_body("{link_members}");
	$delete_all=$tpl->_ENGINE_parse_body("{delete_all}");
	$folder_text=$tpl->_ENGINE_parse_body("{directory}:");
	//

	
	$buttons="
	buttons : [
	{name: '$link_members', bclass: 'Add', onpress : LinkMember$t},
	{name: '$delete_all', bclass: 'Delz', onpress : DeleteUserPrivilegeAll$t},
...
$html="
<input type='hidden' name='flexRT-users-list' id='flexRT-users-list' value='$t'>
...
$(document).ready(function(){
$('#flexRT$t').flexigrid({
	url: '$page?folder-security-list-users-table=yes&prop=$folderEnc&t=$t',
	dataType: 'json',
	colModel : [
		{display: '&nbsp;', name : 'picture', width : 31, sortable : true, align: 'center'},	
		{display: '$members', name : 'members', width :440, sortable : false, align: 'left'},
...
function DeleteUserPrivilegeAll$t(){
	DeleteAllFolderSecu();
}
...
var x_DeleteUserPrivilege$t=function (obj) {
  	var results=obj.responseText;
	document.getElementById('animate-$t').innerHTML='';
	if(results.length>3){alert(results);return;}
	if(document.getElementById('row'+ memid$t)){
		$('#row'+ memid$t).remove();
	}else{
		$('#flexRT$t').flexReload();
	}
...
function DeleteUserPrivilege$t(item,id){
	memid$t=id;
    mem_folder_name='$folder';
    var XHR = new XHRConnection();
    XHR.appendData('SaveFolderProp','$folder');
    ...
    XHR.sendAndLoad('$page', 'POST',x_DeleteUserPrivilege$t);
}	
...
function folder_security_users_table(){
		...
		while (list ($num, $ligne) = each ($usr) ){
			if($query<>null){if(!preg_match("#$query#", $ligne)){continue;}}
			...
			'cell' => array(
				...
				$Displayname</a></span>",
				imgsimple("delete-24.png",null,"DeleteUserPrivilege$t('$encoded','$id')")
				)
			);		
	}
	$data['total'] = $c;
...
function folder_security_list_users_old(){
	$folder=$_GET["prop"];
	$smb=new samba();
...
function folder_security_users_find(){
	...
	if(strlen($_GET["IsTimeCapsule"])>2){$IsTimeCapsule=true;}
...
<tbody class='tbody'>";	
	...
	while (list ($num, $ligne) = each ($hash) ){
		...
		if($IsTimeCapsule){$js="Loadjs('time.capsule.php?AddUservalue=$num&root={$_GET["IsTimeCapsule"]}')";}
...
function folder_delete(){
	...
	$md5=md5($_GET["FolderDelete"]);
	$q=new mysql();
	$q->QUERY_SQL("DELETE FROM `samba_veto_files` WHERE `md5path`='$md5'",'artica_backup');
	$samba->SaveToLdap();
	}
...
function neighborhood_index(){
	...
	<script>
		LoadAjax('$t','samba.neighborhood.php');
	</script>
