<?php
	...
	function js(){
		...
		$tpl=new templates();
		$title=$tpl->_ENGINE_parse_body("{certificate_bundle}:{$_GET["CommonName"]}");
...
function certificate_edit_crt(){
	...
	$button_save=$tpl->_ENGINE_parse_body(button($apply,"SaveBundle$tt()",26));
...
<script>
var xSaveBundle$tt=function (obj) {
...
function SaveBundle$tt(){
	...
	XHR.sendAndLoad('$page', 'POST',xSaveBundle$tt);
}
</script>
";
