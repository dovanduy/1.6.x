...
function STREAM_LINE(){
	...
	$EnableWebProxyStatsAppliance=$sock->GET_INFO("EnableWebProxyStatsAppliance");
	if(!is_numeric($EnableWebProxyStatsAppliance)){$EnableWebProxyStatsAppliance=0;}
	if($EnableWebProxyStatsAppliance==0){writelogs("EnableWebProxyStatsAppliance=$EnableWebProxyStatsAppliance from ".$_SERVER["REMOTE_ADDR"] ." (aborting)",__FUNCTION__,__FILE__,__LINE__);die();}
	$q=new mysql();
...
function GetProxyName($hostname,$ipaddr){
	$q=new mysql_squid_builder();
	$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT ipaddr FROM squidservers WHERE hostname ='$hostname'"));
	...
	$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT hostname FROM squidservers WHERE ipaddr ='$ipaddr'"));
...
function SQUID_GRAPHS(){
    ...
    $cmd="/bin/tar -xf $moved_file -C $content_dir/";
    writelogs("$cmd",__FUNCTION__,__FILE__,__LINE__);
    exec("$cmd 2>&1",$results);
    while (list ($num, $val) = each ($results) ){writelogs("$val",__FUNCTION__,__FILE__,__LINE__);	}
