...
function manual_update(){
	...
	$page=CurrentPageName();
	$tpl=new templates();
	$UploadAFile=$tpl->javascript_parse_text("{key_file}");
	$allowedExtensions="allowedExtensions: ['key'],";
	$UploadAFile=str_replace(" ", "&nbsp;", $UploadAFile);
	$html="
	<H2>{upload_new_license}</H2>
...
function upload_artica_final(){
	...
	echo "
	<script>
		Loadjs('Kav4Proxy.license.progress.php?filename=$fileNameEnc');
	</script>
