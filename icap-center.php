...
function service_js(){
	...
	if($ID>0){
		...
		$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT service_name FROM c_icap_services WHERE ID=$ID"));
...
function move_items(){
	...
	$t=$_POST["t"];
	$dir=$_POST["dir"];
	$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT zOrder FROM c_icap_services WHERE ID='$ID'"));
	if(!$q->ok){echo $q->mysql_error;}
...
function service_popup(){
	...
	if($ID>0){
		$btname="{apply}";
		$q=new mysql_squid_builder();
		$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT * FROM c_icap_services WHERE ID=$ID"));
	}
