...
$unix=new unix();
$GLOBALS["PHP5_DAEMON"]="exec.hypercache-tail.php";
$GLOBALS["DAEMON_NAME"]="hypercache-tail";
$GLOBALS["TAIL_STARTUP"]=$unix->LOCATE_PHP5_BIN()." /usr/share/artica-postfix/{$GLOBALS["PHP5_DAEMON"]}";
$GLOBALS["log_path"]="/var/log/squid/hypercache-plugin.log";
$GLOBALS["PID_TAIL_PATH"]="/etc/artica-postfix/exec.hypercache-tail.php.pid";
...
function GETPID(){
	...
	$pid=$unix->PIDOF_PATTERN($GLOBALS["PHP5_DAEMON"]);
	if($unix->process_exists($pid)){return $pid;}
	$pid=$unix->PIDOF("/bin/hypercache-tail");
	if($unix->process_exists($pid)){return $pid;}
...
function start($aspid=false){
	...
	if($HyperCacheStoreID==0){
		if($GLOBALS["OUTPUT"]){echo "Starting......: ".date("H:i:s")." [INIT]: HyperCache is disabled...\n";}
		return;
	...
	if(!is_file("/bin/hypercache-tail")){@copy($tail, "/bin/hypercache-tail");}
	...
	for($i=0;$i<6;$i++){
		$pid=GETPID();
		if($unix->process_exists($pid)){break;}
		if($GLOBALS["OUTPUT"]){echo "Starting......: ".date("H:i:s")." [INIT]: {$GLOBALS["DAEMON_NAME"]}, waiting $i/5\n";}
		sleep(1);
	}
	$pid=GETPID();
	if($unix->process_exists($pid)){
...
function stop($aspid=false){
	...
	for($i=0;$i<6;$i++){
		$pid=GETPID();
		if(!$unix->process_exists($pid)){break;}
		if($GLOBALS["OUTPUT"]){echo "Stopping......: ".date("H:i:s")." [INIT]: {$GLOBALS["DAEMON_NAME"]}, waiting pid: $pid $i/5\n";}
		unix_system_kill_force($pid);
		sleep(1);
	}	
	...
	$pid=GETPID();
	if(!$unix->process_exists($pid)){
		events("{$GLOBALS["DAEMON_NAME"]} success to stop daemon...");
		if($GLOBALS["OUTPUT"]){echo "Stopping......: ".date("H:i:s")." [INIT]: {$GLOBALS["DAEMON_NAME"]}, success\n";}
	}else{
		events("{$GLOBALS["DAEMON_NAME"]} failed to stop daemon...");
