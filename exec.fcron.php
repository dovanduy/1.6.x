...
if($argv[1]=="--BULK_IMAP_SCHEDULE"){BULK_IMAP_SCHEDULE();die();}
...
function BULK_IMAP_SCHEDULE(){
	$sock=new sockets();
	$unix=new unix();	
	$targetfile="/etc/cron.d/BulkImapSchedule";
	@unlink($targetfile);
	...
	if($ImapBulkImapExportEnable==0){return;}
	$BULK_IMAP_SCHEDULE=$array["BULK_IMAP_SCHEDULE"];
	if($GLOBALS["VERBOSE"]){echo "BULK_IMAP_SCHEDULE = $BULK_IMAP_SCHEDULE -> $ImapBulkImapExportEnable\n";}
	
	$php5=$unix->LOCATE_PHP5_BIN();
 	$f[]="PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/X11R6/bin:/usr/share/artica-postfix/bin";
	$f[]="MAILTO=\"\"";
	$f[]="$BULK_IMAP_SCHEDULE  root $php5 /usr/share/artica-postfix/exec.bulkimap.php >/dev/null 2>&1";
	$f[]="";	
	if($GLOBALS["VERBOSE"]){echo " -> $targetfile\n";}
	@file_put_contents($targetfile,implode("\n",$f));
	if(!is_file($targetfile)){if($GLOBALS["VERBOSE"]){echo " -> $targetfile No such file\n";}}
