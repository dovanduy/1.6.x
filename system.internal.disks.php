...
function tabs(){
	...
	$array["philesight"]='{APP_PHILESIGHT}';
	...
	while (list ($num, $ligne) = each ($array) ){
			...
			if($num=="philesight"){
				$html[]= $tpl->_ENGINE_parse_body("<li><a href=\"philesight.php?popup=yes\"><span $style>$ligne</span></a></li>\n");
				continue;
			}		
		
		
		if($num=="BTRFS"){
				$html[]= $tpl->_ENGINE_parse_body("<li><a href=\"btrfs.php\"><span $style>$ligne</span></a></li>\n");
				continue;
			}		
		
		if($num=="loop"){
			$html[]= $tpl->_ENGINE_parse_body("<li><a href=\"system.disks.loop.php\"><span $style>$ligne</span></a></li>\n");
			continue;
		}	
		
		
		if($num=="iscsi"){
			$html[]= $tpl->_ENGINE_parse_body("<li><a href=\"system.disks.iscsi.php\"><span $style>$ligne</span></a></li>\n");
			continue;
		}		
		
		if($num=="acls"){
			$html[]= $tpl->_ENGINE_parse_body("<li><a href=\"samba.shared.folders.list.php?acldisks=yes\"><span $style>$ligne</span></a></li>\n");
			continue;
		}
		if($num=="quotas"){
			$html[]= $tpl->_ENGINE_parse_body("<li><a href=\"system.disks.quotas.php\"><span $style>$ligne</span></a></li>\n");
			continue;
		}

		if($num=="smartctl"){
			$html[]= $tpl->_ENGINE_parse_body("<li><a href=\"system.disks.smart.php\"><span $style>$ligne</span></a></li>\n");
			continue;
		}

		if($num=="LVM"){
			$html[]= $tpl->_ENGINE_parse_body("<li><a href=\"system.disks.lvm.php\"><span $style>$ligne</span></a></li>\n");
			continue;			
		}
...
function js(){
	...
	var mem_vggroup='';
		...
		var x_LastLogsFormat= function (obj) {
				...
				if(document.getElementById('HardDrivesTasksSection')){document.getElementById('HardDrivesTasksSection').innerHTML='';}					
		}			
