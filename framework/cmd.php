...
function STUNNEL_RESTART(){
	$unix=new unix();
	$nohup=$unix->find_program("nohup");
	$cmd="$nohup /etc/init.d/artica-postfix restart stunnel >/dev/null 2>&1 &";
	writelogs_framework($cmd,__FUNCTION__,__FILE__,__LINE__);
	shell_exec("$cmd");
}
...
function squid_rebuild(){
	...
	$nohup=$unix->find_program("nohup");
	$php5=$unix->LOCATE_PHP5_BIN();
	$cmd="$nohup $php5 /usr/share/artica-postfix/exec.squid.php --build >/dev/null 2>&1 &";
	writelogs_framework($cmd,__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);
}
...
function fetchmail_logs(){
	...
	exec("$grep -E '$search' /var/log/fetchmail.log|$tail -n $rp",$results);
	echo "<articadatascgi>". base64_encode(serialize($results))."</articadatascgi>";
...
function ARTICA_MAILLOG_RESTART(){
	...
	shell_exec("$nohup /etc/init.d/artica-postfix restart auth-logger");
}
...
function SYSLOG_QUERY(){
	...
	if(strlen($pattern)>1){
		if(($preprend<>null) && (strlen($preprend)>3)){$grep="$grepbin '$preprend'|$grepbin -E '$pattern'";}else{
			$grep="$grepbin -E '$pattern'";
		}
	}
