...
function members_mac(){
	...
	if(isset($GLOBALS["members_mac_executed"])){return;}
...
function members_mac_from_hourtable($tablename,$time){
	$zdate=date("Y-m-d",$time);
	$q=new mysql_squid_builder();
	$sql="SELECT MAC, SUM(size) as size,SUM(hits) as hits FROM $tablename GROUP BY MAC
	HAVING LENGTH(MAC)>0";
	$results=$q->QUERY_SQL($sql);
	if(!$q->ok){if($GLOBALS["VERBOSE"]){echo "############# ERROR #########\n$q->mysql_error\Line:".__LINE__."\n#############\n";}return false;}
