...
function getent(){
	...
	if($GLOBALS["VERBOSE"]){system_admin_events("[getent passwd]:: Executing, please wait....",__FUNCTION__,__FILE__,__LINE__,"samba");}
	exec("$getent passwd 2>&1",$results);
	if($GLOBALS["VERBOSE"]){system_admin_events("[getent passwd]:: parsing  ".count($results)." elements ",__FUNCTION__,__FILE__,__LINE__,"samba");}
	...
	while (list ($num, $ligne) = each ($results) ){
		if(preg_match("#^(.+?):.+?:#", $ligne,$re)){
			...
			$sql[]="('{$re[1]}')";
			if(count($sql)>500){
				system_admin_events("[getent passwd]:: Inserting ".count($sql)." elements ",__FUNCTION__,__FILE__,__LINE__,"samba");
	...
	if(is_file($wbinfo)){
		...
		while (list ($num, $DOMAIN) = each ($cmd_domains_list) ){
			...
			$results=array();		
			if($GLOBALS["VERBOSE"]){system_admin_events("[wbinfo -u]::  Executing for $DOMAIN, please wait",__FUNCTION__,__FILE__,__LINE__,"samba");}
			exec("$wbinfo -u --domain=$DOMAIN 2>&1",$results);
			system_admin_events("[wbinfo -u]:: parsing  ".count($results)." elements for domain $DOMAIN",__FUNCTION__,__FILE__,__LINE__,"samba");
			while (list ($num, $user) = each ($results) ){
				if(preg_match("#^Error looking#", $user,$re)){
					system_admin_events("[wbinfo -u]:: wbinfo (users) for $DOMAIN failed ",__FUNCTION__,__FILE__,__LINE__,"samba");
				...
				$sql[]="('$user')";
				if(count($sql)>500){
					if($GLOBALS["VERBOSE"]){system_admin_events("[wbinfo -u]:: Inserting ".count($sql)." elements ",__FUNCTION__,__FILE__,__LINE__,"samba");}
					$sqlfinal=$prefix.@implode(",", $sql);
					$q->QUERY_SQL($sqlfinal,"artica_backup");
					if(!$q->ok){echo $q->mysql_error."\n";}
					$sql=array();
...
if(count($sql)>1){
	...
	while (list ($num, $ligne) = each ($results) ){
		if(preg_match("#^(.+?):.+?([0-9]+):#", $ligne,$re)){
			...
			$sql[]="('{$re[1]}','$gpid')";
			if(count($sql)>500){
				system_admin_events("[getent group]:: Inserting ".count($sql)." elements ",__FUNCTION__,__FILE__,__LINE__,"samba");
				$sqlfinal=$prefix.@implode(",", $sql);
				$q->QUERY_SQL($sqlfinal,"artica_backup");
...
if(count($sql)>0){
	...
	if(is_file($wbinfo)){
			...
			if(trim($DOMAIN)==null){continue;}
			$results=array();
			if($GLOBALS["VERBOSE"]){system_admin_events("[wbinfo -g]:: checks domain $DOMAIN, please wait...",__FUNCTION__,__FILE__,__LINE__,"samba");}
			...
			while (list ($num, $group) = each ($results) ){
				...
				$sql[]="('$group')";
				if(count($sql)>500){
					system_admin_events("[wbinfo -g]:: Inserting ".count($sql)." elements ",__FUNCTION__,__FILE__,__LINE__,"samba");
					$sqlfinal=$prefix.@implode(",", $sql);
					$q->QUERY_SQL($sqlfinal,"artica_backup");
					if(!$q->ok){echo $q->mysql_error."\n";}
					$sql=array();
...
if(count($sql)>1){
	system_admin_events("[GROUPS]:: Inserting ".count($sql)." elements ",__FUNCTION__,__FILE__,__LINE__,"samba");
	$sqlfinal=$prefix.@implode(",", $sql);
	$q->QUERY_SQL($sqlfinal,"artica_backup");
	if(!$q->ok){echo $q->mysql_error."\n";}
	$sql=array();
