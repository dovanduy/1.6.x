...
function js(){
	...
	$suffix=suffix();
	$t=time();
	$title=$tpl->_ENGINE_parse_body("{categories_this_month}::{$_GET["category"]}:: {$_GET["field"]}::{$_GET["value"]}");
...
function suffix(){
	...
	return "&field={$_GET["field"]}&value=".urlencode($_GET["value"])."&time={$_GET["time"]}&category=".urlencode($_GET["category"]);
}
...
function tabs(){
	...
	$suffix=suffix();
	$array["flow-day"]="{$_GET["category"]}: {flow_by_day}";
	$array["websites"]="{$_GET["category"]}: {websites}";
...
function flow_day(){
	...
	FROM {$date}_maccess GROUP BY {$_GET["field"]},category,tday 
	HAVING {$_GET["field"]}='{$_GET["value"]}' AND category='{$_GET["category"]}' ORDER BY tday";
	$time=time();
...
function websites_search(){
	...
	$table="(SELECT SUM(hits) as hits,SUM(size) as size,familysite,{$_GET["field"]},category
	FROM {$date}_maccess GROUP BY familysite,{$_GET["field"]} 
	HAVING {$_GET["field"]}='{$_GET["value"]}' AND category='{$_GET["category"]}') as t";
