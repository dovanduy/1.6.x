...
function table(){
	...
	$q=new mysql();
	$CountEvents=$q->COUNT_ROWS("system_rotate_events", "artica_events");
	...
	{display: '$description', name : 'description', width : 959, sortable : false, align: 'left'},
	],$buttons
	searchitems : [
...
function search(){
	...
	$sock=new sockets();
	$q=new mysql();
	$table="system_rotate_events";
	
	if($q->COUNT_ROWS($table,"artica_events")==0){
		json_error_show("Empty table...",1);
	}
	...
	while ($ligne = mysql_fetch_assoc($results)) {
		...
		$color="black";
		if(preg_match("#(error|fatal|overloaded|aborting|Failed)#i", $description)){
			$color="#BA0000";
		}
