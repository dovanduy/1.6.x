...
if(!$users->CORP_LICENSE){
	...
	$f[]="";
	@file_put_contents("/tmp/bash_license_menu.sh", @implode("\n",$f));
	@chmod("/tmp/bash_license_menu.sh",0755);
	die();
...
$f[]="trap \"rm \$OUTPUT; rm \$INPUT; exit\" SIGHUP SIGINT SIGTERM";
$f[]="DIALOG=\${DIALOG=dialog}";
$f[]="\t$DIALOG --title \"Corporate Edition\" --msgbox \"".@implode("\\n", $text)."\" 20 70";
$f[]="";
@file_put_contents("/tmp/bash_license_menu.sh", @implode("\n",$f));
@chmod("/tmp/bash_license_menu.sh",0755);
die();
...
$diag[]="KEYBOARD \"Setup your keyboard\"";
$diag[]="LANGUAGE \"Setup the language\"";
...
if($GLOBALS["VERBOSE"]){echo "Writing /tmp/bash_license_menu.sh\n";}
@file_put_contents("/tmp/bash_license_menu.sh", @implode("\n",$f));
@chmod("/tmp/bash_license_menu.sh",0755);
