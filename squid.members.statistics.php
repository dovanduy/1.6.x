...
function tabs(){
	...
	if($num=="query"){
			$html[]= "<li><a href=\"squid.members.query.php\"><span>$ligne</span></a></li>\n";
			continue;
		}	
...
function day_top_members(){
	...
	$sql="SELECT SUM(hits) as tcount,client,uid,MAC,hostname FROM $tablesrc GROUP BY client,uid,MAC,hostname ORDER BY SUM(hits) DESC LIMIT 0,10";
	$dayText=date('l',$date);
	if(!$q->TABLE_EXISTS($tablesrc)){return;}
	...
	if(!$q->ok){echo "<H2>$q->mysql_error</H2><center style='font-size:11px'><code>$sql</code></center>";}	
		...
		while($ligne=@mysql_fetch_array($results,MYSQL_ASSOC)){	
			...
			if($uid==null){
				...
				OnClick=\"javascript:LoadAjax('day_graphs_for_members','$page?day-graphs-perform=yes&day={$_GET["day"]}&user-field=client&user=$ipaddr');\" 
				style='text-decoration:underline'>$legendHREF</a>";
				
				if($mac<>null){
					$uid="<a href=\"javascript:blur();\" 
					OnClick=\"javascript:LoadAjax('day_graphs_for_members','$page?day-graphs-perform=yes&day={$_GET["day"]}&user-field=MAC&user=$mac');\" 
					style='text-decoration:underline'>$legendHREF</a>";
				}
				...
				$uid="<a href=\"javascript:blur();\" 
				OnClick=\"javascript:LoadAjax('day_graphs_for_members','$page?day-graphs-perform=yes&day={$_GET["day"]}&user-field=uid&user=$uid');\" 
				style='text-decoration:underline'>$uid</a>";
			...
			$table=$table.
				...
				$legend=$ligne["client"];
				if($hostname<>null){$legend=$hostname;}
				$gp->xdata[]=$ligne["tcount"];
				$gp->ydata[]=$ligne["uid"]."-".$legend;
		}	
...
function day_user_search(){
	...
	if(!$q->TABLE_EXISTS($tablesrc)){return;}
	$sql="SELECT client,uid,MAC,hostname FROM `$tablesrc` GROUP BY client,uid,MAC,hostname 
...
<tbody>";
	...
	while($ligne=@mysql_fetch_array($results,MYSQL_ASSOC)){
			...
			$ipaddrlink=
			"<a href=\"javascript:blur();\" 
				OnClick=\"javascript:LoadAjax('day_graphs_for_members','$page?day-graphs-perform=yes&day={$_GET["day"]}&user-field=client&user={$ligne["client"]}');\" 
				style='text-decoration:underline'>{$ligne["client"]}</a>";
			...
			$legendHREF=$ipaddr;
				...
				$userfield="&user-field=client&user=$ipaddr";	
				if($mac<>null){$userfield="&user-field=MAC&user=$mac";}	
				$uid="<a href=\"javascript:blur();\" 
				OnClick=\"javascript:LoadAjax('day_graphs_for_members','$page?day-graphs-perform=yes&day={$_GET["day"]}$userfield');\" 
				style='text-decoration:underline;font-weight:bold'>$legendHREF</a>";
			}else{
				$uid="<a href=\"javascript:blur();\" 
				OnClick=\"javascript:LoadAjax('day_graphs_for_members','$page?day-graphs-perform=yes&day={$_GET["day"]}&user-field=uid&user=$uid');\" 
				style='text-decoration:underline'>$uid</a>";
...
function status(){
	...
	<td valign='top' width=1%>
		...
		<td valign='top' width=99% style='padding-left:15px'><div id='squid-members-graphs' style='text-align:center'></div></td>
	</tr>
	</tbody>
	</table>
	<script>
