...
function table(){
	...
	$q=new mysql();
	$sys_alerts=FormatNumber($q->COUNT_ROWS("sys_alerts", "artica_events"));
	...
	{display: '$load', name : 'load', width : 215, sortable : true, align: 'right'},
	
	],
	$buttons
	searchitems : [
	{display: '$date', name : 'zDate'},
	{display: '$load', name : 'load'},
	],
	sortname: 'zDate',
	sortorder: 'desc',
	usepager: true,
...
function search(){
	...
	$FORCE_FILTER=null;
	$total=0;
	$table="(SELECT zDate,`load`,`zmd5` FROM sys_alerts) as t";
...
$fontsize=20;
	...
	while ($ligne = mysql_fetch_assoc($results)) {
		...
		if(strlen($ligne["load"])==3){$ligne["load"]="{$ligne["load"]}0";}
		if(strlen($ligne["load"])==1){$ligne["load"]="{$ligne["load"]}.00";}
		$data['rows'][] = array(
		'id' => $ms5,
		'cell' => array(
			"<span $style>$js{$date}</a></span>",
			"<span $style>{$js}{$ligne["load"]}</a></span>",
			)
		);
