...
$user=new usersMenus();
if($user->AsSquidAdministrator==false){echo FATAL_ERROR_SHOW_128("{ERROR_NO_PRIVS}");die();}
...
function object_js(){
	...
	if($ID==0){
		$TITLE=$tpl->javascript_parse_text("{new_quota_object}");
	}else{
		$q=new mysql_squid_builder();
		$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT objectname FROM quota_objects WHERE ID='$ID'"));
...
function delete_js(){
	...
	$tpl=new templates();
	$q=new mysql_squid_builder();
	$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT objectname FROM quota_objects WHERE ID='{$_GET["ID"]}'"));
	$TITLE=utf8_encode($ligne["objectname"]);
...
var xDel$t= function (obj) {
	...
	$('#TABLE_QUOTAS_OBJECT').flexReload();
}
...
function Delete(){
	...
	$q=new mysql_squid_builder();
	$q->QUERY_SQL("DELETE FROM webfilters_sqgroups WHERE GroupType='time_quota:{$_POST["delete"]}'");
	if(!$q->ok){echo $q->mysql_error;}
	$q->QUERY_SQL("DELETE FROM quota_objects WHERE ID='{$_POST["delete"]}'");
	if(!$q->ok){echo $q->mysql_error;}
...
function object_popup(){
	...
	if($ID>0){
		$btname="{apply}";
		$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT * FROM quota_objects WHERE ID='$ID'"));
...
<div style='width:98%' class=form>
	...
	<tr>
		<td class=legend style='font-size:22px'>". texttooltip("{pauselen} ({seconds})","{pauselen_explain}").":</td>
		<td>". Field_text("sleep-$t",$ligne["sleep"],"font-size:22px;width:220px")."</td>
	</tr>
						
				
				
	<tr>
		<td class=legend style='font-size:22px'>". texttooltip("{ttl} ({seconds})","{squid_ttl_explain}").":</td>
...
function Save$t(){
	...
	XHR.appendData('sleep', document.getElementById('sleep-$t').value);
...
function Save(){
	...
	if($ID==0){
		...
		$q->QUERY_SQL("INSERT IGNORE INTO `quota_objects` (objectname,identifier,ttl,sleep) 
				VALUES ('$objectname','$identifier','$ttl','$sleep')");
		
	}else{
		
		$q->QUERY_SQL("UPDATE `quota_objects` 
...
function table(){
	...
	$new_quota_object=$tpl->_ENGINE_parse_body("{new_quota_object}");
	$title=$tpl->javascript_parse_text("{quota_objects}");
	...
	$q=new mysql_squid_builder();
	$sql="CREATE TABLE IF NOT EXISTS `quota_objects` (
...
<table class='$t' style='display: none' id='TABLE_QUOTAS_OBJECT' style='width:99%;text-align:left'></table>
<script>
$(document).ready(function(){
$('#TABLE_QUOTAS_OBJECT').flexigrid({
	...
	dataType: 'json',
		...
		{display: '<span style=font-size:22px>$pauselen</span>', name : 'sleep', width : 240, sortable : false, align: 'left'},
...
function search(){
	...
	while ($ligne = mysql_fetch_assoc($results)) {
		...
		OnClick=\"javascript:Loadjs('$MyPage?object-js=yesID={$ligne["ID"]}');\"
