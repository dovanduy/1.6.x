...
$diag[]="JOIN \"Join a Meta server\"";
$diag[]="PING \"Ping the Meta server\"";
...
$f[]="function JOINMETA(){";
$f[]="\tEnableArticaMetaServer=$EnableArticaMetaServer";
$f[]="\tif [ EnableArticaMetaServer eq 1]; then";
$f[]="\t$DIALOG --title \"Join Meta\" --msgbox \"Sorry,This is a meta server\" 9 70";
$f[]="\treturn";
$f[]="\tfi";
$f[]="\t$DIALOG --clear --title \"Give the address of the meta server\" --inputbox \"Give the address of the meta server\\nExample: 255.255.255.0\" 10 68 {$ArticaMetaHost} 2> $DPREF/ArticaMetaHost";
$f[]="\tif [ $? = 1 -o $? = 255 ]; then";
$f[]="\t\trm -f $DPREF/ArticaMetaHost";
$f[]="\t\treturn";
$f[]="\tfi";		
$f[]="\t$DIALOG --clear --title \"ENTER the listen port of the Meta server\" --inputbox \"Enter the listen port of the meta server\" 10 68 {$ArticaMetaPort} 2> $DPREF/ArticaMetaPort";
$f[]="\tif [ $? = 1 -o $? = 255 ]; then";
$f[]="\t\trm -f $DPREF/ArticaMetaPort";
$f[]="\t\treturn";
$f[]="\tfi";	
$ArticaMetaUsername=$unix->shellEscapeChars($ArticaMetaUsername);
$f[]="\t$DIALOG --clear --title \"ENTER Meta administrator account for authentication\" --inputbox \"ENTER Meta username for authentication\" 10 68 \"{$ArticaMetaUsername}\" 2> $DPREF/ArticaMetaUsername";
$f[]="\t$DIALOG --clear --insecure --passwordbox \"ENTER Meta administrator password for authentication\" 10 68 \"{$ArticaMetaPassword}\" 2> $DPREF/ArticaMetaPassword";
$f[]="\t$DIALOG --title \"Join Meta server ?\" --yesno \"Join the Meta server ? Press 'Yes' to continue, or 'No' to exit\" 0 0";
$f[]="\tcase $? in";
$f[]="\t\t0)";
$f[]="\tif [ -f /tmp/dns.log ]; then";
$f[]="\t\trm /tmp/dns.log";
$f[]="\tfi";
$f[]="\t$php5 /usr/share/artica-postfix/exec.artica-meta-client.php --ping --output --progress --force --enable >/tmp/dns.log &";
...
$f[]="\t\trm /tmp/dns.log";
$f[]="\tfi";
$f[]="\t$php5 /usr/share/artica-postfix/exec.artica-meta-client.php --ping --output --progress --force >/tmp/dns.log 2>&1 &";
$f[]="\t$DIALOG --tailbox /tmp/dns.log  25 150";
$f[]="}";
$f[]="";
...
@file_put_contents("/tmp/bash_meta_menu.sh", @implode("\n",$f));
@chmod("/tmp/bash_meta_menu.sh",0755);
