...
function js(){
	...
	$title=$tpl->javascript_parse_text("{upload2} access.log");
...
function manual_update(){
	...
	$page=CurrentPageName();
	$tpl=new templates();
	$UploadAFile=$tpl->javascript_parse_text("{upload_a_file} access.log");
	$allowedExtensions="allowedExtensions: ['log'],";
	$UploadAFile=str_replace(" ", "&nbsp;", $UploadAFile);
	$html="
	<H2 style='margin-bottom:20px'>{test_categories_rate}</H2>
...
function upload_artica_final(){
	...
	echo "<script>YahooWinBrowseHide();Loadjs('ufdbcat.check.progress.php?filename=$fileName');</script>";
