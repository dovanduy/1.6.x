...
function rules_list(){
	...
	while ($ligne = mysql_fetch_assoc($results)) {
		...
		$data['rows'][] = array(
				'id' => "acl{$ligne['ID']}",
				'cell' => array("<a href=\"javascript:blur();\" OnClick=\"javascript:Loadjs('$MyPage?session-rule-js=yes&ID={$ligne['ID']}&t=$t&tOrg=$tOrg&mainrule=$mainrule&tt=$tt');\" style=\"font-size:16px;text-decoration:underline;color:$color\">{$ligne['rulename']}</a>$gplist",
...
function session_rule_js(){
	...
	if($mainrule>0){
		$q=new mysql();
		$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT QuotaName FROM ext_time_quota_acl WHERE ID=$mainrule","artica_backup"));
...
function session_rule_settings(){
	...
	if($ID>0){
		$btname="{apply}";
		$q=new mysql();
		$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT * FROM ext_time_quota_acl_rules WHERE ID=$ID","artica_backup"));
...
function session_rule_enable(){
	...
	$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT enabled FROM ext_time_quota_acl_rules WHERE ID=$ID","artica_backup"));
