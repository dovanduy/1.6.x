...
if(isset($_POST["servername"])){Upload();exit;}
...
function popup_index(){
	...
	unset($lang["language"]);
	$lang[null]="{default}";
	$language=Field_array_Hash($lang,'OUlanguage',$langdef,null,null,0,"font-size:14px;padding:5px");	
	...
	<td valign='top'>". Field_text('organization_name',$ou,"font-size:14px;padding:3px")."</td>
...
function EXPORT_ORG(){
	...
	$users=new usersMenus();
	$button=button("{send}","UploadOrganization()",16);
	if($users->EnableManageUsersTroughActiveDirectory){$button=null;}
	if(!is_numeric($ini->_params["CONF"]["port"])){$ini->_params["CONF"]["port"]=9000;}
	if($ini->_params["CONF"]["username"]==null){$ini->_params["CONF"]["username"]="Manager";}
	$form2="
	<table style='width:99%;margin-top:15px' class=form>
	<tr>
		<td valign='top' class=legend nowrap style='font-size:14px'>{REMOTE_ARTICA_SERVER}:</td>
		<td valign='top' >". Field_text("servername-$t",$ini->_params["CONF"]["servername"],"font-size:14px;padding:3px")."</td>
	</tr>
	<tr>
		<td valign='top' class=legend nowrap style='font-size:14px'>{REMOTE_ARTICA_SERVER_PORT}:</td>
		<td valign='top' >". Field_text("port-$t",$ini->_params["CONF"]["port"],"font-size:14px;padding:3px;width:60px")."</td>
	</tr>
	<tr>
		<td valign='top' class=legend nowrap style='font-size:14px'>{REMOTE_ARTICA_USERNAME}:</td>
		<td valign='top' >". Field_text("username-$t",$ini->_params["CONF"]["username"],"font-size:14px;padding:3px")."</td>
	</tr>	
	<tr>
		<td valign='top' class=legend nowrap style='font-size:14px'>{REMOTE_ARTICA_PASSWORD}:</td>
		<td valign='top'>". Field_password("password-$t",$ini->_params["CONF"]["password"],"font-size:14px;padding:3px")."</td>
	</tr>		
	<tr>
	...
	$html="<div style='font-size:16px;font-weight:bold'>{duplicate_to_remote_server}</div>
	$form2
	<br>
	...
	function UploadOrganization(){
		...
		XHR.sendAndLoad('$page', 'POST', x_UploadOrganization);			
	}
	</script>	
