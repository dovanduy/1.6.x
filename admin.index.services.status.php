...
function memory(){
	...
	return  $os->html_Memory_usage()."<script>
		LoadAjax('system-events','artica.events.php?popup=yes&full-medium=yes');
...
while (list ($num, $ligne) = each ($array) ){
		...
		$html[]= "<li><a href=\"$page?status=yes&section=$num&filterby={$_GET["filterby"]}\"><span style='font-size:14px'>$ligne</span></a></li>\n";
	...
	return "
	<div id=services_status style='width:100%;height:870px;overflow:auto'>
		<ul>". implode("\n",$html)."</ul>
	</div>
		<script>
...
function INDEX(){
	...
	<div id='system-events' style='width:98%;height:560px;overflow:auto;margin-top:5px' class=form></div>
		<script>
...
function SERVICES_STATUS(){
	...
	if($_GET["section"]=="artica_services"){
			...
			$html=$html . BuildRow($users,$ini->_params["APP_MONIT"],"{APP_MONIT}");
			
			
				while (list ($key, $array) = each ($ini->_params) ){
					if(preg_match("#ARTICA_MYSQL:([0-9]+)#",$key,$re)){
						$html=$html . BuildRow($users,$ini->_params[$key],"{APP_MYSQL_ARTICA}","instance:{$re[1]}");
					}
				}
				reset($ini->_params);	
			...
			$html=$html . BuildRow($users,$ini->_params["APP_ARKEIAD"],"{APP_ARKEIAD}");
			$html=$html . BuildRow($users,$ini->_params["APP_ARKWSD"],"{APP_ARKWSD}");
	...
	if($_GET["section"]=="mail_services"){
		...
		$html=$html . BuildRow($users,$ini->_params["ROUNDCUBE"],"{APP_ROUNDCUBE}");
				...
				$arrayZarafa[]="APP_ZARAFA_SEARCH";
				$arrayZarafa[]="APP_YAFFAS";
