...
//EnableDisableForceCategory
	...
	var x_EnableDisableForceCategory= function (obj) {
		var results=obj.responseText;
		if(results.length>3){alert(results);}
	}

function EnableDisableForceCategory(category,md){
		...
		XHR.sendAndLoad('$page', 'POST',x_EnableDisableForceCategory);
...
function categories_save(){
	$q=new mysql_squid_builder();
	if($_POST["enable"]==1){
		$sql="INSERT IGNORE INTO usersisp_blkcatz (category) VALUES ('{$_POST["category"]}')";
		$q->QUERY_SQL("");
		
	}else{
		$sql="DELETE FROM usersisp_blkcatz WHERE category='{$_POST["category"]}'";
