...
function ShowID_js(){
	...
	$sql="SELECT subject FROM blackwhite_admin_mysql WHERE ID=$id";
	$q=new mysql();
...
$tpl=new templates();
$sql="SELECT content FROM blackwhite_admin_mysql WHERE ID={$_GET["ShowID"]}";
$q=new mysql();
$ligne=@mysql_fetch_array($q->QUERY_SQL($sql,"artica_events"));
...
function empty_table(){
	$q=new mysql();
	$q->QUERY_SQL("TRUNCATE TABLE blackwhite_admin_mysql","artica_events");
}
...
function events_table(){
	...
	$total=0;
	if($q->COUNT_ROWS($table,"artica_events")==0){json_error_show("no row",1);}
