...
function load_balance_form_backend_search(){
	...
	$give_the_remote_listen_port=$tpl->javascript_parse_text("{give_the_remote_listen_port}");
	...
	<script>
		...
		function EnableServiceCross2(){
			...
			if(document.getElementById('enabled').checked){
				...
				document.getElementById('www_port').disabled=false;		
			}
		}
...
function load_balance_form_status_service_stats(){
	...
	while (list ($index, $arrays) = each ($array)){
		...
		$html=$html."<table style='width:99%;margin-top:5px' class=form>
		<tbody>
			<tr>
