...
function object_js(){
	...
	if($ID==0){
		...
		$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT objectname FROM sessions_objects WHERE ID='$ID'"));
...
function Delete(){
	...
	$q=new mysql_squid_builder();
	$q->QUERY_SQL("DELETE FROM ads_domains WHERE GroupType='time_session:ACTIVE:{$_POST["delete"]}'");
	$q->QUERY_SQL("DELETE FROM ads_domains WHERE GroupType='time_session:LOGIN:{$_POST["delete"]}'");
	$q->QUERY_SQL("DELETE FROM ads_domains WHERE GroupType='time_session:LOGOUT:{$_POST["delete"]}'");
...
function object_popup(){
	...
	if($ID>0){
		$btname="{apply}";
		$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT * FROM sessions_objects WHERE ID='$ID'"));
...
function table(){
	...
	$object_name=$tpl->javascript_parse_text("{domains}");
	...
	$updatenow=$tpl->javascript_parse_text("{update_now}");
	$tablewidht=883;
	$t=time();
...
$buttons="buttons : [
		{name: '<strong style=font-size:18px>$updatenow</strong>', bclass: 'Add', onpress : AddQuota$t},
],	";
echo "
	
<table class='$t' style='display: none' id='TABLE_ADS_OBJECT' style='width:99%;text-align:left'></table>
<script>
$(document).ready(function(){
$('#TABLE_ADS_OBJECT').flexigrid({
	url: '$page?search=yes&t=$t',
	dataType: 'json',
		colModel : [
		{display: '<span style=font-size:22px>$object_name</span>', name : 'servername', width : 450, sortable : false, align: 'left'},
		{display: '<span style=font-size:22px>$enabled</span>', name : 'enabled', width : 90, sortable : false, align: 'left'},
