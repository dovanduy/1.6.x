<?php
	...
	if(isset($_POST["X_HEADER"])){headers_save();exit;}
...
function ajax_js(){
	...
	if(isset($_GET["in-front-ajax"])){
		$start="$('#BodyContent').load('$page?ajax-pop=yes&in-front-ajax=yes');";	
	}
...
function ajax_popup(){
	...
	if(isset($_GET["in-front-ajax"])){
		$array["amavis-status"]='{status}';
		$ajaxpop="&in-front-ajax=yes";
...
function main_select_popups(){
		...
		case "headers":headers_popup();break;
...
<table style='width:99%' class=form>	
	...
	var x_SaveAmavisSpamAssassinSingle= function (obj) {
		...
		XHR.appendData('INI_SAVE','BEHAVIORS');
	      if(document.getElementById('$t-spam_subject_tag_maps_enable').checked){XHR.appendData('spam_subject_tag_maps_enable',1);}else{XHR.appendData('spam_subject_tag_maps_enable',0);}
	      if(document.getElementById('$t-spam_quarantine_spammy').checked){XHR.appendData('spam_quarantine_spammy',1);}else{XHR.appendData('spam_quarantine_spammy',0);}
	      if(document.getElementById('$t-spam_quarantine_spammy2').checked){XHR.appendData('spam_quarantine_spammy2',1);}else{XHR.appendData('spam_quarantine_spammy2',0);}
	      if(document.getElementById('replicate_conf_all_domains').checked){XHR.appendData('replicate_conf_all_domains',1);}else{XHR.appendData('replicate_conf_all_domains',0);}
		  
	      XHR.appendData('sa_tag2_level_deflt',document.getElementById('$t-sa_tag2_level_deflt').value);
	      XHR.appendData('sa_tag3_level_deflt',document.getElementById('$t-sa_tag3_level_deflt').value);
	      XHR.appendData('sa_kill_level_deflt',document.getElementById('$t-sa_kill_level_deflt').value);
	      XHR.appendData('sa_dsn_cutoff_level',document.getElementById('$t-sa_dsn_cutoff_level').value);
	      XHR.appendData('sa_quarantine_cutoff_level',document.getElementById('$t-sa_quarantine_cutoff_level').value);
	      XHR.appendData('spam_subject_tag_maps',document.getElementById('$t-spam_subject_tag_maps').value);
	      XHR.appendData('sa_tag_level_deflt',document.getElementById('$t-sa_tag_level_deflt').value);
	      XHR.appendData('spam_subject_tag2_maps',document.getElementById('$t-spam_subject_tag2_maps').value);
   	      XHR.appendData('sa_timeout',document.getElementById('$t-sa_timeout').value);
	      
		  AnimateDiv('$t');
		  XHR.sendAndLoad('$page', 'GET',x_SaveAmavisSpamAssassinSingle);
		}	
...
function notification_popup(){
	...
	$array["headers"]=$tpl->_ENGINE_parse_body('{headers}');


	while (list ($num, $ligne) = each ($array) ){
		$html[]= "<li><a href=\"$page?popup=$num\"><span style='font-size:14px'>$ligne</span></a></li>\n";
	}
...
if(strpos(" {$amavis->main_array["HEADERS"]["X_HEADER_LINE"]}",'$')>0){$amavis->main_array["HEADERS"]["X_HEADER_LINE"]=null;}
if($amavis->main_array["HEADERS"]["X_HEADER_LINE"]==null){$def="<div style='font-size:11px'>{default}:\$myproduct_name \$myversion_id (\$myversion_date) at \$mydomain</div>";}
$html="
<div id='div-$t'>
<table style='width:99%' class=form>
<tr>
	<td class=legend style='font-size:14px'>{add} X-Virus-Scanned</td>
	<td>". Field_checkbox("X_HEADER", 1,$amavis->main_array["HEADERS"]["X_HEADER"],"CheckAmavisXhedear()")."</td>
</tr>
<tr>
	<td class=legend style='font-size:14px'>TAG:</td>
	<td>". Field_text("X_HEADER_TAG", $amavis->main_array["HEADERS"]["X_HEADER_TAG"],"font-size:14px;width:210px")."</td>
</tr>
<tr>
	<td class=legend style='font-size:14px'>{value}:</td>
	<td>". Field_text("X_HEADER_LINE", $amavis->main_array["HEADERS"]["X_HEADER_LINE"],"font-size:14px;width:310px")."$def</td>
</tr>
<tr>
	<td colspan=2 align='right'><hr>". button("{apply}","SaveAmavisXHeader()",16)."</td>
</tr>
</table>
</div>
<script>
	var x_SaveAmavisXHeader=function (obj) {
			var results=obj.responseText;
			if (results.length>0){alert(results);}
			RefreshTab('main_amavis_notifications');
		}
	
		function SaveAmavisXHeader(){
			var XHR = new XHRConnection();
			
			if(document.getElementById('X_HEADER').checked){XHR.appendData('X_HEADER',1);}else{XHR.appendData('X_HEADER',0);}
			XHR.appendData('X_HEADER_TAG',document.getElementById('X_HEADER_TAG').value);
			XHR.appendData('X_HEADER_LINE',document.getElementById('X_HEADER_LINE').value);
			XHR.appendData('virus_admin',document.getElementById('virus_admin').value);			
			AnimateDiv('div-$t');
			XHR.sendAndLoad('$page', 'POST',x_SaveAmavisXHeader);			
		}
		
		function CheckAmavisXhedear(){
			document.getElementById('X_HEADER_TAG').disabled=true;
			document.getElementById('X_HEADER_LINE').disabled=true;
			if(document.getElementById('X_HEADER').checked){
				document.getElementById('X_HEADER_TAG').disabled=false;
				document.getElementById('X_HEADER_LINE').disabled=false;			
			}
...
function headers_save(){
	$amavis=new amavis();
	
	if(strpos($_POST["X_HEADER_LINE"], '$')>0){
		echo "Fatal: '$' not allowed!\n";
		return;	
	}
	
	while (list ($key, $line) = each ($_POST) ){
		$amavis->main_array["HEADERS"][$key]=$line;
...
function filterbehavior_popup(){
	...
	while (list ($num, $ligne) = each ($array) ){
		...
		if($num=="bypass"){
			$html[]=$tpl->_ENGINE_parse_body("<li><a href=\"amavis.bypass.php\"><span style='font-size:14px'>$ligne</span></a></li>\n");
			continue;
		}
		
		$html[]=$tpl->_ENGINE_parse_body("<li><a href=\"$page?popup=$num\"><span style='font-size:14px'>$ligne</span></a></li>\n");
	}
	
	
	echo "
	<div id=main_config_amavisfilterbehavior style='width:100%;height:630px;overflow:auto'>
		<ul>". implode("\n",$html)."</ul>
	</div>
		<script>
				$(document).ready(function(){
