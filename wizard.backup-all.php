...
$user=new usersMenus();
	...
	if(isset($_GET["W_SSH_SERVER"])){
		$_SESSION["WIZARD"]["W_SSH_SERVER"]=$_GET["W_SSH_SERVER"];
		$_SESSION["WIZARD"]["W_SSH_USERNAME"]=$_GET["W_SSH_USERNAME"];
		$_SESSION["WIZARD"]["W_SSH_SHAREDDIR"]=$_GET["W_SSH_SHAREDDIR"];
		exit;
	}		
...
function CloseTimeOut(){
	...
	function WizardSSHSaveRessource(){
		var XHR = new XHRConnection();
		var W_SSH_SERVER=document.getElementById('W_SSH_SERVER').value;
		if(W_SSH_SERVER.length>1){
				XHR.appendData('W_SSH_SERVER',W_SSH_SERVER);
				XHR.appendData('W_SSH_USERNAME',document.getElementById('W_SSH_USERNAME').value);
				XHR.appendData('W_SSH_SHAREDDIR',document.getElementById('W_SSH_SHAREDDIR').value);
				XHR.sendAndLoad('$page', 'GET',x_WizardUSBSaveRessource);
			}
	}	
...
function popup_finish(){
	...
	switch ($_SESSION["WIZARD"]["W_RESOURCE"]) {
		...
		case "ssh":$html=$html."<div style='color:rgb(194, 51, 2);font-size:13px'>{remote_ssh_service}: ssh://{$_SESSION["WIZARD"]["W_SSH_SERVER"]}:{$_SESSION["WIZARD"]["W_SSH_SHAREDDIR"]}</div>";break;
...
<div class=form style='width:95%;margin:10px'>
	...
	<table style='width:100%'>
		<tr>
			<td width=50%>". button("{back}","WizardScheduleShow()",16)."</td>
...
<table style='width:99%' class=form>
		...
		<tr>
			<td style='font-size:14px' align='right'>{time}:</td>
			<td nowrap>
			<table>
			<tr>
			<td>$hours</td><td>:</td><td>$mins</td>
			</tr>
			</table>
			</td>
		</tr>	
...
<hr>
	<table style='width:100%'>
		<tr>
			<td width=50%>". button("{back}","WizardRessourceShow()",16)."</td>
			<td width=50% align='right'>". button("{next}","WizardBackupScheduleSave()",16)."</td>
		</tr>
	</table>	
	";
$tpl=new templates();
...
function popup_resource(){
	...
	switch ($_SESSION["WIZARD"]["W_RESOURCE"]) {
		...
		case "ssh":$html=$html.popup_resource_ssh();break;
...
function popup_resource_smb(){
	...
	<table style='width:99%' class=form>
		<tr>
			...
			<td>". Field_text("W_SMB_SERVER",$_SESSION["WIZARD"]["W_SMB_SERVER"],"font-size:14px;padding:5px")."</td>
		</tr>	
		<tr>
			<td style='font-size:14px' align='right'>{username}:</td>
			<td>". Field_text("W_SMB_USERNAME",$_SESSION["WIZARD"]["W_SMB_USERNAME"],"font-size:14px;padding:5px")."</td>
		</tr>
		<tr>
			<td style='font-size:14px' align='right'>{password}:</td>
			<td>". Field_password("W_SMB_PASSWORD",$_SESSION["WIZARD"]["W_SMB_PASSWORD"],"font-size:14px;padding:5px")."</td>
		</tr>	
		<tr>
			<td style='font-size:14px' align='right'>{shared_folder}:</td>
			<td>". Field_text("W_SMB_SHAREDDIR",$_SESSION["WIZARD"]["W_SMB_SHAREDDIR"],"font-size:14px;padding:5px")."</td>
		</tr>	
	</table>			
...
<hr>
	<table style='width:100%'>
		<tr>
			<td width=50%>". button("{back}","WizardBackupLoad()",16)."</td>
			<td width=50% align='right'>". button("{next}","WizardSMBSaveRessource()",16)."</td>
		</tr>
	</table>	
	";
	return $html;
...
function popup_resource_ssh(){
	
	$t=time();
	$sock=new sockets();
	$users=unserialize(base64_decode($sock->getFrameWork("cmd.php?unixLocalUsers=yes")));
	$userF=Field_array_Hash($users,"W_SSH_USERNAME",$_SESSION["WIZARD"]["W_SSH_USERNAME"],"style:font-size:14px;padding:3px");
	...
	<table style='width:99%' class=form>
		<tr>
			<td style='font-size:14px' align='right'>{hostname}:</td>
			<td>". Field_text("W_SSH_SERVER",$_SESSION["WIZARD"]["W_SSH_SERVER"],"font-size:14px;padding:5px")."</td>
		</tr>	
		<tr>
			<td style='font-size:14px' align='right'>{username}:</td>
			<td>$userF</td>
		...
		<tr>
			<td style='font-size:14px' align='right'>{target_folder}:</td>
			<td>". Field_text("W_SSH_SHAREDDIR",$_SESSION["WIZARD"]["W_SSH_SHAREDDIR"],"font-size:14px;padding:5px")."</td>
		</tr>	
	</table>			
...
<hr>
	<table style='width:100%'>
		<tr>
			<td width=50%>". button("{back}","WizardBackupLoad()",16)."</td>
			<td width=50% align='right'>". button("{next}","WizardSSHSaveRessource()",16)."</td>
		</tr>
	</table>	
	";
	return $html;	
...
function popup_resource_local(){
	...
	<table style='width:100%'>
		<tr>
			<td width=50%>". button("{back}","WizardBackupLoad()",16)."</td>
			<td width=50% align='right'>". button("{next}","WizardLocalSaveRessource()",16)."</td>
		</tr>
	</table>	
<script>
...
function popup(){
	...
	if($users->SSHFS_INSTALLED){
		$storages["ssh"]="{remote_ssh_service}";
	}
	
	$select=Field_array_Hash($storages,"storage",null,null,null,0,"font-size:18px;padding:5px;;");
	...
	<table style='width:100%'>
		<tr>
			<td width=50%>". button("{cancel}","CancelBackupWizard()",16)."</td>
			<td width=50% align='right'>". button("{next}","WizardRessource()",16)."</td>
		</tr>
	</table>
	</td></tr></table>	
	";
...
function BACKUP_COMPILE(){
	...
	switch ($_SESSION["WIZARD"]["W_RESOURCE"]) {
		...
		case "ssh":$pattern=$backup->build_ssh_protocol($_SESSION["WIZARD"]["W_SSH_SERVER"],
		$_SESSION["WIZARD"]["W_SSH_SHAREDDIR"],
		$_SESSION["WIZARD"]["W_SSH_USERNAME"],
		$_SESSION["WIZARD"]["W_SSH_PASSWORD"]);break;		
