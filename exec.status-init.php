...
function start($aspid=false){
	...
	if($EnableArticaStatus==0){
		if($GLOBALS["OUTPUT"]){echo "Starting......: ".date("H:i:s")." [INIT]: {$GLOBALS["TITLENAME"]} service disabled (see EnableArticaStatus)\n";}
		return;
	}
...
function stop($aspid=false){
	...
	if($GLOBALS["FORCE"]){shell_exec("$kill -9 $pid >/dev/null 2>&1");}
	for($i=0;$i<5;$i++){
		$pid=PID_NUM();
		if(!$unix->process_exists($pid)){break;}
...
function PID_NUM(){
	...
	return $unix->PIDOF_PATTERN("$php5\s+". dirname(__FILE__)."/exec.status.php");
