...
function js(){
	...
	$title=$tpl->javascript_parse_text("{upload_a_file}: {sources_files}");
...
function manual_update(){
	...
	$allowedExtensions="allowedExtensions: ['gz','zip'],";
	$UploadAFile=str_replace(" ", "&nbsp;", $UploadAFile);
	$html="
	<H2>{upload_a_file}: {sources_files} (*.gz,*.zip)</H2>
...
function upload_artica_final(){
	...
	echo "
	<script>
		Loadjs('squid.statistics.import.upload.progress.php?filename=$fileNameEnc');
