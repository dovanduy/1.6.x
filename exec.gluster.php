...
if($argv[1]=='--client-dismount'){CLIENT_DISMOUNT_FOLDERS();die();}
...
if($argv[1]=='--mounted-client'){CLIENT_LIST_MOUNTED_FOLDERS();die();}
...
if($argv[1]=='--help'){help();die();}
if($argv[1]=='-h'){help();die();}
...
function ReceiveParams(){
	...
	system_admin_events("Success receive order from master server {$_SERVER['REMOTE_ADDR']}" , __FUNCTION__, __FILE__, __LINE__, "cluster");
	$q=new mysql();
	$q->QUERY_SQL($sql,"artica_backup");
	if(!$q->ok){
		system_admin_events("failed $q->mysql_error" , __FUNCTION__, __FILE__, __LINE__, "cluster");
...
function MASTER_GET_SOURCE_CONNECT($host,$ID){
	...
	if(!preg_match("#<SOURCES>(.+?)</SOURCES>#s",$curl->data,$re)){
		...
		system_admin_events("failed $host, unable to preg_match - Error parsing sources" , __FUNCTION__, __FILE__, __LINE__, "cluster");
	...
	system_admin_events("receive ". count($paths)." directories from $host" , __FUNCTION__, __FILE__, __LINE__, "cluster");
	...
	$q->QUERY_SQL($sql,"artica_backup");
	if(!$q->ok){
		system_admin_events("Error $q->mysql_error from $host" , __FUNCTION__, __FILE__, __LINE__, "cluster");
	...
	if($DirectoryNumber>0){
		...
		system_admin_events("Success update clustered $DirectoryNumber directorie(s) from $host" , __FUNCTION__, __FILE__, __LINE__, "cluster");
	}
...
function CLIENT_DISMOUNT_FOLDERS(){
	...
	if(is_array($array)){
		while (list ($index, $volfile) = each ($array) ){
			...
			system_admin_events("umount foler $index for \"".$volfile."\"\n".@implode("\n", $results), __FUNCTION__, __FILE__, __LINE__, "cluster");
		}
	}
...
function probes(){
	...
	if($GLOBALS["VERBOSE"]){echo "SELECT client_ip,hostname FROM glusters_clients WHERE LENGTH(uuid)=0 = ".mysql_num_rows($results)." rows\n";}
...
function ROUNDCUBE_REPLIC(){
	...
	if(!is_array($auth)){echo "<ERROR>token auth no such array</ERROR>";die();	}	
	$ldap=new clladp();
	$q=new mysql();
	if(strtolower($ldap->ldap_admin)<>strtolower($auth["username"])){echo "<ERROR>Authentication failed</ERROR>";die();}
	if(md5($ldap->ldap_password)<>$auth["password"]){echo "<ERROR>Authentication failed</ERROR>";die();	}	
	$servername=trim($_POST["RDCUBE-REPLIC"]);
	if($servername==null){echo "<ERROR>Local FreeWebs server name is not set</ERROR>";die();}
	$free=new freeweb($servername);
	if($free->loading_error){echo "<ERROR>Internal error $free->loading_error_text</ERROR>";die();}
	$database=$free->mysql_database;
	if($free->groupware<>"ROUNDCUBE"){echo "<ERROR>Local FreeWebs server $servername Mysql Database:$database is not a roundcube server ($free->groupware)</ERROR>";die();}
