...
function popup(){
	...
	$styleright="style='padding-left:3px;border-left:3px solid #CCCCCC'";
	if($_GET["philesight"]<>null){
		$styleright="width=0%";
		$styleleft="width=100%";
	}
	$html="
	...
	<td valign='top' $styleright><div id='browser-infos'></div></td>
	</tr>
	</table>
	...
	function LoadTree(){
		$(document).ready(function(){
			$('#browser').treeview({
			url: '$page?startpath={$_GET["root"]}&philesight={$_GET["philesight"]}&field={$_GET["field"]}&with-capsule={$_GET["with-capsule"]}&org-root={$_GET["root"]}&replace-start-root={$_GET["replace-start-root"]}&with-samba={$_GET["with-samba"]}'
	...
	function BrowserExpand(encpath){
		LoadAjax('browser-infos','$page?browser-infos='+encpath+'&philesight={$_GET["philesight"]}&field={$_GET["field"]}&with-capsule={$_GET["with-capsule"]}&org-root={$_GET["root"]}&replace-start-root={$_GET["replace-start-root"]}&with-samba={$_GET["with-samba"]}');
...
function brower_infos(){
	...
	if($_GET["philesight"]<>null){
		echo "
		". $tpl->_ENGINE_parse_body("<center style='margin:10px'>". button("{browse}...", "Loadjs('browse-reps.php?root={$_GET["browser-infos"]}')",12)."</div>")."
		
		<script>
			 LoadAjax('{$_GET["philesight"]}','philesight.php?imgphile=$root');
			</script>
		";
		return;
	...
	if(!is_numeric($_GET["with-capsule"])){$_GET["with-capsule"]=0;}
	...
	if($_GET["with-capsule"]==1){
		include_once(dirname(__FILE__)."/ressources/class.mysql.inc");
		$q=new mysql();
		$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT sharedname FROM netatalk WHERE directory='$root'","artica_backup"));
		
		
		if($ligne["sharedname"]==null){
			$samba="
			<tr>
				<td width=1% valign='top'>
			" . imgtootltip('folder-granted-48.png','{share_this}',"ShareCapsule()")."</td>
				<td><a href=\"javascript:blur();\" OnClick=\"javascript:ShareCapsule()\"  style='font-size:14px;text-decoration:underline'>{share_this}</td>
			</tr>
			";		
		}else{
			$samba="
	...
	var x_SmbShare2= function (obj) {
	 	...
	 	if(document.getElementById('timedCapsule')){RefreshTimeCapsuleList();}
		}
	...
	function SmbShare2(){
	 ...
	 function ShareCapsule(){
	 	if(confirm('$share_this')){
		        ...
		        var XHR = new XHRConnection();
		        XHR.appendData('AddTreeFolders','$root');
		        XHR.sendAndLoad('time.capsule.php', 'POST',x_SmbShare2);
	        }          
	 } 
