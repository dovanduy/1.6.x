...
function showhits(){
	...
	$sql="SELECT SUM(hits) as tcount,uri FROM $dansguardian_events WHERE sitename='$www' GROUP BY uri ORDER BY tcount DESC LIMIT 0,200";
	$q=new mysql();
	$results=$q->QUERY_SQL($sql,"artica_events");	
...
function thumbnail($zmd5=null){
	...
	if($zmd5<>null){$md=$zmd5;}
	if(isset($_GET["zmd5"])){$md=$_GET["zmd5"];}
	...
	if(is_file($imagepath)){
		...
		if($GLOBALS["VERBOSE"]){echo "<li>$imagepath no such file</li>";}
	}
	
	if(!is_file($imageSourcePath)){
		$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT savedate,filemd5,picture,LinkTo FROM webfilters_thumbnails WHERE zmd5='$md'"));
		if($GLOBALS["VERBOSE"]){echo "<li>$md LinkTo={$ligne["LinkTo"]} - ". strlen($ligne["picture"])." bytes</li>";}
		if(trim($ligne["LinkTo"])<>null){thumbnail($ligne["LinkTo"]);return;}
		if($ligne["picture"]==null){if($md_family<>$md){thumbnail($md_family);return;}}
		if($ligne["picture"]<>null){
			@file_put_contents($imageSourcePath, $ligne["picture"]);
		}
	}else{
		if($GLOBALS["VERBOSE"]){echo "<li>$md $imageSourcePath exists on disk</li>";}
		if($GLOBALS["VERBOSE"]){echo "<li>$imageSourcePath file_md5=". md5_file($imageSourcePath)."</li>";}
	}
	...
	if(is_file($imageSourcePath)){
		if($width<400){
			if($GLOBALS["VERBOSE"]){echo "<li>$md  ->SimpleImage($imageSourcePath)</li>";}
			$image = new SimpleImage();
	  		$image->load($imageSourcePath);
			if($GLOBALS["VERBOSE"]){echo "<li>$md  ->resize($width,$width)</li>";}
