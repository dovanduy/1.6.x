...
function items_js(){
	...
	$q=new mysql();
	$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT QuotaType FROM ext_time_quota_acl WHERE ID=$mainrule","artica_backup"));
...
function items_enabled(){
	...
	$q=new mysql();
	$ligne=mysql_fetch_array($q->QUERY_SQL("SELECT enabled FROM ext_time_quota_acl_link WHERE zmd5='$md5'","artica_backup"));
