...
function deleteall(){
	$q=new mysql();
	$q->QUERY_SQL("DROP TABLE perfs_queue","artica_events");
...
function unlink_js(){
	...
	$database="artica_events";
	$page=CurrentPageName();
	$xdate=$_GET["unlink-js"];
	



	$q=new mysql();
	$sql="SELECT subject FROM $table WHERE zDate='$xdate'";
	$ligne=mysql_fetch_array($q->QUERY_SQL($sql,$database));
	$title=$tpl->time_to_date(strtotime($ligne["subject"]),true);
	$title=$tpl->javascript_parse_text("{delete}: $title $xdate");
...
function unlink_perform(){
	$q=new mysql();
	$q->QUERY_SQL("DELETE FROM perfs_queue WHERE zDate='{$_POST["unlink"]}'",'artica_events');
...
function search(){
	...
	while ($ligne = mysql_fetch_assoc($results)) {
		...
		$delete=imgsimple("delete-32.png",null,"Loadjs('$MyPage?unlink-js=$xdateenc')");
		$restore=imgsimple("32-import.png",null,"Loadjs('snapshots.restore.php?ID={$ligne["ID"]}')");
		$download="<a href=\"$MyPage?snapshot-download=$xdateenc\"><img src=img/download-32.png></a>";
		$cell=array();
		$cell[]="<span $style>$xdate - $date</a></span>";
		$cell[]="<span $style>$subject</a></span>";
...
function content_table(){
	...
	<table class='artica_events_DETAILS_TABLE' style='display: none' id='artica_events_DETAILS_TABLE' style='width:100%'></table>
	<script>
	$(document).ready(function(){
	$('#artica_events_DETAILS_TABLE').flexigrid({
...
function content_search(){
	...
	if(!$q->TABLE_EXISTS($table,$database)){
		json_error_show("no data - no table $table/$database");
	}
...
function  download_snapshot(){
	...
	$sql="SELECT * FROM perfs_queue WHERE zDate='$ID'";
	$ligne=mysql_fetch_array($q->QUERY_SQL($sql,"artica_events"));
	$xdate=$ID;
	
	$filename=strtotime($xdate).".tar.gz";
	$fsize = strlen($ligne["file"]);
	if(!$GLOBALS["VERBOSE"]){
