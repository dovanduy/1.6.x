...
function ShowID_js(){
	...
	$sql="SELECT subject FROM stats_admin_events WHERE ID=$id";
	$q=new mysql();
...
$tpl=new templates();
$sql="SELECT content FROM stats_admin_events WHERE ID={$_GET["ShowID"]}";
$q=new mysql();
$ligne=@mysql_fetch_array($q->QUERY_SQL($sql,"artica_events"));
...
function empty_table(){
	$q=new mysql();
	$q->QUERY_SQL("TRUNCATE TABLE stats_admin_events","artica_events");
}
...
function popup(){
	...
	$q=new mysql();
	$results=$q->QUERY_SQL("SELECT filename FROM stats_admin_events GROUP BY filename ORDER BY filename","artica_events");
