...
var wbl;
	...
	function PostfixMultiStartPage(){
		YahooWinS(900,'$page?popup=yes&ou={$_GET["ou"]}&hostname={$_GET["hostname"]}','$title');
...
<ul>
	<li><a href=\"$page?rule-reject=yes&hostname=$hostname&ou=$ou\"><span style='font-size:14px'>{blockips}</span></a></li>
	<li><a href=\"$page?ruleform=yes&hostname=$hostname&ou=$ou\"><span style='font-size:14px'>{postfix_regex}</span></a></li>
	<li><a href=\"$page?SimpleWords=yes&hostname=$hostname&ou=$ou\"><span style='font-size:14px'>{RegexSimpleWords}</span></a></li>
</ul>
<script>
...
function postfix_check_client_access(){
	...
	var x_check_client_access_multi_add=function(obj){
      ...
      if(document.getElementById('main_config_regex')){RefreshTab('main_config_regex');}
      execute_function('RefreshExtraPostfixRegexTab');
	}	
...
<thead class='thead'>
	<tr>
		<th width=1%>".imgtootltip("plus-24.png","{add}","check_client_access_multi_white_add()")."</th>
		<th width=99% colspan=3>{servers}</th>
	</tr>
</thead>
<tbody class='tbody'>		
";	
	
	if(!is_array($hash)){return $tpl->_ENGINE_parse_body("$html</table><i>{no_rules}</i>$js");}
