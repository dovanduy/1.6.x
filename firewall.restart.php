...
function js(){
	$t=$_GET["t"];
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("{save_firewall_settings}");
	$please_wait_building_network=$tpl->javascript_parse_text("{please_wait_building_rules}");
	$please_wait_restarting_network=$tpl->javascript_parse_text("{please_wait_restarting_firewall}");
	
	
	
	@unlink(dirname(__FILE__)."/ressources/logs/web/squid.status.html");
	$compile_ask=$tpl->javascript_parse_text("{apply_firewall_configuration_warn}");
	...
	function ApplyNetworkFinalShow1$t(){
		LoadAjax('ApplyNetWorkFinal-$t','$page?ApplyNetWorkFinal-tests=yes&t=$t');
...
function execute(){
	...
	$t=$_GET["t"];	
	$sock=new sockets();
	$sock->getFrameWork("network.php?firewall-reconfigure=yes&stay=yes");
...
function ApplyNetWorkFinal_tests(){
	$t=$_GET["t"];
	$datas=@file_get_contents("/usr/share/artica-postfix/ressources/logs/web/exec.firewall.php.html");
	if(strlen($datas)<10){return;}
	$sock=new sockets();
