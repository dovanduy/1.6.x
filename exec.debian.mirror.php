...
if($argv[1]=="--debian-size"){debian_size();die();}
if($argv[1]=="--pid"){$GLOBALS["VERBOSE"]=true;echo RSYNC_PID()."\n";die();}
if($argv[1]=="--start"){$GLOBALS["OUTPUT"]=true;start();die();}
if($argv[1]=="--stop"){$GLOBALS["OUTPUT"]=true;stop();die();}
if($argv[1]=="--kill"){$GLOBALS["OUTPUT"]=true;kill_process();die();}
if($argv[1]=="--logs"){$GLOBALS["OUTPUT"]=true;ChecksLogs();die();}
if($argv[1]=="--schedule"){$GLOBALS["OUTPUT"]=true;rsync_mirror_execute(true);ChecksLogs();die();}
...
function kill_process(){
	...
	if(!is_numeric($MirrorDebianMaxExecTime)){die();}
	if($MirrorDebianMaxExecTime==0){die();}
	if($MirrorDebianMaxExecTime<5){die();}
	
	
	$pid=RSYNC_PID();
	$processtime=$unix->PROCCESS_TIME_MIN($pid);
	if($processtime<$MirrorDebianMaxExecTime){die();}
...
function debian_size(){
	...
	if(is_overloaded()){if($GLOBALS["VERBOSE"]){echo "Overloaded system...\n";}return;}
