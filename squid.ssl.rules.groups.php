...
function items_js(){
	...
	$MyTime=time();
	$html="
	<input type='hidden' id='SSL_RULES_GROUPS_ID' value='table-items-$t'>
...
function items_negation(){
	...
	$sql="UPDATE sslrules_sqacllinks SET negation={$_POST["value"]} WHERE zmd5='$md5'";
	$q=new mysql_squid_builder();
	$q->QUERY_SQL($sql);
...
function items_unlink(){
	...
	$sql="DELETE FROM sslrules_sqacllinks WHERE zmd5='$md5'";
	$q=new mysql_squid_builder();
	$q->QUERY_SQL($sql);
	if(!$q->ok){echo $q->mysql_error;return;}
...
function items_link(){
	...
	$gpid=$_POST["gpid"];
	$md5=md5($aclid.$gpid);
	$sql="INSERT IGNORE INTO sslrules_sqacllinks (zmd5,aclid,gpid,zOrder) VALUES('$md5','$aclid','$gpid',1)";
	$q=new mysql_squid_builder();
	$q->QUERY_SQL($sql);
	if(!$q->ok){echo $q->mysql_error;return;}
...
function items_list(){
	...
	WHERE sslrules_sqacllinks.gpid=webfilters_sqgroups.ID AND sslrules_sqacllinks.aclid=$ID
	ORDER BY sslrules_sqacllinks.zOrder
	) as t";
	
	$page=1;

	if($q->COUNT_ROWS("sslrules_sqacllinks")==0){json_error_show("No datas");}
...
function items_link_order(){
	...
	$aclid=$_POST["aclid"];
	$table="sslrules_sqacllinks";
	//up =1, Down=0
	$q=new mysql_squid_builder();
	$sql="SELECT zOrder FROM sslrules_sqacllinks WHERE zmd5='$mkey'";
	$ligne=mysql_fetch_array($q->QUERY_SQL($sql));
	
	$OlOrder=$ligne["zOrder"];
	if($direction==1){$NewOrder=$OlOrder+1;}else{$NewOrder=$OlOrder-1;}
	$sql="UPDATE sslrules_sqacllinks SET zOrder='$OlOrder' WHERE zOrder='$NewOrder' AND aclid='$aclid'";
//	echo $sql."\n";
	$q->QUERY_SQL($sql);
	if(!$q->ok){echo $q->mysql_error;}
	$sql="UPDATE sslrules_sqacllinks SET zOrder='$NewOrder' WHERE zmd5='$mkey'";
	$q->QUERY_SQL($sql);
//	echo $sql."\n";
	if(!$q->ok){echo $q->mysql_error;}
	
	$results=$q->QUERY_SQL("SELECT zmd5 FROM sslrules_sqacllinks WHERE aclid='$aclid' ORDER BY zOrder");
	$c=1;
	while ($ligne = mysql_fetch_assoc($results)) {
		$zmd5=$ligne["zmd5"];
		$q->QUERY_SQL("UPDATE sslrules_sqacllinks SET zOrder='$c' WHERE zmd5='$zmd5'");
		$c++;
