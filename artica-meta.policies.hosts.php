...
function page(){
	...
	<table class='ARTICA_META_POLICYHOSTS_TABLE' style='display: none' id='ARTICA_META_POLICYHOSTS_TABLE' style='width:1200px'></table>
	<script>
	$(document).ready(function(){
	$('#ARTICA_META_POLICYHOSTS_TABLE').flexigrid({
	url: '$page?search=yes&policy-id={$_GET["policy-id"]}',
	dataType: 'json',
	colModel : [
	{display: '$hosts', name : 'hostname', width : 453, sortable : true, align: 'left'},
...
function search(){
	...
	$table="(SELECT metahosts.hostname,metahosts.hostag,
			metahosts.uuid	FROM metahosts,metagroups_link,metapolicies_link WHERE
			metagroups_link.uuid=metahosts.uuid
			AND metagroups_link.gpid=metapolicies_link.gpid
			AND metapolicies_link.`policy-id`={$_GET["policy-id"]}) as t";
