...
if(isset($argv[1])){
	...
	if($unix->process_exists($oldpid,basename(__FILE__))){writelogs(basename(__FILE__).":Already executed PID: $oldpid ++{$argv[1]}.. aborting the process",basename(__FILE__),__FILE__,__LINE__);die();}
...
if($argv[1]=='--login-script-user'){LogonScriptsUser($argv[2]);die();}
...
if($argv[1]=='--smbpasswd'){smbpasswd();die();}
...
function ParseHomeDirectories(){
	...
	$EnableSambaActiveDirectory=$sock->GET_INFO("EnableSambaActiveDirectory");
		...
		if($EnableSambaActiveDirectory==1){
			...
			$NumTot=mysql_num_rows($results);
			system_admin_events("$NumTot users to check", __FUNCTION__, __FILE__, __LINE__, "samba");
			while($ligne=@mysql_fetch_array($results,MYSQL_ASSOC)){
				...
				if(!is_dir($homeDirectory)){
					@mkdir($homeDirectory,true,0755);
					system_admin_events("$homeDirectory has been created for $uid", __FUNCTION__, __FILE__, __LINE__, "samba");
				}
...
function smbpasswd(){
	...
	if(!is_file($smbpasswd)){
		system_admin_events("smbpasswd no such binary",__FUNCTION__,__FILE__,__LINE__, "samba");
		return;
	}
	$ldap=new clladp();
	...
	system_admin_events("Samba ldap password updated\n".@implode("\n", $results),__FUNCTION__,__FILE__,__LINE__, "samba");
}
...
function reconfigure(){
	...
	shell_exec($unix->LOCATE_PHP5_BIN()." /usr/share/artica-postfix/exec.pam.php --build");
...
function LogonScriptsUser($uid){
	...
	$u=new user($uid);
	if($u->dn==null){return;}
	if($u->NotASambaUser){system_admin_events("$uid is not a Samba user",__FUNCTION__,__FILE__,__LINE__,"samba");return;}
	...
	system_admin_events("logon script $scriptName for $uid updated.",__FUNCTION__,__FILE__,__LINE__,"samba");
	return true;
}
...
function LogonScripts(){
	...
	system_admin_events("$count logon scripts updated.",__FUNCTION__,__FILE__,__LINE__,"samba");
...
function monit_config_smbd(){
	...
	$reloadmonit=false;
	$monit_file="/etc/monit/conf.d/APP_SAMBA_SMBD.monitrc";
	...
	if($MonitConfig["watchdog"]==0){
		...
		if(is_file($monit_file)){
			...
			@unlink("/usr/sbin/smbd-monit-stop");
			$reloadmonit=true;}
	}
	...
	if($MonitConfig["watchdog"]==1){
		...
		$f[]="check process smbd";
   		...
   		$f[]="start program = \"/usr/sbin/smbd-monit-start\"";
   		$f[]="stop program =  \"/usr/sbin/smbd-monit-stop\"";
	   ...
	   $f[]="/etc/init.d/artica-postfix start samba";
	   $f[]="exit 0\n";
 	   @file_put_contents("/usr/sbin/smbd-monit-start", @implode("\n", $f));
 	   shell_exec("$chmod 777 /usr/sbin/smbd-monit-start");
	   $f=array();
	   $f[]="#!/bin/sh";
	   $f[]="PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/X11R6/bin";
	   $f[]="/etc/init.d/artica-postfix stop samba";
	   $f[]="exit 0\n";
 	   @file_put_contents("/usr/sbin/smbd-monit-stop", @implode("\n", $f));
 	   shell_exec("$chmod 777 /usr/sbin/smbd-monit-stop");	   
	}
...
function monit_config_nmbd(){
	...
	if(!is_numeric($MonitConfig["watchdogMEM"])){$MonitConfig["watchdogMEM"]=1500;}	
	$reloadmonit=false;
	$monit_file="/etc/monit/conf.d/APP_SAMBA_NMBD.monitrc";
	...
	if($MonitConfig["watchdog"]==0){
		...
		if(is_file($monit_file)){
			...
			@unlink("/usr/sbin/nmbd-monit-stop");
			$reloadmonit=true;}
	}
	...
	if($MonitConfig["watchdog"]==1){
		...
		$f[]="check process nmbd";
   		$f[]="with pidfile $pidfile";
   		$f[]="start program = \"/usr/sbin/nmbd-monit-start\"";
   		$f[]="stop program =  \"/usr/sbin/nmbd-monit-stop\"";
   		if($MonitConfig["watchdogMEM"]){
  			$f[]="if totalmem > {$MonitConfig["watchdogMEM"]} MB for 5 cycles then alert";
   		}
   		if($MonitConfig["watchdogCPU"]>0){
	   ...
	   $f[]="/etc/init.d/artica-postfix start samba";
	   $f[]="exit 0\n";
 	   @file_put_contents("/usr/sbin/nmbd-monit-start", @implode("\n", $f));
 	   shell_exec("$chmod 777 /usr/sbin/nmbd-monit-start");
	   $f=array();
	   $f[]="#!/bin/sh";
	   $f[]="PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/X11R6/bin";
	   $f[]="/etc/init.d/artica-postfix stop samba";
	   $f[]="exit 0\n";
 	   @file_put_contents("/usr/sbin/nmbd-monit-stop", @implode("\n", $f));
 	   shell_exec("$chmod 777 /usr/sbin/nmbd-monit-stop");	   
	}
