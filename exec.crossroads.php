...
function multiples_start(){
	...
	while($ligne=@mysql_fetch_array($results,MYSQL_ASSOC)){	
		...
		$www_port_enabled=$arrayConf["GLOBAL_SETTINGS"]["www_port_enabled"];
		$www_port=$arrayConf["GLOBAL_SETTINGS"]["www_port"];
		$www_password=$arrayConf["GLOBAL_SETTINGS"]["www_password"];
		$www_user=$arrayConf["GLOBAL_SETTINGS"]["www_user"];
		$buffer=$arrayConf["GLOBAL_SETTINGS"]["buffer"];
		if(!is_numeric($buffer)){$buffer=2048;}
		...
		if($pid==0){
			...
			if($buffer>2048){$cd[]="--buffer-size $buffer";}
			
			if($www_port_enabled==1){
				if(is_numeric($www_port)){
					$cd[]="--web-interface $ipaddr:$www_port:\"$ipaddr\"";
					if($www_password<>null && $www_user<>null){$cd[]="--web-interface-auth $www_user:\"$www_password\"";}
				}
			}
