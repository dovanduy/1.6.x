...
if($argv[1]=="--backup"){perform_db_backup(0,$argv[2],$argv[3],1);die();}
if($argv[1]=="--restore"){perform_db_restore($argv[2],$argv[3],$argv[4]);die();}
...
if(systemMaxOverloaded()){
	system_admin_events("This system is too many overloaded, die()",__FUNCTION__,__FILE__,__LINE__,"backup");
	die();
}
...
function perform_db_restore($InstanceID,$database,$sourcefile){
	...
	if(!is_file($sourcefile)){
		system_admin_events("$database: `$sourcefile` no such file...",__FUNCTION__,__FILE__,__LINE__,"mysql-restore");
		return;
	}
	...
	if(!is_file($mysqlbin)){
		system_admin_events("$database: `mysql` no such binary...",__FUNCTION__,__FILE__,__LINE__,"mysql-restore");
		return;
	}	
	...
	if($extension=="gz"){
		if(!is_file($gunzip)){
			system_admin_events("$database: `gunzip` no such binary...",__FUNCTION__,__FILE__,__LINE__,"mysql-restore");
			return;
		}
...
function perform_db_backup($InstanceID,$database,$targetDir,$compress){
	...
	if(!is_dir($targetDir)){system_admin_events("$database:[$InstanceID]:$targetDir no such directory",__FUNCTION__,__FILE__,__LINE__, "backup");return false;}
	$t=time();
	@file_put_contents("$targetDir/$t", time());
	if(!is_file("$targetDir/$t")){system_admin_events("$database:[$InstanceID]:$targetDir permission denied",__FUNCTION__,__FILE__,__LINE__, "backup");return false;}
	@unlink("$targetDir/$t");
	if(!is_numeric($InstanceID)){$InstanceID=0;}
...
function perform_db_clean($InstanceID,$database,$MaxDay){
	...
	$q=new mysql();
	$results = $q->QUERY_SQL($sql,"artica_backup");
	if(!$q->ok){system_admin_events("$database:[$InstanceID]: $q->mysql_error", __FUNCTION__, __FILE__, __LINE__, "backup");return;}
	$c=0;
	while ($ligne = mysql_fetch_assoc($results)) {
	...
	if($c>0){
			system_admin_events("$database:[$InstanceID]: $c deleted container(s)", __FUNCTION__, __FILE__, __LINE__, "backup");
	}
